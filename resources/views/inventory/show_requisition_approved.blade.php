                        ...
                        <form action="{{ route('requisition_approved.store') }}" method="post">
                            <table id="student_list" class="table table-striped">
                                ...
                                <tr>
                                    ...
                                    @endphp
                                    <td><input id="{{$val->id}}" value="{{$val->id}}" name="requisitions[]" type="checkbox" {{$readonly}}></td> 
                                    ...
                                    <td>{{$val->item_qty}}</td>
                                    <td>
                                        <input type="number" name="approved_qty[{{$val->id}}]" value="{{$val->approved_qty}}" class="form-control" {{$readonly}}>
                                    </td>
                                    <td>{{$val->expected_delivery_time}}</td>
                                    <td>{{$val->remarks}}</td>                                   
                                    @if( isset($data['requisition_status_data']))
                                    <td>
                                        <select name="requisition_status[{{$val->id}}]" class="form-control" {{$readonly}}>
                                            <option value="">Select</option>
                                                @foreach ($data['requisition_status_data'] as $ekey =>$status)
                                                    <option @if($val->requisition_status == $status->requisition_status) selected="selected" @endif value="{{$status->requisition_status}}">{{$status->title}}</option>
                                                @endforeach
                                        </select>
                                    </td>
                                    @endif 
                                    ...
                                    <td>
                                        <textarea class="form-control" rows="2" name="requisition_approved_remarks[{{$val->id}}]" {{$readonly}}>{{$val->requisition_approved_remarks}}</textarea>
                                    </td>
                                    <td style="width: 11%;">
                                        <a href="{{ route('add_item_direct_purchase.create') }}" class="btn btn-info add-new" target="_blank">Direct Purchase</a>
                                    </td>
                                </tr>
                                @endforeach
                                <tr>                                           
