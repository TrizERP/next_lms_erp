			...
			<div class="white-box">
				<div class="panel-body">
					...
					<div class="col-lg-3 col-sm-3 col-xs-3">
						{{--
						<a href="{{ route('lo_master.create') }}" class="btn btn-info add-new">
					...
					<div class="card">
						<form action="{{ route('lo_marks_entry.store') }}" enctype="multipart/form-data" method="post">
								...
								<table id="example" class="table table-striped table-bordered dataTable" style="width:100%">
									<thead>
										<tr>
											...
											<th>Name</th>
											<th>Standard</th>
											@foreach ($data['questions'] as $question=>$mark) @php $ids = explode('--',$question); $que = $ids[0]; $id = $ids[1]; @endphp
											<th>{{ $que }}({{ $mark }})</th>
											@endforeach
									...
									<tbody>
										@php $i=1; @endphp @foreach($data['stud'] as $key => $datas)
										<tr id={{$datas->ID}}>
											<td>{{$i}}</td>
											<td>{{$datas->roll_no}}</td>
											<td>{{$datas->name}}</td>
                                            ...
                                            @endphp
											<td>
												<input type="text" class="form-control" onchange="return validateMarks('{{ $mark }}',this,'{{ $datas->student_id }}')"
												 name="result[{{ $datas->student_id }}][{{ $id }}]" value="{{ $datas->$resultQid }}" id="result[{{ $datas->student_id }}][{{ $id }}]">
											</td>
											@endforeach
											<td>
												<input type="text" class="form-control" name="resultTotal[{{ $datas->student_id }}]" value="{{$datas->total}}" readonly=readonly
							...
							<center>
								<input type="submit" value="Submit" onclick="return validateMarksError();" class="btn btn-success">
							</center>
						</form>
					</div>
				</div>
