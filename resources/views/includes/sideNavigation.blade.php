                            ...
                            <div class="sub-drop-body">
                                ...
                                @if(!empty($subChildmenuMaster[$submenuValue['id']]))   
                                    @foreach($subChildmenuMaster[$submenuValue['id']] as $subChildmenuKey => $subChildmenuValue)
                                            ...
                                            <li> <a href="javascript:void(0);" onclick="sessionMenu({{ $subChildmenuValue['id'] }});"><i class="{{ $subChildmenuValue['icon'] }}" data-icon="v"></i><span class="hide-menu" id="{{ $subChildmenuValue['name'] }}">{{ $subChildmenuValue['name'] }}</span></a> </li>
                                            @else                       
                                            <li><a href="{{ route($subChildmenuValue['link']) }}" onclick="sessionMenu({{ $subChildmenuValue['id'] }});redirect_pages_soni('{{ route($subChildmenuValue['link']) }}','{{$submenuValue['id']}}','{{$i}}');"><i class="{{ $subChildmenuValue['icon'] }}" data-icon="v"></i><span class="hide-menu" id="{{ $subChildmenuValue['name'] }}">{{ $subChildmenuValue['name'] }}</span></a></li>
                                            @endif                                                                                      
                                        @endforeach
