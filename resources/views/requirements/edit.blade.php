...
<div id="page-wrapper">
    <div class="container-fluid">
        ...
        <div class="card">
            <form action="{{route('requirements.update',$data['editData']['id'])}}" method="POST" enctype="multipart/form-data">
                {{ method_field('PUT') }}
                <div class="row">
                    <div class="col-md-4">
                        <label for="">Menu Name</label>
                        <select name="menuDetails" id="menuDetails" class="form-control" disabled>
                            <option value="">Select Menu</option>
                            @foreach($data['allMenu'] as $key=>$value)
                            <option value="{{$value['parent_menu_id'].'/'.$value['menu_title']}}" @if(isset($data['editData']['menu_id']) && $value['parent_menu_id']==$data['editData']['menu_id']) selected @endif>{{$value['menu_title']}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-12">
                        <label for="topicType">Add Process</label>
                        <textarea name="requirements" id="requirementText" contenteditable="true">{{$data['editData']['requirements']}}</textarea>
                    </div>
                    ...
                    <div class="col-md-12">
                        <center>
                            <input type="submit" value="Update" id="submit" class="btn btn-success col-md-2">
                        </center>
                    </div>
                    @endif
                </div>
