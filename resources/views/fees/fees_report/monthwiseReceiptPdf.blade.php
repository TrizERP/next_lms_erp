        ...
        <div class="container-fluid">
            <div class="row bg-title">
                <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                    <h4 class="page-title">Receipt PDF</h4>
                </div>
            </div>
            @php
                ...
                $fees_type = $data['fees_type'] ?? 'regular';
            @endphp
            <div class="card">
                ...
                <div class="col-md-4 form-group ml-0 mr-0">
                    ...
                    <select name="fees_type" id="fees_type" class="form-control" required>
                        ...
                        <option value="regular"
                            {{ isset($data['fees_type']) && $data['fees_type'] == 'regular' ? 'selected' : '' }}>Regular
                            Fees</option>
                        <option value="other"
                            {{ isset($data['fees_type']) && $data['fees_type'] == 'other' ? 'selected' : '' }}>Other Fees
                        </option>
                    </select>
                </div>
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-striped">
                            <thead>
                                <tr>
                                    <th>SR No.</th>
                                    <th>{{ $fees_type == 'other' ? 'Receipt ID' : 'Receipt No.' }}</th>
                                    <th>{{ $fees_type == 'other' ? 'Deduction Date' : 'Received Date' }}</th>
                                    <th>{{ App\Helpers\get_string('studentname', 'request') }}</th>
                                    <th>{{ App\Helpers\get_string('std/div', 'request') }}</th>
                                    <th class="text-left">{{ $fees_type == 'other' ? 'Deduction Amount' : 'Paid Amount' }}
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($data['fees_details'] as $key => $value)
                                    <tr>
                                        ...
                                        @if ($fees_type == 'other')
                                            ...
                                            <textarea name="studentData[{{ $value->student_id }}][0]" class="d-none">{{ $value->id }}</textarea>
                                            <textarea name="studentData[{{ $value->student_id }}][1]" class="d-none">{{ $value->paid_fees_html ?? '' }}</textarea>
                                        @else
                                            {{--  Regular Fees --}}
                                            ...
                                            <textarea name="studentData[{{ $value->student_id }}][0]" class="d-none">{{ $value->receipt_no }}</textarea>
                                            <textarea name="studentData[{{ $value->student_id }}][1]" class="d-none">{{ $value->fees_html }}</textarea>
                                        @endif
                                    </tr>
                                @endforeach
                            </tbody>
