    ...
    <div class="container-fluid mb-5">
        <div class="coursr-chp-list" id="cource-chap-list">
            ...
            @foreach($data['data'] as $key => $chdata)
                ...
                <div class="col-md-4 mb-2 chp-details" data-toggle="collapse" href="#collapseExample{{ $collapse }}" role="button" aria-expanded="false" aria-controls="collapseExample">
                ...
                <div class="col-md-8 mb-2 d-md-flex align-items-center justify-content-end total-count">
                        ...
                        @if(Session::get('user_profile_name') != 'Student')
                            @if($chdata->total_triz_content > 0)
                                <a href="{{ route('topic_master.index',['id'=>$chdata->id,'content_category'=>'Triz']) }}" class="btn btn-outline-dark mx-1 my-1">Triz</a>
                            @endif
                            @if($chdata->total_OER_content > 0)
                                <a href="{{ route('topic_master.index',['id'=>$chdata->id,'content_category'=>'OER']) }}" class="btn btn-outline-dark mx-1 my-1">OER</a>                                
                            @endif
                            @if( $data['show_content'] == 'chapterwise' )
                                <a target="_blank" href="{{ route('lms_teacherResource.index',['standard_id'=>$chdata->standard_id,'subject_id'=>$chdata->subject_id,'chapter_id'=>$chdata->id]) }}" class="btn btn-outline-dark mx-1 my-1">Teacher Resource</a>
                                <a target="_blank" href="{{ route('lms_lessonplan.index',['standard_id'=>$chdata->standard_id,'subject_id'=>$chdata->subject_id,'chapter_id'=>$chdata->id]) }}" class="btn btn-outline-dark mx-1 my-1">Lesson Planning</a>
                                <a target="_blank" href="{{ route('lmsmapping.index',['chapter_id'=>$chdata->id]) }}" class="btn btn-outline-dark mx-1 my-1">Chapter-wise Mapping</a>
                            @endif
                        @endif
                        ...
                        <a target="_blank" href="{{ route('create_content_master', ['chapter_id' => $chdata->id]) }}" class="btn btn-outline-dark mx-1 my-1">Add Content</a>                                                
                        {{-- Add Chapter wise content : END --}}

                        {{-- Add Chapter wise Question : START --}}
                        <a target="_blank" href="{{ route('question_chapter_master', ['chapter_id' => $chdata->id]) }}" class="btn btn-outline-dark mx-1 my-1">Question Answer</a>
                    ...
                    @endif
                    <a href="javascript:edit_data('{{route('chapter_master.update',$chdata->id)}}','{{$chdata->id}}','{{$chdata->standard_id}}','{{$chapter_name}}','{{$chapter_desc}}','{{$chdata->availability}}','{{$chdata->show_hide}}','{{$chdata->sort_order}}');" class="pr-1 mr-1 border-right text-dark"><i class="mdi mdi-pencil-outline m-0"></i></a>                    
                    <form action="{{ route('chapter_master.destroy', $chdata->id)}}" method="post" onsubmit="return delete_chapter({{$chdata->id}});">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="pr-1 mr-1 border-right" style="border: none !important;background-color: transparent !important;">
                    ...
                    <div class="chapter-content-list-cola mb-4 collapse" id="chapter-content-tar-list-{{ $chdata->id }}-{{ $subColapse }}" data-parent="#cource-chap-list" style="">
                        ...
                        <div class="row chapter-content-box my-2 py-2 mx-0">
                            ...
                            <div class="col-md-9 chapter-details">
                                <a href="{{ $content_file_url }}" class="chapter-title" target="_blank">{{ $single_content['title'] }}</a>
                                <div class="chapter-des">{{ $single_content['description'] }}</div>
                            </div>
                            <div class="col-md-2 time text-secondary d-flex justify-content-end" style="font-size: 20px;">
                                <a href="{{ route('lms_flashcard.index',['content_id' => $single_content['id']])}}" target="_blank"
                                    class="btn btn-outline-warning btn-sm mx-1" data-toggle="tooltip" title=""
                                    data-original-title="Add Flash Card"><i class="mdi mdi-cards-playing-outline"></i></a>
                                <a href="{{ route('content_master.edit',['id' => $single_content['id'],'std_id'=>$single_content['standard_id']])}}"
                                    class="btn btn-outline-success btn-sm mx-1"><i class="mdi mdi-pencil-outline"></i></a>
                                <form action="{{ route('content_master.destroy', $single_content['id'] )}}" method="post">
                                    @csrf
                                    @method('DELETE') 
