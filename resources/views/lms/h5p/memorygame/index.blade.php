...
@section('container')
{{--
    Memory game -- legacy Blade surface.

    The authoring UI and the player for this type live in the Next.js app
    (app/h5p/h5p_memory_game/*), which is where every H5P surface is being built
...
<div class="container-fluid py-3">
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            <h4 class="mb-1">Memory game</h4>
            <p class="text-muted mb-0" style="font-size: 13px;">
                Learners match pairs of cards to build recall.
            </p>
        </div>
        <span class="badge bg-secondary">H5P.MemoryGame</span>
    </div>
    ...
    <div class="card">
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    @forelse(($memoryGameLists ?? []) as $i => $row)
                        <tr>
                            <td class="text-muted">{{ $i + 1 }}</td>
                            <td>{{ $row['title'] ?? 'Untitled' }}</td>
                            <td class="text-muted">{{ count($row['cards'] ?? []) }}</td>
                            <td>
                            ...
                            <td>
                                <a class="btn btn-sm btn-outline-secondary"
                                   href="{{ route('h5p_memory_game.export', ['id' => $row['id']]) }}">
                                    Export .h5p
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">
                                ...
                                <strong>H5P content &rarr; Memory game</strong>.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
