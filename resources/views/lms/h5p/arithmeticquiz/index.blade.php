...
@section('container')
{{--
    Arithmetic quiz -- legacy Blade surface.

    The authoring UI and the player for this type live in the Next.js app
    (app/h5p/h5p_arithmetic_quiz/*), which is where every H5P surface is being built
...
<div class="container-fluid py-3">
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            <h4 class="mb-1">Arithmetic quiz</h4>
            <p class="text-muted mb-0" style="font-size: 13px;">
                Generated arithmetic drills for fluency practice.
            </p>
        </div>
        <span class="badge bg-secondary">H5P.ArithmeticQuiz</span>
    </div>
    ...
    <div class="card">
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    @forelse(($arithmeticQuizLists ?? []) as $i => $row)
                        <tr>
                            <td class="text-muted">{{ $i + 1 }}</td>
                            <td>{{ $row['title'] ?? 'Untitled' }}</td>
                            <td class="text-muted">{{ (int) ($row['max_questions'] ?? 0) }}</td>
                            <td>
                            ...
                            <td>
                                <a class="btn btn-sm btn-outline-secondary"
                                   href="{{ route('h5p_arithmetic_quiz.export', ['id' => $row['id']]) }}">
                                    Export .h5p
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">
                                ...
                                <strong>H5P content &rarr; Arithmetic quiz</strong>.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
