...
<div id="page-wrapper">
    <div class="container-fluid mb-5">
            ...
            <div class="header-content">
                ...
                <div class="header-right">
                    <div class="header-stats">
                        ...
                        {{ count($data['videolists']) }} videos
                    </div>
                    <div class="header-stats">
                        <a href="{{route('h5p_interactive_video.create',['chapter_id' => $data['chapter_id'],'standard_id'=>$data['standard_id'],'subject_id'=>$data['subject_id']])}}" class="text-white">
        ...
        <div class="table-responsive">
            <table id="videoTable" class="table table-stripped" style="width:100%">
                <thead>
                    <tr>
                        <th>SR No</th>
                        <th>Title</th>
                ...
                <tbody>
                    @if(count($data['videolists']) > 0)
                    @foreach($data['videolists'] as $key => $video)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{$video->title}}</td>
                        <td><a href="{{$video->video_path}}" target="_blank">{{$video->title}}</a></td>
                        <td>{{ $video->interactions_count ?? $video->interactions->count() }} Points</td>
                        <td>
                             
                            <div class="action-buttons">
                                <a href="{{ route('h5p_interactive_video.show', $video->id) }}?chapter_id={{$data['chapter_id']}}&standard_id={{$data['standard_id']}}&subject_id={{$data['subject_id']}}" class="btn-view">
                                    <i class="fas fa-eye"></i>
                                </a>
                            @if(!in_array(session()->get('user_profile_name'),['student','Student','STUDENT']))
                                <a href="{{ route('h5p_interactive_video.edit', $video->id) }}?chapter_id={{$data['chapter_id']}}&standard_id={{$data['standard_id']}}&subject_id={{$data['subject_id']}}" class="btn-edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('h5p_interactive_video.destroy', $video->id)}}"
...
<script>
    $(document).ready(function() {
        ...
        $('#videoTable thead tr').clone(true).appendTo('#videoTable thead');
        $('#videoTable thead tr:eq(1) th').each(function(i) {
