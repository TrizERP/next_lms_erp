         ...
         <div class="row bg-title align-items-center justify-content-between">
            ...
            <div class="col-md-3 mb-4 text-md-right">
                @if(isset($data['chapter_topic_data']['chapter_topic_id']) && $data['chapter_topic_data']['action'] == 'chapter')                                        
                    <a href="{{ route('lmsmapping.create',['chapter_id'=>$data['chapter_topic_data']['chapter_topic_id']]) }}" class="btn btn-info add-new"><i class="fa fa-plus"></i> Add LMS Mapping</a>
                @elseif(isset($data['chapter_topic_data']['chapter_topic_id']) && $data['chapter_topic_data']['action'] == 'topic')
                    <a href="{{ route('lmsmapping.create',['topic_id'=>$data['chapter_topic_data']['chapter_topic_id']]) }}" class="btn btn-info add-new"><i class="fa fa-plus"></i> Add LMS Mapping</a>
                @else
                    <a href="{{ route('lmsmapping.create') }}" class="btn btn-info add-new"><i class="fa fa-plus"></i> Add LMS Mapping</a>
                @endif                
            </div>                       
        </div>
                    ...
                    <table id="subject_list" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>                           
                            ...
                            <tr>    
                                ...
                                <td>                                   
                                    <a href="{{ route('lmsmapping.edit',['id'=>$lmsdata['id']])}}" class="btn btn-outline-success btn-sm"><i class="ti-pencil-alt"></i></a>                                                                        
                                    
                                    @if( !isset($lmsdata['CHILD_ARR']) )
                                    <form action="{{ route('lmsmapping.destroy', $lmsdata['id'])}}" method="post" class="btn btn-outline-danger btn-sm">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirmDelete();" type="submit" class="border-0 bg-transparent"><i class="ti-trash"></i></button>																		                                                                     
                                    </form>                               
                                    @endif     
                                </td>                               
                            </tr>
                                ...
                                <tr>    
                                    ...
                                    <td>                                        
                                        <a href="{{ route('lmsmapping.edit',['id'=>$child_data['id']])}}" class="btn btn-outline-success btn-sm"><i class="ti-pencil-alt"></i></a>                                                                        
                                        
                                        <form action="{{ route('lmsmapping.destroy', $child_data['id'])}}" method="post" class="btn btn-outline-danger btn-sm">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirmDelete();" type="submit" class="border-0 bg-transparent"><i class="ti-trash"></i></button>                                                                                                                                          
                                        </form>                                    
