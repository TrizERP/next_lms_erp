...
<div id="page-wrapper">
    <div class="container-fluid">
        ...
        @endphp
        <div class="card">                
            <form method="POST" enctype="multipart/form-data" action="{{ route('lmsAssignment_submission.store') }}">
                <div class="row">                    
                            ...
                            <table id="example" class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>                                       
                                        ...
                                        @else
                                            <td><input type="file" id="image[{{$data['id']}}]" name="image[{{$data['id']}}]" class="form-control"></td>
                                        @endif  
                                        <td>{{$data['teacher_remarks']}}</td>                                       
                                        @if($data['teacher_submission_status'] == 'Y')
                                            <td>
                                                <a target="_blank" href="{{ route('lmsAssignment_submission.show',['id'=>$data['id'],'student_id'=>$data['student_id'],'question_paper_id'=>$data['exam_id']]) }}" class="btn btn-info btn-outline btn m-r-5">View Assignment</a>     
                                            </td>
                                        @else
                                            <td></td>
                                        @endif                                                                      
