                        ...
                        <div class="col-md-3 form-group">
                            ...
                            <select id='order_by' name="order_by" class="form-control">
                                ...
                                <option @if ($order_by == 'standard_id') selected="selected" @endif value="standard_id">
                                    {{ App\Helpers\get_string('standard', 'request') }}</option>
                                <option @if ($order_by == 'enrollment_no') selected="selected" @endif value="enrollment_no">
                                    {{ App\Helpers\get_string('grno', 'request') }}</option>
                                <option @if ($order_by == 'roll_no') selected="selected" @endif value="roll_no">Roll No
                                </option>
                                <option @if ($order_by == 'last_name') selected="selected" @endif value="last_name">Last
                                    Name</option>
                            </select>
                        </div>
                        <div class="col-md-3 col-sm-offset-4 text-center form-group">
                            <input type="submit" name="submit" value="Search" class="btn btn-success triz-btn">
                            <div class="btn btn-outline-primary btn-sm ml-2 py-2 px-3 cursor-pointer" data-toggle="modal"
                                data-target="#modalCenter"><span class="mdi mdi-tune"></span></div>
                        </div>
                    </div>
                    <!-- Modal -->
                                ...
                                <thead>
                                    <tr>
                                        @foreach ($data['headers'] as $hkey => $header)
                                            <th data-toggle="tooltip" title="{{ $header['name'] }}"> {{ $header['name'] }}
                                            </th>
                                        @endforeach
                                                ...
                                                @else
                                                    @if ($header['type'] == 'textbox')
                                                        @if ($hkey == 'mobile' || $hkey == 'mother_mobile')
                                                            <td><input type="text" pattern="[1-9]{1}[0-9]{9}"
                                                                    name="values[{{ $value->id }}][{{ $hkey }}]"
                                                                    value="{{ $value->$hkey }}" class="form-control"></td>
                                                        @elseif($hkey == 'email')
                                                            <td>{{ $value->$hkey }}</td>
                                                        @else
                                                            <td><input type="text"
                                                    ...
                                                    @elseif($header['type'] == 'file')
                                                        <td><input type="file"
                                                                name="values[{{ $value->id }}][{{ $hkey }}]">
                                                            @if ($value->$hkey != '' && $hkey == 'image')
                                                                <img height="50px" width="50px"
                                                                    src="/storage/student/{{ $value->$hkey }}">
                                                                @elseif($value->$hkey!='' && in_array($hkey, ['father_image', 'mother_image']))
                                                                        <img height="50px" width="50px"
                                                                        src="https://s3-triz.fra1.cdn.digitaloceanspaces.com/public/parents_image/{{ $value->$hkey }}">
                                                            @endif
                                                        </td>
                                                    @endif
                                                @endif
