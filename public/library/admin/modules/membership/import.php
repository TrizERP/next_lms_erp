        ...
        while (!feof($file)) {
            ...
            if ($row_count < $record_offset) {
                ...
                if ($field) {
                    ...
                    $member_pass=$field[17]?'\''.$field[17].'\'':'NULL';
                    // sql insert string
                        $sql_str = "INSERT IGNORE INTO member (member_id, member_name, gender, member_type_id, member_email, member_address, postal_code, inst_name, is_new, member_image, pin, member_phone, member_fax, member_since_date, register_date,    expire_date, member_notes,mpasswd, input_date, last_update) VALUES ($member_id, $member_name, $gender,                      $member_type_id, $member_email, $member_address, $postal_code, $inst_name, $is_new, $member_image, $pin, $member_phone,        $member_fax, $member_since_date, $register_date, $expire_date, $member_notes,$member_pass, $curr_datetime, $curr_datetime)";
                    // send query
                    @$dbs->query($sql_str);
                    if (!$dbs->error) {
                        $inserted_row++;
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'membership/import.php class="headerText2">Import Member Data</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
