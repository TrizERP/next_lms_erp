...
//        $this->grace_periode = intval($this->member_type_prop['grace_periode']);
    ...
    // return this item first
      ...
      $_re_borrow = $this->obj_db->query("select c.cat_re_issue_limit from loan as lr
                                                ...
                                                LEFT JOIN category_mast AS c ON l.user_group_id=c.cat_mast_user 
      ...
      //  echo "SELECT renewed FROM loan  WHERE loan_id=$int_loan_id AND member_id='".$_SESSION['memberID']."'";
        ...
        {         
         ...
         $_loan_rules_q = $this->obj_db->query("select c.cat_issue_period,lr.due_date from loan as lr
                                                left join item as t on lr.item_code=t.item_code
                                                left join biblio as b on t.biblio_id=b.biblio_id
                                                LEFT JOIN ".$inte_schema.".tblstudent AS l ON l.SUB_INSTITUTE_ID='$_SESSION[SUB_INSTITUTE_ID]' AND lr.member_id=l.enrollment_no 
                                                LEFT JOIN category_mast AS c ON l.user_group_id=c.cat_mast_user 
    ...
    public function countOverdueValue($int_loan_id, $str_return_date)
    {
        ...
        $_loan_q = $this->obj_db->query("SELECT l.due_date,biblio.material_sub_id as loan_rules_id,l.item_code,l.member_id,biblio.material_sub_id as loan_rules_id  FROM loan AS l left join item on item.item_code=l.item_code left join biblio on biblio.biblio_id=item.biblio_id WHERE l.loan_id=$int_loan_id");
        ...
        if ($_date == $str_return_date) 
        {
            ...
            if (!empty($_loan_d[1])) 
             {                                               
                $sql="select category_mast.cat_fine_each_day  from ".$inte_schema.".tblstudent ts LEFT JOIN category_mast ON ts.user_group_id =category_mast.cat_mast_user where ts.SUB_INSTITUTE_ID='$_SESSION[SUB_INSTITUTE_ID]' AND enrollment_no like '$member_id' and material_sub_id=$material_sub_id";
                $_loan_rules_q = $this->obj_db->query($sql);                                
                if($_loan_rules_q->num_rows==0)
                {
                    $sql="select category_mast.cat_fine_each_day  from ".$inte_schema.".tblstaff ts INNER JOIN category_mast ON ts.user_group_id =category_mast.cat_mast_user where ts.SUB_INSTITUTE_ID='$_SESSION[SUB_INSTITUTE_ID]' AND ts.user_name like '$member_id' and material_sub_id=$material_sub_id";                    
                    $_loan_rules_q = $this->obj_db->query($sql);                                
                }
                $_loan_rules_d = $_loan_rules_q->fetch_row();
