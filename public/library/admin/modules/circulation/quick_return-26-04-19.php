...
<div class="menuBoxInner masterFileIcon">
    ...
    <input type="text" name="keywords_for_quick_return" size="30" />
...
if(isset($_REQUEST['keywords_for_quick_return'])){
    ...
    INNER JOIN ".$inte_schema.".tblstudent AS m ON l.member_id collate utf8_general_ci = m.enrollment_no collate utf8_general_ci
    INNER JOIN item AS i ON l.item_code=i.item_code
    INNER JOIN biblio AS b ON i.biblio_id=b.biblio_id
	inner join ".$cms_database.".STUDENT_ENROLLMENT SE on SE.student_id =m.student_id and SE.SYEAR=".$defaultYear."
	inner join ".$cms_database.".COURSE_SUBJECTS CS on SE.standard_id =CS.subject_id
	inner join ".$cms_database.".SCHOOL_SECTIONS SS on SS.section_id=SE.section_id
	inner join ".$cms_database.".SCHOOL_GRADELEVELS SG on SG.ID= SE.grade_id AND SG.SUB_INSTITUTE_ID='".$_SESSION['SUB_INSTITUTE_ID']."'";
    ...
    if ($can_read AND $can_write) {
		...
		'concat_ws ("-",CS.title,SS.section_name) AS \''.__('Std').'\'',
	...
	/* callback function to show overdue */
     ...
     // {
         ...
         // //if (simbio_date::compareDates($array_data[0], $_curr_date) == $_curr_date) {
           // // return '<strong style="color: #f00;">'.$array_data[0].' '.__('OVERDUED').'</strong>';
        // // } else {
             // return $array_data[0];
         // //}
     // }
     // // modify column value
     // $datagrid->modifyColumnContent(1, 'callback{showOverdue}');
     // set table and table header attributes
	...
	// $datagrid->modifyColumnContent(0, 'callback{callbackSubjectType}');
