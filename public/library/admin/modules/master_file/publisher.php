...
if (isset($_POST['saveData']) AND $can_read AND $can_write) 
{
    ...
    {
        utility::jsAlert(__('Publisher Name Can\'t be Numeric!'));
        exit();        
    }
    ...
    else
    {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            else
            {
                ...
                utility::jsAlert(__('Duplicate Entery!Publisher is already Exist')); 
                exit();
	...
	if($checkflag==1)
	{
		...
		while($rownew = $rec_name->fetch_assoc())
		{
			$publisher_name_set = $rownew['publisher_name'];	
                ...
                $error_num++;
		 utility::jsAlert(__('You can not delete the publisher Type: '.$publisher_name_set.'; because it is associate with the book.'));
	}
	else
	{
...
<tr>
	<td class="tab_menu_top">
                            <ul class="tabs"> 
				<li>
<a href="<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/publisher.php?action=detail" class="headerText2"><?php echo __('Add New Publisher'); ?></a> </li>
<li> 
<a href="<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/publisher.php" class="headerText2"><?php echo __('Publisher List'); ?></a> </li>
</ul>
	</td>
</tr>
</table>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
   ...
   //comment by iresh on 25-1-2011 $form->addTextField('text', 'publisherName', __('Publisher Name').'*', $rec_d['publisher_name'], 'style="width: 60%;"');
    $form->addTextField('text', 'publisherName', __('Publisher Name').'*', $rec_d['publisher_name'], 'style="width: 140px;" onblur="return charactercheck(this.name);" onkeyup="return checkspecialcharacterdynamic(this.name);"');

    // edit mode messagge
    if ($form->edit_mode) {
        echo '<div class="infoBox">'.__('You are going to edit data').' : <b>'.$rec_d['publisher_name'].'</b> <br />'.__('Last Update').$rec_d['last_update'] //mfc
            .'</div>'."\n";
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('p.publisher_id',
            ...
            'DATE_FORMAT(p.last_update,"%d-%m-%Y") AS \''.__('Last Update').'\'');
    } else {
