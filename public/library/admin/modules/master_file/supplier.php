...
else if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) 
{
	...
	$checkflag = 0;
	$rec_f = $dbs->query('SELECT supplier_id from item where supplier_id ='.$itemID);
	...
	if($checkflag==1)
	{
		$rec_name = $dbs->query('SELECT supplier_name from mst_supplier where supplier_id ='.$itemID);
