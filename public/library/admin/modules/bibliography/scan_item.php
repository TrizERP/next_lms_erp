...
if (isset($_REQUEST['keywords_for_scan_book']) AND $_REQUEST['keywords_for_scan_book'] != '') 
{
    ...
    $sql_op->custom_update("SELECT * FROM item_scan_details WHERE item_code = '".$itemCode."' ");

    if($sql_op->affected_rows == 1){
         $sql_op->custom_update("UPDATE item_scan_details SET scan_status = 'Yes',syear = '".$dyear."',created_by = '".$_SESSION['ID']."',created_ip = '".$_SERVER['REMOTE_ADDR']."',remarks = NULL,item_status_id = NULL WHERE item_code = '".$itemCode."' ");
    }else{
         $sql_op->custom_update("INSERT INTO item_scan_details(syear,item_code,scan_status,created_by,created_ip) values('".$dyear."','".$itemCode."','Yes','".$_SESSION['ID']."','".$_SERVER['REMOTE_ADDR']."')");
    }
...
$bradecum .= '</a>->';
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/scan_item.php class="headerText2">Book Verification</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
