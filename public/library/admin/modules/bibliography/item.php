...
// start the session
// echo 'div : '.SENAYAN_BASE_DIR.'admin/default/session.inc.php';
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail'))
{
    ...
    {
		$qry = $dbs->query("SELECT i.item_code, (cast((SUBSTRING(i.item_code,2)) as UNSIGNED) + 1) as max_char,SUBSTRING(i.item_code,1,1) as first_char
							FROM item i
							order by item_id desc limit 1");
           
	   $result = $qry->fetch_assoc();
       $max_char = $result['max_char'];

         if(strlen($max_char)==1)
         {
                $max_char = "00000".$max_char ;
         }
         else if(strlen($max_char)==2)
         {
                $max_char = "0000".$max_char ;
         }
         else if(strlen($max_char)==3)
         {
                $max_char = "000".$max_char ;
         }
		 else if(strlen($max_char)==4)
         {
                $max_char = "00".$max_char ;
         }
		 else if(strlen($max_char)==5)
         {
                $max_char = "0".$max_char ;
         }
		 else
		 ...
		 $items = $result['first_char'].$max_char;	
