...
if (!$can_read) 
{
    die('<div class="errorBox">'.getttext('You are not authorized to view this section').'</div>');
}
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) 
{
    ...
    // check form validity
    if (empty($title)) {
        utility::jsAlert(getttext('Title can not be empty'));
        exit();
    } if($_POST['materialresourceid']=="N/A")
	{
	utility::jsAlert(getttext('Material Resource Type can not be empty'));
        exit();
	}if($_POST['gmdID']==0)
	{
	utility::jsAlert(getttext('Material Type can not be empty'));
        exit();
	}if($_POST['materialsubid']==0)
	{
	utility::jsAlert(getttext('Material Sub Type can not be empty'));
        exit();
	}else {
...
//$data['doc_type'] = trim($dbs->escape_string(strip_tags($_POST['doc_type'])));
        ...
        if (!empty($_FILES['image']) AND $_FILES['image']['size']) {
            ...
            if ($img_upload_status == '1') {
                ...
                utility::jsAlert(getttext('Image Uploaded Successfully'));
            } else {
                // write log
	     ...
	     /* UPDATE RECORD MODE */
            ...
            if ($update) {
            	...
            	if ($sysconf['bibliography_update_notification']) {
                    utility::jsAlert(getttext('Data Successfully Updated'));
		  //  require_once SENAYAN_BASE_DIR.'admin/modules/bibliography/biblio_notification_mail.php';		
                ...
                // write log
                utility::writeLogs($dbs, 'tbladmin', $_SESSION['DUSER_ID'], 'bibliography', $_SESSION['realname'].' update bibliographic data ('.$data['title'].') with biblio_id ('.$_POST['itemID'].')');
            ...
            } else { utility::jsAlert(getttext('Data FAILED to Updated. Please Contact System Administrator')."\n".$sql_op->error); }
            exit();
        } else {
            
            $title = trim(strip_tags($_POST['title']));
            $gmid=$_POST['gmdID'];           
            $sql = "select * from biblio where title='$title' and gmd_id=$gmid";        
            $data_available = $dbs->query($sql);
            if ($data_available->num_rows>1) 
            {
                utility::jsAlert(getttext('Same Title and Material Type Data is Available'));
                exit();
            } 
            /* INSERT RECORD MODE */
            ...
            if ($insert) {
               ...
               // add standard
                ...
                utility::jsAlert(getttext('Data Successfully Saved'));
		//require_once SENAYAN_BASE_DIR.'admin/modules/bibliography/biblio_notification_mail.php';	
                // write log
                utility::writeLogs($dbs, 'tbladmin', $_SESSION['DUSER_ID'], 'bibliography', $_SESSION['realname'].' insert bibliographic data ('.$data['title'].') with biblio_id ('.$last_biblio_id.')');
            ...
            } else { utility::jsAlert(getttext('Bibliography Data FAILED to Save. Please Contact System Administrator')."\n".$sql_op->error); }
            exit();
        }
    }
    exit();
...
else if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction']))
{
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        if ($biblio_item_d[1] < 1) {
            if (!$sql_op->delete('biblio', "biblio_id=$itemID")) {
                ...
                // write log
                utility::writeLogs($dbs, 'tbladmin', $_SESSION['DUSER_ID'], 'bibliography', $_SESSION['realname'].' DELETE bibliographic data ('.$biblio_item_d[0].') with biblio_id ('.$itemID.')');
                // delete related data
    ...
    if ($still_have_item) {
        ...
        utility::jsAlert(getttext('Below data can not be deleted:')."\n".$titles);
    ...
    // error alerting
    if ($error_num == 0) {
        utility::jsAlert(getttext('All Data Successfully Deleted'));
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    } else {
        utility::jsAlert(getttext('Some or All Data NOT deleted successfully!\nPlease contact system administrator'));
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    }
    exit();
}
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/virtual_resources.php?virtual class="headerText2">Virtual Resources</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
<tr>
	<td class="tab_menu_top">
                            <ul class="tabs"> 
				<li>
<!--<a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/index.php" class="headerText2"><?php echo getttext('Library Resource List'); ?></a> </li><li>-->
<a href="<?php echo  MODULES_WEB_ROOT_DIR; ?>bibliography/index.php?action1=detail" class="headerText2"><?php echo getttext('Physical Resources'); ?></a> </li><li>  
<a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/virtual_resources.php?virtual" class="headerText2"><?php echo getttext('Virtual Resources'); ?></a></li></ul>
</td></tr></table>
<fieldset class="menuBox">
<div class="menuBoxInner biblioIcon">
    <?php //echo strtoupper(getttext('Virtual Resources List')); ?>  <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/virtual_resources.php?action=detail" class="headerText3"><?php echo getttext('Add New Virtual Resources'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/virtual_resources.php?virtual" class="headerText3"><?php echo getttext('Virtual Resources List'); ?></a>
    <?php
    // enable UCS?
    if ($sysconf['ucs']['enable']) {
    ?>
    <div class="marginTop"><a href="#" onclick="ucsUpload('<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/ucs_upload.php', serializeChbox('dataList'))" class="notAJAX ucsUpload"><?php echo getttext('Upload Selected Bibliographic data to Union Catalog Server*'); ?></a></div>
    <?php
    }
    ?>
    <p class="only_border">&nbsp;</p>

    <form name="search" action="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/virtual_resources.php" id="search" method="get" style="display: inline;"><?php echo getttext('Search'); ?> :
   ...
   <!-- added by iresh on 25-1-2011 --> <input type="text" name="keywords" id="keywords" width=140px/>
    <select name="field"><option value="0"><?php echo getttext('All Fields'); ?></option><option value="title"><?php echo getttext('Title/Series Title'); ?> </option><option value="subject"><?php echo getttext('Topics'); ?></option><option value="author"><?php echo getttext('Authors'); ?></select>
    <input type="submit" id="doSearch" value="<?php echo getttext('Search'); ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail'))
 {
   ...
   $form = new simbio_form_table_AJAX('mainForms', $_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'], 'post');
    $form->submit_button_attr = 'name="saveData" value="'.getttext('Save').'" class="button"';
   ...
   $form->table_header_attr = 'class="alterCell" style="font-weight: bold;"';
    ...
    if ($rec_q->num_rows > 0) {
        ...
        $form->submit_button_attr = 'name="saveData" value="'.getttext('Update').'" class="button"';
    ...
    /* Form Element(s) */
    // biblio title
    $form->addTextField('textarea', 'title', getttext('Title').'*', $rec_d['title'], 'rows="1" style="width: 100%; overflow: auto;"');
 // biblio gmd
 ...
 $material_q = $dbs->query('SELECT material_resource_id, material_resource_name FROM mst_material_resource_type where active_inactive="1"  AND material_resource_name="Virtual Library"');
	//$gmd_options='';
       //$material_options = array('N/A');
        $material_options = array('--Virtual Material Type--');
...
$ajax = "ajaxFillSelect('".SENAYAN_WEB_ROOT_DIR."admin/AJAX_material_sub_type_handler.php', 'mst_gmd', 'gmd_id:gmd_name:material_resource_id', 'gmdID', $('materialresourceid').getValue())";
	...
	$mst_options[] = array('0', getttext('--Material Type--'));
 ...
 $ajax_exp = "ajaxFillSelect('".SENAYAN_WEB_ROOT_DIR."admin/AJAX_material_sub_type_handler.php', 'mst_material_sub_type', 'material_sub_id:material_sub_name:gmd_id', 'materialsubid', $('gmdID').getValue())";
	...
	$mst_material_sub_type_options[] = array('0', getttext('--Material Sub Type--'));
        // string element
...
$str_input .= '<tr><td id="materialnewid1" class="alterCell2" colspan="3"></td></tr>';
 ...
 $form->addAnything(getttext('Material Sub Type*'), $str_input);
...
{ 
 $form->addTextField('text', 'publication', getttext('Publication'), $rec_d['publication'] , 'rows="1" style="width: 100%;"');
}
//biblio sub title
...
{
$form->addTextField('text', 'subTitle', getttext('Sub Title'), $rec_d['sub_title'], 'rows="1" style="width: 100%;"');
}
...
{
   // biblio series title
    $form->addTextField('textarea', 'seriesTitle', getttext('Series Title'), $rec_d['series_title'], 'rows="1" style="width: 100%;"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==88)
{
  // biblio edition
    $form->addTextField('text', 'edition', getttext('Edition'), $rec_d['edition'], 'style="width: 40%;"');
}
...
{
       // biblio keywords
    $form->addTextField('text', 'tags', getttext('Keywords'), $rec_d['tags'], 'style="width: 40%;"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==88)
{
    // biblio specific detail info/area
    $form->addTextField('textarea', 'specDetailInfo', getttext('Specific Detail Info'), $rec_d['spec_detail_info'], 'rows="2" style="width: 100%"');
}
...
{
    // biblio authors
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'bibliography/pop_author.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.getttext('Authors/Roles').'\')">'.getttext('Add Author(s)').'</a></div>';
    ...
    $form->addAnything(getttext('Author(s)'), $str_input);
}
/*
// biblio editors
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'bibliography/pop_editor.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.getttext('Editor').'\')">'.getttext('Add Editor(s)').'</a></div>';
    ...
    $form->addAnything(getttext('Editor(s)'), $str_input);
*/
...
// biblio publish frequencies
        ...
        $freq_options[] = array('0', strtoupper(getttext('SELECT FREQUENCY')));
        ...
        $str_input .= '&nbsp;';
        $str_input .= ' '.getttext('Use this for Serial publication');
   	  $form->addAnything(getttext('Frequency'), $str_input);
}
...
{
    // biblio ISBN/ISSN
    $form->addTextField('text', 'isbn_issn', getttext('ISBN/ISSN'), $rec_d['isbn_issn'], 'style="width: 40%;" onchange="return checkspecialcharacterdynamic(this.name);"');
}
//biblio company
if($rec_d['material_sub_id']==115)
{
    $form->addTextField('text', 'company', getttext('Company'), $rec_d['company'] , 'style="width: 40%;"');
}
//biblio Key Actors
if($rec_d['material_sub_id']==115)
{
    $form->addTextField('text', 'actors', getttext('Key Actors'), $rec_d['actors'] , 'style="width: 40%;"');
}
//biblio country
if($rec_d['material_sub_id']==115 || $rec_d['material_sub_id']==118)
{
    $form->addTextField('text', 'country', getttext('Country'), $rec_d['country'] , 'style="width: 40%;"');
}
//biblio state
if($rec_d['material_sub_id']==118)
{
    $form->addTextField('text', 'state', getttext('State'), $rec_d['state'] , 'style="width: 40%;"');
}
//biblio city
if($rec_d['material_sub_id']==118)
{
    $form->addTextField('text', 'city', getttext('City'), $rec_d['city'] , 'style="width: 40%;"');
}
//biblio Age Groups
if($rec_d['material_sub_id']==115)
{
    $form->addTextField('text', 'age_group', getttext('Age Groups'), $rec_d['age_group'] , 'style="width: 40%;"');
}
//biblio Awards
if($rec_d['material_sub_id']==115)
{
    $form->addTextField('text', 'awards', getttext('Awards'), $rec_d['awards'] , 'style="width: 40%;"');
}
//biblio editorial
if($rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==88)
{
 $form->addTextField('text', 'editorial', getttext('Editorial'), $rec_d['editorial'], 'style="width: 40%;"');
}
...
{    
  // biblio classification
   $form->addTextField('text', 'class', getttext('Classification No.'), $rec_d['classification'], 'style="width: 40%;"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==118 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==87)
{
    // biblio publisher
        ...
        $publ_options[] = array('0', getttext('Publisher'));
        // string element
    ...
    $form->addAnything(getttext('Publisher'), $str_input);
}
...
{
    // biblio publish year
    $form->addTextField('text', 'year', getttext('Publishing Year'), $rec_d['publish_year'], 'style="width: 40%;"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113  || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==87)
{
    // biblio publish place
        ...
        $plc_options[] = array('0', getttext('Publishing Place'));
        $str_input = simbio_form_element::selectList('placeID', $plc_options, '', 'style="width: 50%;"');
        $str_input .= '&nbsp;';
        $str_input .= simbio_form_element::textField('text', 'plc_search_str', $rec_d['place_name'], 'style="width: 45%;" onkeyup="'.$ajax_exp.'"');
    $form->addAnything(getttext('Publishing Place'), $str_input);
}
// biblio qualification/degree
if($rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==88)
{
//$form->addTextField('text', 'qualification', getttext('Qualification/Degree'), $rec_d['qualification'], 'style="width: 40%;"');
}
// biblio college/inst/dept
if($rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==88)
{
//$form->addTextField('text', 'college_inst_dept', getttext('College/Inst/Dept'), $rec_d['college_inst_dept'], 'style="width: 40%;"');
}
// biblio university
if($rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==88)
{
   // $form->addTextField('text', 'university', getttext('University'), $rec_d['university'], 'style="width: 40%;"');
}
...
{
    //$form->addTextField('text', 'vol_no', getttext('Volume No.'), $rec_d['vol_no'], 'style="width: 40%;"');
}
//biblio index number
if($rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==87)
{
  //  $form->addTextField('text', 'index_no', getttext('Index No.'), $rec_d['index_no'], 'style="width: 40%;"');
}
//biblio duration
if($rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==115)
{
  //  $form->addTextField('text', 'duration', getttext('Duration'), $rec_d['duration'], 'style="width: 40%;"');
}
//biblio issue number
if($rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==88)
{
//    $form->addTextField('text', 'issue_no', getttext('Issue No.'), $rec_d['issue_no'], 'style="width: 40%;"');
}
//biblio serial no
if($rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==88)
{
$form->addTextField('text', 'serial_no', getttext('Serial No.'), $rec_d['serial_no'], 'style="width: 40%;"');
}
...
{
    //$form->addDateField('pubdate', getttext('Publication Date'), $rec_d['publication_date']?$rec_d['publication_date']:date('Y-m-d'));
    $form->addDateField('pubdate', getttext('Publication Date'), $rec_d['publication_date']?$rec_d['publication_date']:date('d-m-Y'));
}
...
{    
// biblio collation
    $form->addTextField('text', 'collation', getttext('Book Size/ Number of page'), $rec_d['collation'] , 'style="width: 40%;" onchange="return numericcheck(this.name);"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==88)
{   
    // biblio call_number
   // $form->addTextField('text', 'callNumber', getttext('Call Number'), $rec_d['call_number'] , 'style="width: 40%;"');
}

if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==118 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==84)
{ 
    // biblio topics
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX"  href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'bibliography/pop_topic.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.getttext('Subjects/Topics').'\')">'.getttext('Add Subject(s)').'</a></div>';
    ...
    $form->addAnything(getttext('Subject(s)'), $str_input);
}
...
{
 // biblio standard
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX"  href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'bibliography/pop_standard.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.getttext('Standard').'\')">'.getttext('Add Standard(s)').'</a></div>';
    ...
    $form->addAnything(getttext('Standard(s)'), $str_input);
}

 //$form->addTextField('text', 'subject', getttext('Subject'), $rec_d['subject'], 'style="width: 40%;"');
//  $form->addTextField('text', 'standard', getttext('Standard'), $rec_d['standard'], 'style="width: 40%;"');
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==118 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==88)
{
    // biblio language
      ...
      $form->addSelectList('languageID', getttext('Language'), $lang_options, $lang_d1[0]);	
     //$form->addSelectList('languageID', getttext('Language'), $lang_options, $rec_d['language_id']);	
    //$form->addSelectList('languageID[]', getttext('Language'), $lang_options, '' ,'multiple="multiple" size=5');
     //$str_input = simbio_form_element::textField('text', 'price', '0', 'style="width: 40%;"');
   /*  $str_input = simbio_form_element::textField('text', 'price', $rec_d['price'], 'style="width: 40%;"');
     
    $str_input .= simbio_form_element::selectList('priceCurrency', $sysconf['currencies'], $rec_d['price_currency'] );
     //$str_input .= simbio_form_element::selectList('priceCurrency', $rec_d['price_currency'], '' );
    
    $form->addAnything(getttext('Price'), $str_input);*/
}
...
{
    // biblio note
    $form->addTextField('textarea', 'notes', getttext('Notes'), $rec_d['notes'] , 'style="width: 100%;" rows="2"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==118 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==87 || $rec_d['material_sub_id']==115)
{
    // biblio abstract
    $form->addTextField('textarea', 'abstract', getttext('Abstract'), $rec_d['abstract'] , 'style="width: 100%;" rows="2"');
}
...
{
 // biblio review
    $form->addTextField('text', 'review', getttext('Book Review'), $rec_d['review'] , 'style="width: 40%;" rows="2"');
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==88)
{
    // biblio cover image
    if (!trim($rec_d['image'])) {
        ...
        $form->addAnything(getttext('Image'), $str_input);
    } else {
...
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==115 || $rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==86 || $rec_d['material_sub_id']==87 || $rec_d['material_sub_id']==89 || $rec_d['material_sub_id']==90 || $rec_d['material_sub_id']==91 || $rec_d['material_sub_id']==92 || $rec_d['material_sub_id']==93 || $rec_d['material_sub_id']==94 || $rec_d['material_sub_id']==95 || $rec_d['material_sub_id']==96 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==97 || $rec_d['material_sub_id']==98 || $rec_d['material_sub_id']==99 || $rec_d['material_sub_id']==100 || $rec_d['material_sub_id']==101 || $rec_d['material_sub_id']==102 || $rec_d['material_sub_id']==104 || $rec_d['material_sub_id']==105 || $rec_d['material_sub_id']==106 || $rec_d['material_sub_id']==68 || $rec_d['material_sub_id']==69 || $rec_d['material_sub_id']==70 || $rec_d['material_sub_id']==71 || $rec_d['material_sub_id']==72 || $rec_d['material_sub_id']==73 || $rec_d['material_sub_id']==74 || $rec_d['material_sub_id']==75 || $rec_d['material_sub_id']==76)
{
    ...
    $str_input = '<div class="'.$visibility.'"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'bibliography/pop_attach.php?biblioID='.$rec_d['biblio_id'].'\', 900, 380, \''.getttext('File Attachments').'\')">'.getttext('Add Attachment').'</a></div>';
    ...
    $form->addAnything(getttext('File Attachment'), $str_input);
}
if($rec_d['material_sub_id']==78 || $rec_d['material_sub_id']==112 || $rec_d['material_sub_id']==113 || $rec_d['material_sub_id']==114 || $rec_d['material_sub_id']==115 || $rec_d['material_sub_id']==116 || $rec_d['material_sub_id']==117 || $rec_d['material_sub_id']==118 || $rec_d['material_sub_id']==77 || $rec_d['material_sub_id']==79 || $rec_d['material_sub_id']==80 || $rec_d['material_sub_id']==81 || $rec_d['material_sub_id']==82 || $rec_d['material_sub_id']==83 || $rec_d['material_sub_id']==84 || $rec_d['material_sub_id']==85 || $rec_d['material_sub_id']==86 || $rec_d['material_sub_id']==87 || $rec_d['material_sub_id']==89 || $rec_d['material_sub_id']==90 || $rec_d['material_sub_id']==91 || $rec_d['material_sub_id']==92 || $rec_d['material_sub_id']==93 || $rec_d['material_sub_id']==94 || $rec_d['material_sub_id']==95 || $rec_d['material_sub_id']==96 || $rec_d['material_sub_id']==88 || $rec_d['material_sub_id']==97 || $rec_d['material_sub_id']==98 || $rec_d['material_sub_id']==99 || $rec_d['material_sub_id']==100 || $rec_d['material_sub_id']==101 || $rec_d['material_sub_id']==102 || $rec_d['material_sub_id']==104 || $rec_d['material_sub_id']==105 || $rec_d['material_sub_id']==106 || $rec_d['material_sub_id']==68 || $rec_d['material_sub_id']==69 || $rec_d['material_sub_id']==70 || $rec_d['material_sub_id']==71 || $rec_d['material_sub_id']==72 || $rec_d['material_sub_id']==73 || $rec_d['material_sub_id']==74 || $rec_d['material_sub_id']==75 || $rec_d['material_sub_id']==76)
{
// biblio labels
    ...
    $hide_options[] = array('0', getttext('Show'));
    $hide_options[] = array('1', getttext('Hide'));
    $form->addRadio('opacHide', getttext('Hide For Member Access'), $hide_options, $rec_d['opac_hide']?'1':'0');
    // biblio promote to front page
    $promote_options[] = array('0', getttext('Don\'t Promote'));
    $promote_options[] = array('1', getttext('Promote'));
    $form->addRadio('promote', getttext('Promote To Homepage'), $promote_options, $rec_d['promoted']?'1':'0');
  /*  // biblio labels
        $arr_labels = !empty($rec_d['labels'])?unserialize($rec_d['labels']):array();
        if ($arr_labels) {
            foreach ($arr_labels as $label) { $arr_labels[$label[0]] = $label[1]; }
    ...
    if ($form->edit_mode) {
        echo '<div class="infoBox" style="overflow: auto;">'
            .'<div style="float: left; width: 80%;">'.getttext('You are going to edit biblio data').' : <b>'.$rec_d['title'].'</b>  <br />'.getttext('Last Updated').$rec_d['last_update'].'</div>'; //mfc
            if ($rec_d['image']) {
...
else 
{
    ...
    function showTitleAuthors($obj_db, $array_data)
    {
       ...
       /* $_sql_biblio_q = sprintf('SELECT b.title, a.author_name, opac_hide, promoted FROM biblio AS b
        ...
        $_sql_biblio_q = sprintf('SELECT b.title, b.publish_year, b.publisher_id, a.author_name, opac_hide, promoted FROM biblio AS b LEFT JOIN biblio_author AS ba ON b.biblio_id=ba.biblio_id
            LEFT JOIN mst_author AS a ON ba.author_id=a.author_id
            WHERE b.material_resource_id=5 and b.biblio_id=%d', $array_data[0]);         
        $_biblio_q = $obj_db->query($_sql_biblio_q);
        
        $_authors = '';
                 if ($_biblio_q->num_rows>0)
                 {
                        ...
                        $_sql_biblio_q1 = 'SELECT COUNT(item_id) from item WHERE biblio_id='.$array_data[0];
                        ...
                        $_output = '<div style="float: left;"><b>'.$_title.'</b><br/><br />Author :- <i>'.$_authors.'</i><br/>Publisher :- '.$_publication.'<br/>Publish Year :- '.$_publish_year.'</div>';
    ...
    if ($can_read AND $can_write) 
    {
        ...
        $datagrid->setSQLColumn('biblio.biblio_id', 'biblio.biblio_id AS bid',
            'biblio.title AS \''.getttext('Title').'\'',
            //'biblio.isbn_issn AS \''.getttext('ISBN/ISSN').'\'',
            'biblio.image AS \''.getttext('').'\''); 
    ...
    else
    {
           ...
           $datagrid->setSQLColumn('biblio.biblio_id AS bid', 'biblio.title AS \''.getttext('Title').'\'',
           // 'biblio.isbn_issn AS \''.getttext('ISBN/ISSN').'\'',
            'biblio.image AS \''.getttext('').'\'');
        // modify column value
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords'])
    {
        ...
        //$search_str=$search_str."and material_resource_id=5";
    ...
    if (isset($criteria))
    {
        $datagrid->setSQLcriteria('('.$criteria['sql_criteria'].') and biblio.material_resource_id=5 ');
    }
    
    // table spec
    $table_spec = 'biblio LEFT JOIN item ON biblio.biblio_id=item.biblio_id and biblio.material_resource_id=5 LEFT JOIN mst_gmd AS g ON g.gmd_id=biblio.gmd_id';

    // set group by
    //and biblio.material_resource_id=5 
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords']) 
    {
        $msg = str_replace('{result->num_rows}', $datagrid->num_rows, getttext('Found <strong>{result->num_rows}</strong> from your keywords')); //mfc
        echo '<div class="infoBox">'.$msg.' : "'.$_GET['keywords'].'"<div>'.getttext('Query took').' <b>'.$datagrid->query_time.'</b> '.getttext('second(s) to complete').'</div></div>'; //mfc
    }
