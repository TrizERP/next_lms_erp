...
if (isset($_GET['removesess'])) {
    ...
    echo 'alert(\''.gettext('Editor succesfully removed!').'\');';
    echo 'location.href = \'iframe_editor.php\';';
    echo '</script>';
}
...
if (isset($_POST['remove'])) {
    ...
    $sql_op->delete('biblio_editor', 'editor_id='.$id.' AND biblio_id='.$bid);
    echo '<script type="text/javascript">';
    echo 'alert(\''.gettext('Editor removed!').'\');';
    echo 'location.href = \'iframe_editor.php?biblioID='.$bid.'\';';
    echo '</script>';
}
...
if ($biblioID) {
    ...
    // database list
    $biblio_editor_q = $dbs->query("SELECT ba.*, a.editor_name FROM biblio_editor AS ba
        LEFT JOIN mst_editor AS a ON ba.editor_id=a.editor_id
        WHERE ba.biblio_id=$biblioID ORDER BY level ASC");

    $row = 1;
    while ($biblio_editor_d = $biblio_editor_q->fetch_assoc()) {
        // alternate the row color
        $row_class = ($row%2 == 0)?'alterCell':'alterCell2';

        // remove link
        $remove_link = '<a href="#" onclick="confirmProcess('.$biblioID.', '.$biblio_editor_d['editor_id'].')"
            style="color: #FF0000; text-decoration: underline;">Delete</a>';
        $editor = $biblio_editor_d['editor_name'];

        $table->appendTableRow(array($remove_link, $editor, $sysconf['authority_level'][$biblio_editor_d['level']]));
    ...
    if ($_SESSION['biblioAuthor']) {
        ...
        foreach ($_SESSION['biblioeditor'] as $biblio_session) {
            // remove link
            $remove_link = '<a href="iframe_author.php?removesess='.$biblio_session[0].'"
                style="color: #000000; text-decoration: underline;">Remove</a>';

            if ($biblio_session) {
                $editor_q = $dbs->query("SELECT editor_name FROM mst_editor
                    WHERE editor_id=".$biblio_session[0]);
                $editor_d = $editor_q->fetch_row();
                $editor = $editor_d[0];
            }

            $table->appendTableRow(array($remove_link, $editor, $sysconf['authority_level'][$biblio_session[1]]));
