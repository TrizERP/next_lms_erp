...
if (isset($_GET['action']) AND $_GET['action'] == 'print') 
{
  ...
  //  echo 'SELECT IF(i.call_number!=\'\', i.call_number, b.call_number) FROM biblio AS b LEFT JOIN item AS i ON b.biblio_id=i.biblio_id WHERE '.$criteria;die;
    // send query to database
    $biblio_q = $dbs->query('SELECT b.title,IF(i.call_number<>\'\',i.call_number, b.call_number) FROM biblio AS b LEFT JOIN item AS i ON b.biblio_id=i.biblio_id WHERE '.$criteria);
    ...
    $html_str .= '.labelStyle { width: 6cm; height: '.$box_height.'cm; text-align: center; margin: '.$items_margin.'cm; padding: 0; border: '.$border_size.'px solid #000000; }'."\n";
    ...
    $html_str .= '<table style="margin: 0; padding: 0;" cellspacing="0" cellpadding="10" width="1200px">'."\n";
    ...
    foreach ($chunked_label_arrays as $label_data) 
    {
        ...
        {
            $html_str .= '<td valign="top" width="6cm;">';
            $html_str .= '<div class="labelStyle" valign="top">';
            if ($include_header_text)
            {
                 $html_str .= '<div class="labelHeaderStyle">Title</div>'; 
              //
              //
                //$html_str .= '<div class="labelHeaderStyle">
                  //  SELECT IF(i.call_number!=\'\', i.call_number, b.call_number) FROM biblio AS b LEFT JOIN item AS i ON b.biblio_id=i.biblio_id WHERE '.$criteria.'</div>'; 
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/dl_print.php class="headerText2">Label Print</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
