...
{    
    ...
    $biblio_author_q = $dbs->query("SELECT ba.*, a.author_name FROM biblio_author AS ba
        INNER JOIN mst_author AS a ON ba.author_id=a.author_id
...
{    
    if ($_SESSION['biblioAuthor']) {
        ...
        {        
            ...
            if ($biblio_session) 
            {
                echo "SELECT author_name FROM mst_author WHERE author_id=".$biblio_session[0];die;
                $author_q = $dbs->query("SELECT author_name FROM mst_author WHERE author_id=".$biblio_session[0]);
