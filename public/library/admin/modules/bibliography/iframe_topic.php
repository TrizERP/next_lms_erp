...
if ($biblioID) {
    ...
    // database list
	/*echo "SELECT bt.*,st.subject_type_name,t.topic FROM biblio_topic AS bt
        LEFT JOIN mst_topic AS t ON bt.topic_id=t.topic_id
	LEFT JOIN mst_subject_type AS st ON st.subject_type_id=bt.subject_type_id
        WHERE bt.biblio_id=$biblioID";die;*/

    $biblio_topic_q = $dbs->query("SELECT bt.*,st.subject_type_name,t.topic FROM biblio_topic AS bt
        LEFT JOIN mst_topic AS t ON bt.topic_id=t.topic_id
	LEFT JOIN mst_subject_type AS st ON st.subject_type_id=bt.subject_type_id
    ...
    while ($biblio_topic_d = $biblio_topic_q->fetch_assoc()) 
    {
        ...
        $topictype = $biblio_topic_d['subject_type_name'];
	$topic = $biblio_topic_d['topic'];
        $table->appendTableRow(array($remove_link, $topictype,$topic));
        //$table->appendTableRow(array($remove_link, $topictype));
        $table->setCellAttr($row, 0, 'valign="top" class="'.$row_class.'" style="font-weight: bold; width: 10%;"');
        $table->setCellAttr($row, 1, 'valign="top" class="'.$row_class.'" style="font-weight: bold; width: 30%;"');
        $table->setCellAttr($row, 2, 'valign="top" class="'.$row_class.'" style="width: 60%;"');
   ...
   /*if ($_SESSION['biblioTopic']) 
    {
		...
		//echo "SELECT st.subject_type_name,t.topic FROM mst_subject_type AS st
					//LEFT JOIN mst_topic AS t ON t.topic_id=st.topic_id					
					// WHERE st.subject_type_id=".$biblio_session[1]."";
                $topic_q = $dbs->query("SELECT st.subject_type_name,t.topic FROM mst_subject_type AS st
					LEFT JOIN mst_topic AS t ON t.topic_id=st.topic_id					
					 WHERE st.subject_type_id=".$biblio_session[0]);
