...
{    
        window.open ('<?php MODULES_WEB_ROOT_DIR?>customs/test.php?id='+id+"&gmd_main="+gmd_main+"&continue_loan="+continue_loan,"mywindow","status = 1, height = 300, width = 300, resizable = 0" );  
}
function c(id,gmd_main,renewed)
{ 
    window.open ('<?php MODULES_WEB_ROOT_DIR ?>customs/test.php?id='+id+"&gmd_main="+gmd_main+"&renew="+renewed,"mywindow","status = 1, height = 300, width = 300, resizable = 0" );  
}

function d(id,gmd_main,over_due)
{ 
    window.open ('<?php MODULES_WEB_ROOT_DIR?>customs/test.php?id='+id+"&gmd_main="+gmd_main+"&over_due="+over_due,"mywindow","status = 1, height = 300, width = 300, resizable = 0" );   
}
</script>
...
$ajax = "a('".SENAYAN_WEB_ROOT_DIR."admin/AJAX_material_sub_type_handler_report.php', 'mst_gmd', 'gmd_id:gmd_name:material_resource_id', 'gmdID', $('materialresourceid').getValue())";
 ...
 ?> 
    ...
    <input type="submit" name="applyFilter" value="<?php echo __('Generate Reports'); ?>" />
...
(																
        SELECT mrt.material_resource_id m_id,mrt.material_resource_name material , b.biblio_id biblio , if(i.item_code is null , 0 , i.item_code) item ,																
