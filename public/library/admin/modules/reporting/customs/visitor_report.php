...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'reporting/customs/visitor_report.php class="headerText2">Visitor Statistic</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
    ...
    <!-- filter end -->
    <iframe name="reportView" id="reportView" src="<?php echo $_SERVER['PHP_SELF'].'?reportView=true'; ?>" frameborder="0" style="width: 100%; min-height: 600px;"></iframe>
<?php
} else {
    ob_start();
    // months array
    ...
    $_q = $dbs->query("SELECT user_id,user_name FROM category_user LIMIT 100");
    ...
    foreach ($member_types as $id => $member_type) {
        ...
        foreach ($months as $month_num => $month) {
            ...
            $sql_str = "SELECT COUNT(visitor_id) FROM visitor_count AS vc
                INNER JOIN ('.$inte_schema.'.tblstudent AS m LEFT JOIN category_user AS mt ON m.user_group_id=mt.user_id) ON m.enrollment_no=vc.member_id
                WHERE m.user_group_id=$id AND vc.checkin_date LIKE '$selected_year-$month_num-%'";
