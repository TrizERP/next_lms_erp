...
echo $bradecum;
    ...
    <legend style="font-weight: bold"><?php echo strtoupper(gettext('Overdued List')); ?> - <?php echo gettext('Report Filter'); ?></legend>
...
<?php
    ...
    $reportgrid->setSQLColumn('m.enrollment_no AS \''.gettext('Member ID').'\'',
                               ...
                               'i.item_code AS \''.gettext('Accession Number').'\'',
                               'i.item_title AS \''.gettext('Book Title').'\'',
                               'cs.name AS \''.gettext('Standard').'\'',
                               'ss.name AS \''.gettext('Division').'\'');
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        while ($ovd_title_d = $ovd_title_q->fetch_assoc()) {
            ...
            $_buffer .= '<td width="20%">'.gettext('Overdue').': '.$ovd_title_d['Overdue Days'].' '.gettext('day(s)').'</td>';
