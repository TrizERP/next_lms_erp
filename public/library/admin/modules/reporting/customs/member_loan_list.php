...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'reporting/customs/member_loan_list.php class="headerText2">Loan List By Member</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
<?php
    ...
    $table_spec = ''.$inte_schema.'.tblstudent AS m LEFT JOIN loan AS l ON m.enrollment_no=l.member_id';
    ...
    if (isset($_GET['id_name']) AND $_GET['id_name']) {
        ...
        if (count($words) > 1) {
            ...
            $overdue_criteria .= " AND m.enrollment_no LIKE '%$keyword%' AND m.first_name LIKE '%$keyword%' OR m.last_name LIKE '%$keyword%'";
        }
    }
    // loan date
