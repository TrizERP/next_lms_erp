...
$report_d .= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_loan_gmd height=539px width=800px frameborder=0></iframe>';
$loan_report[__('Total Loan By Material Type')] = $report_d;

// total number of loan transaction by Material Sub type
...
$report_d = '<div class="chartLink"><a class="notAJAX" href="#" onclick="openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_loan_material_sub_type\', 700, 470, \''.__('Total Loan By Material Sub Type').'\')">'.__('Show in chart/plot').'</a></div>';
while ($data = $report_q->fetch_row()) {
    $report_d .= '<strong>'.$data[0].'</strong> : '.$data[1].', ';
}
$report_d .= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_loan_material_sub_type height=539px width=800px frameborder=0></iframe>';
$loan_report[__('Total Loan By Material Sub Type')] = $report_d;
...
$report_d .= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_loan_colltype height=539px width=800px frameborder=0></iframe>';
...
$loan_report1[__('Total Loan Transactions')] = $report_d;
$peak_transaction_data = $report_q->fetch_row();

// transaction average per day
//added started and commented by Parth 19/8/2011
...
//$loan_report[__('Transaction Average (Per Day)')] = @ceil($loan_report1[__('Total Loan Transactions')]/$total_loan_days);

$total_loan_days_query = $dbs->query('SELECT MONTH(NOW()),YEAR(NOW()),CURDATE(),DAYOFMONTH(NOW())');
$total_loan_days = $total_loan_days_query->fetch_assoc();
$day = $total_loan_days['DAYOFMONTH(NOW())'];
$month = $total_loan_days['MONTH(NOW())']-3;
$year = $total_loan_days['YEAR(NOW())'];
$date_made = $year."-".$month."-".$day;
$total_loan_days_query = $dbs->query('SELECT * from loan where loan_date BETWEEN '.$date_made.' AND CURDATE()');
$total_loan_days = $total_loan_days_query->num_rows;
$loan_report[__('Transaction Average (Per Month)')] = @ceil($total_loan_days/3);
//added ended and commented by Parth 19/8/2011
// peak transaction
$loan_report[__('Total Peak Transaction')] = $peak_transaction_data[0];

// total members having loans
$report_q = $dbs->query('SELECT DISTINCT member_id FROM loan where is_lent=1 AND is_return = 0');
$report_d = $report_q->num_rows;
$loan_report[__('Members Already Had Loans')] = $report_d;

// total members having loans
// get total member that already not expired
$total_members_query = $dbs->query('SELECT COUNT(enrollment_no) FROM '.$inte_schema.'.tblstudent
...
//added start and commented by Parth 18/8/2011
/*$report_q = $dbs->query('SELECT COUNT(loan_id) FROM loan WHERE
    is_lent=1 AND is_return=0 AND TO_DAYS(due_date)>TO_DAYS(\''.date('Y-m-d').'\')');*/
$report_q = $dbs->query('SELECT COUNT(loan_id) FROM loan WHERE
    is_lent=1 AND is_return=0 AND TO_DAYS(due_date)>\''.date('Y-m-d').'\'');
//added ended and commented by Parth 18/8/2011
$report_d = $report_q->fetch_row();
$loan_report[__('Total Overdued Loans')] = $report_d[0];
//added started and commented by Parth 18/8/2011
//total issue book titles
$report_q = $dbs->query('SELECT b.title,l.item_code,COUNT(l.loan_id) AS total_loans FROM `loan` AS l INNER JOIN item AS i ON  l.item_code=i.item_code INNER JOIN biblio AS b ON i.biblio_id=b.biblio_id where l.is_lent=1 AND l.is_return=0 GROUP BY b.biblio_id ORDER BY COUNT(l.loan_id) DESC LIMIT 10');
$report_d = $report_q->fetch_row();
$loan_report[__('Issued Books By Members')] = $report_d[0];
//added ended and commented by Parth 18/8/2011
// table header
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'reporting/loan_report.php class="headerText2">Loan Report</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
<div class="menuBoxInner statisticIcon">
    ...
    <input type="hidden" name="print" value="true" /><input type="submit" value="<?php echo __('Print Report'); ?>" class="button" />
    </form>
