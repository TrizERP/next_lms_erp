...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'stock_take/resync.php class="headerText2">Resyncronize</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
<?php
echo '<div class="infoBox">'.gettext('Re-synchronize will only update current stock take\'s item data. It won\'t update any new bibliographic or item data that were inserted in the middle of stock take proccess')."\n";
echo '<p class="only_border">&nbsp;</p>'."\n";
echo '<form action="'.$_SERVER['PHP_SELF'].'" method="post" target="resyncSubmit">'."\n";
echo '<input type="submit" name="resync" value="'.gettext('Resynchronize Now').'" class="button" />'."\n";
echo '</form>'."\n";
