...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'stock_take/st_report.php class="headerText2">Stock Take Report</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
    ...
    <fieldset class="menuBox">
        ...
        <?php echo strtoupper(gettext('CURRENT STOCK TAKE REPORT')); ?>
        ...
        <input type="hidden" name="print" value="true" /><input type="submit" value="<?php echo gettext('Print Report'); ?>" class="button" />
        </form>
...
<?php
    ...
    $report_row[gettext('Stock Take Name')] = $stk_data['stock_take_name'];
    $report_row[gettext('Total Item Stock Taked')] = $stk_data['total_item_stock_taked'];
    // stock take item lost
    $report_row[gettext('Total Item Lost')] = intval($stk_data['total_item_lost']);
    if ($report_row[gettext('Total Item Lost')] < 1) {
        $report_row[gettext('Total Item Lost')] = gettext('None');
    }
    // stock take item on loan
    if (empty($stk_data['total_item_loan'])) {
        $report_row[gettext('Total Item On Loan')] = gettext('None');
    } else {
        $report_row[gettext('Total Item On Loan')] = $stk_data['total_item_loan'];
    }
    ...
    $report_row[gettext('Total Checked/Scanned Items')] = $checked_count.' ('.$checked_procent.'%) '.$progress_bar;
    // stock take participants data
    $report_row[gettext('Stock Take Participants')] = '<ul>';
        ...
        while ($st_other_users_d = $st_other_users_q->fetch_row()) {
            if ($st_other_users_d[0] != $stk_data['stock_take_users']) {
                $report_row[gettext('Stock Take Participants')] .= '<li>'.$st_other_users_d[0].' ('.$st_other_users_d[1].' '.gettext('items already checked').')</li>'; //mfc
            }
        }
    ...
    $report_row[gettext('Start Date')] = nl2br($stk_data['start_date']);
    // stock take end date
    $report_row[gettext('End Date')] = nl2br($stk_data['end_date']);
    ...
    $class_count_str .= '<tr><td class="dataListHeader">'.gettext('Classification').'</td>
        <td class="dataListHeader">'.gettext('Lost Items').'</td>
        <td class="dataListHeader">'.gettext('Existing Items').'</td>
        <td class="dataListHeader">'.gettext('On Loan Items').'</td></tr>';
    while ($class_num < 10) {
        $row_class = ($class_num%2 == 0)?'alterCell':'alterCell2';
        $class_count_str .= '<tr><td class="'.$row_class.'"><strong>'.$class_num.'</strong>'.gettext(' classes').'</td>'; //mfc
    ...
    $coll_type_count_str .= '<tr><td class="dataListHeader">'.gettext('Collection Type').'</td>
        <td class="dataListHeader">'.gettext('Lost Items').'</td>
        <td class="dataListHeader">'.gettext('Existing Items').'</td>
        <td class="dataListHeader">'.gettext('On Loan Items').'</td></tr>';
