       ...
       $_field = $_query['f'];
 		...
 		if (strlen($_q)< 4)
		 {
 		  ...
 		  $_sql_criteria .= " biblio.title LIKE '$_q' ";//added by iresh on 17-3-2011
				...
				if($_REQUEST['gmd_id'])
				{
/*echo  $_subquery = 'SELECT g.gmd_id FROM mst_gmd AS g
				      LEFT JOIN mst_material_sub_type AS mst ON mst.gmd_id=g.gmd_id
				      LEFT JOIN biblio AS b ON b.material_sub_id=mst.material_sub_id
				      WHERE g.gmd_name=\''.$_q.'\' OR (mst.material_sub_name=\''.$_q.'\' AND mst.material_sub_id='.$_REQUEST['gmd_id'].')'; */
/*
 $_subquery = 'SELECT mst.material_sub_id FROM mst_material_sub_type AS mst
				      WHERE mst.material_sub_name=\''.$_q.'\'';
				...
				{
					$_subquery = 'SELECT g.gmd_id FROM mst_gmd AS g
				      LEFT JOIN mst_material_sub_type AS mst ON mst.gmd_id=g.gmd_id
				      LEFT JOIN biblio AS b ON b.material_sub_id=mst.material_sub_id
				      WHERE g.gmd_name=\''.$_q.'\'';
					if ($_b == '-') {
	...
	echo "<form method=post>";
 	echo '<table border=0 cellpadding=5 cellspacing=5 BORDERCOLOR=RED align=center>';
        ...
        while ($_biblio_d = $this->resultset->fetch_assoc()) {
	     ...
	     // $_biblio_d['title'] = '<a href="'.$sysconf['baseurl'].'index.php?p=show_detail&id='.$_biblio_d['biblio_id'].'&gmd_code=Virtual-Resources" title="'.__('Record Detail').'">'.$_biblio_d['title'].'</a>';
