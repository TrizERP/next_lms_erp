...
class biblio_list
{
    ...
    protected $searchable_fields = array('title', 'author', 'subject', 'isbn',
		...
		'location', 'itemcode', 'callnumber', 'itemcallnumber', 'notes','standard','subjecttype','material_sub_type','subjectajax','sub','barcode','subtype','searchsimple','letternew','searchkeyword');
       ...
       if (count($_queries) < 1) {
        ...
        foreach ($_queries as $_num => $_query) {
 		...
 		if (strlen($_q)< 4)
		 {
 		    ...
 		    $_sql_criteria .= " biblio.title LIKE '%$_q%' OR biblio.tags LIKE '%$_q%' ";//added by iresh on 17-3-2011
                    $_title_buffer = '';
                }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function getDocumentList($int_num2show = 10, $bool_return_output = true)
    {
	 ...
	 $_sql_str = 'SELECT SQL_CALC_FOUND_ROWS biblio.biblio_id,biblio.title,biblio.image,biblio.gmd_id, biblio.isbn_issn, biblio.labels';//added by iresh on 5-4-2011
	...
	echo "<form method=post>";
 	echo '<table border=1 cellpadding=5 cellspacing=5 BORDERCOLOR=RED align=center>';
...
$_image_cover = '<img src='.$images_loc.' width="80px" height="120px" style="" >' ;
	...
	*/
		 $_alt_list = ($_i%3 == 0)?'alterList':'alterList2';	
	 // echo '<tr>';
//added by Parth 13/7/2011	
	if(!empty($_image_cover))
	{		
	 echo  '<td  align="left" valign="top"> '.$_image_cover.''.$_biblio_d['title'].'<br />';
...
$img_set = $_biblio_d['isbn_issn'];
	...
	include('test1.php');
	echo ''.$_biblio_d['title'].'<br />';
	}
//ended by Parth 13/7/2011
...
//Commented And Added By Start Parth 14/7/2011
...
//added by Parth 11/7/2011 & 14/7/2011
if(!empty($_biblio_d['spec_detail_info']))
{
 echo '<div class="subItem authorField"><b>'.__('Summary').'</b> : '.$_biblio_d['spec_detail_info'].'</div>';
}
else
{
//$flag=0;
...
//if($jj=='set')
//{
echo '<b>Summary :- </b><iframe src=lib/test3.php?img_set='.$_biblio_d['isbn_issn'].' width=300px frameBorder=0 ></iframe><br/>';
//}
}
...
//ended by Parth 11/7/2011 & 14/7/2011
//added by Parth 11/7/2011	
echo '<div align="right">'.$set_title.'</div>'; 
//ended by Parth 11/7/2011   
