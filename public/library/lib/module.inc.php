...
class module extends simbio {
	...
	public $appended_first = '<li><a class="waves-effect" href="index.php"><i class="mdi mdi-home"></i> Home</a></li><!--<li><a class="menu" href="../index.php" title="View OPAC in New Window" target="_blank">Member Area</a></li>-->';
	...
	public function generateModuleMenu($obj_db) {
		...
		$_menu = '<div class="sidebarNew" role="navigation" style="background:white;width:200px;overflow: scroll;">
		...
		if ($this->module_list) {
			foreach ($this->module_list as $_module) {
				...
				$menu_icons = array(
					"Home" => 'mdi-home',
					"Catalog" => 'mdi-library',
					"Circulation" => 'mdi-book-variant',
					"Member Area" => 'mdi-clipboard-account',
					"Master File" => 'mdi-file-check',
					"Reporting" => 'mdi-signal',
					"Site Map" => 'mdi-map',
				);
				...
				if (isset($_SESSION['priv'][$_module['path']]['r']) && $_SESSION['priv'][$_module['path']]['r'] && file_exists($this->modules_dir . $_mod_dir)) {
					...
					if ($_formated_module_name != 'Home') {
						$_submenu = '';
						$_submenu .= '<ul class="sub-menu">';
						$_submenu_file = $this->modules_dir . $_module['path'] . DIRECTORY_SEPARATOR . 'submenu.php';
						...
						foreach ($menu as $_list) {
							...
							$submenu_icons = array(
								"Library Resources" => 'mdi-library-books',
								"Books" => 'mdi-book-open-variant',
								"Item" => 'mdi-file-multiple',
								"Z3950 Service" => 'mdi-cloud-circle',
								"Import MARC File" => 'mdi-file-import',
								"Print" => 'mdi-cloud-print',
								"Import" => 'mdi-import',
								"Export" => 'mdi-export-variant',
								"Book Verification" => 'mdi-book-lock',
								"Book Verification Remarks" => 'mdi-bookmark-check',
								"Book Circulation" => 'mdi-book-open-variant',
								"Quick Return" => 'mdi-keyboard-return',
								"Reservation" => 'mdi-book-open',
								"Membership" => 'mdi-account-check',
								"Tools" => 'mdi-arrow-collapse',
								"Category Master" => 'mdi-wrap',
								"Publisher" => 'mdi-message-draw',
								"Supplier" => 'mdi-run',
								"Author" => 'mdi-code-less-than-or-equal',
								"Subject" => 'mdi-checkbox-marked',
								"Standard/Class" => 'mdi-format-align-justify',
								"Book Rack/Shelf" => 'mdi-drawing-box',
								"Publishing Place" => 'mdi-memory',
								"Item Status" => 'mdi-check-all',
								"Book Collection Type" => 'mdi-select-inverse',
								"Labels" => 'mdi-apps',
								"Check URL Status" => 'mdi-format-indent-increase',
								"Catalog Statistics" => 'mdi-sort',
								"Loan Report" => 'mdi-format-color-fill',
								"Title List" => 'mdi-format-align-center',
								"Items Book List Report" => 'mdi-vector-arrange-below',
								"Items Title List" => 'mdi-blur',
								"Loans by Classification" => 'mdi-image-broken-variant',
								"Member List" => 'mdi-account-multiple-plus',
								"Loan History" => 'mdi-nature-people',
								"Due Date Warning" => 'mdi-calendar-clock',
								"All Book List" => 'mdi-file-check',
								"Overdues List" => 'mdi-calendar-remove',
								"Site Map" => 'mdi-map-marker-multiple',
								"Summary Report" => 'mdi-chart-tree',
								"Books Title List" => 'mdi-book-open',
								"Issue History" => 'mdi-history',
								"Overdued List" => 'mdi-card-bulleted-off',
								"Books Lost/Damage" => 'mdi-book-remove-multiple',
								"Books Verified List" => 'mdi-bookshelf',
								"Book Pending Verify List" => 'mdi-book-search-outline',
							);

							// echo '<pre>';
							// print_r($_list);
							// if ($_list[0] == 'Header2') {
							// 	$_submenu .= '<li><p class="title">' . $_list[1] . '</p></div><div class="side_mid_bg">';
							// } else if ($_list[0] == 'Header') {
							// 	$_submenu .= '<div class="subMenuHeader">' . $_list[1] . '</div>';
							// } else {
							if ($_list[0] != 'Header2' && $_list[0] != 'Header') {
								$_submenu .= '<li><span class="hide-menu"><a class="subMenuItem waves-effect" href="' . $_list[1] . '" title="' . (isset($_list[2]) ? $_list[2] : $_list[0]) . '"><i class="mdi ' . $submenu_icons[$_list[0]] . ' fa-fw" data-icon="v"></i> ' . $_list[0] . '</a></span></li>';
							}
						}
					}
					$_submenu .= '</ul>';
					...
					// 	$url = '../admin/modules/' . $_module['path'] . '/submenu.php';
					// 	include $url;
