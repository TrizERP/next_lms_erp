...
*/	
	...
	*/
	 ...
	 // echo '<tr>';
	   ...
	   echo  '<td class="item '.$_alt_list.'" '.$_image_cover.' >'.$_biblio_d['title'].'<br />';
	 ...
	 if($tt>0)
{
	   ...
	   else
	   {
			...
			echo '<div align=center><strong width="50%" style="color: red;">'.__('All Copy Currently On Loan (One Of The Copy Due on').date($sysconf['date_format'], strtotime( $due1)).')</strong></div>'; 
		
	   }
}
else
{
	 echo '<div><strong style="color: red; font-weight: bold;">'.__('There is no item/copy for this title yet').'</strong></div>';

}

	$sql="select item_code from item where biblio_id=".$_biblio_d['biblio_id']." AND item_code NOT IN (select item_code from loan where biblio_id=".$_biblio_d  
              ['biblio_id']." AND is_return=0) limit 0,1";
	...
	while($s=$sql->fetch_assoc())
	{
	 ...
	 if($tt>0)
	{    
	     //echo "<div><input type='text' name='values[$s1]' id=name ></div>";
	     echo "<div><input type=checkbox name='item' value='$s1'></div>";
	...
	else
	{
	     ...
	     $due_item="select min(l.due_date) AS due_date from loan l
	      ...
	      echo   $item_loan="select l.item_code AS item_code from loan l
			    left join item i on i.item_code=l.item_code
			    left join biblio b on b.biblio_id=i.biblio_id
		...
		echo '<div><input type=checkbox name="item" value='.$item_l.'></div>'; 
