...
class detail extends content_list
{
    ...
    protected function htmlOutput()
    {
        ...
        $this->metadata .= '<meta name="Abstract" content="'.strip_tags($this->record_detail['abstract']).'" />';
	$a=array();
        ...
        foreach($a as $record)
        {
           ...
           if ($b=='htt')
           {
               $this->record_detail['lables']=$b;
               $this->metadata .= '<meta name="lables" content="'.$this->record_detail['lables'].'" />';
           }
           else {
              
               $this->metadata .= '<meta name="lables" content="Link Not Available" />';
           }
        }
//echo $this->record_detail['lables']= @unserialize($this->record_detail['lables']);
        // check image
        if (!empty($this->record_detail['image'])) {
            ...
            $this->record_detail['image'] = '<img src="images/docs/imagesnotavailable2.jpg" border="0" />';
        }
...
$_biblio_file_q = $this->obj_db->query('SELECT labels FROM biblio WHERE biblio_id='.$this->detail_id);
$lable='';
$i=0;
while($row=$_biblio_file_q->fetch_assoc())
{
	 $lable=$row['labels'];
	

}
$lable_final=@unserialize($lable);
$lable_final=$lable_final[0][1];
if($lable_final=='')
{
$this->record_detail['labels'] ='<a href="index.php?material_sub_type_select=SELECT&q=&Search=Keyword+Search&search1=Search" target="_blank">Back To Previous</a>';
}
else
{
$this->record_detail['labels'] ='<a href="index.php?material_sub_type_select=SELECT&q=&Search=Keyword+Search&search1=Search" target="_blank">Back To Previous</a>';
...
/*
print_r($lable_final);
foreach($lable_final as $l)
{
	foreach($l as $a)
	{	echo $a;
		echo "<br>";
//$this->record_detail['labels'] ='<a href='.$lable_final.' target="_blank">Connect</a>';
$this->record_detail['labels'] ='<a href='.$a[1].' target="_blank">Connect</a>';
	}
}*/
//$lable_final=$lable[0][1];
    ...
    protected function MODSoutput()
    {
        ...
        // personal name
        // get the authors data
        $_biblio_authors_q = $this->obj_db->query('SELECT a.author_name FROM mst_author AS a
             LEFT JOIN biblio_author AS ba ON a.author_id=ba.author_id 
             LEFT JOIN biblio as b on b.biblio_id=ba.biblio_id
              WHERE b.biblio_id='.$this->detail_id);
