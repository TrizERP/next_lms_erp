...
//            FROM user AS u
//            WHERE u.username='%s'
//                AND u.passwd='%s'", $obj_db->escape_string(trim($this->username)), $obj_db->escape_string(trim($this->password)));
//}
//else
//{
//        $_sql_librarian_login = sprintf("SELECT
...
//            $this->real_name = $_user_d['realname'];
            ...
            //$_SESSION['uname'] = $_user_d['username'];
             $_SESSION['uname']=$_SESSION['DUSER_ID'];
            //$_SESSION['realname'] = $_user_d['realname'];
            $_SESSION['realname']=$_SESSION["DUSER_NAME"];
            
            
            //$_SESSION['uname'] = $_user_d['username'];
           
            //$_SESSION['realname'] = $_user_d['realname'];
            $_SESSION['groups']=$_SESSION['USER_GROUP_ID'];
...
//            }
	    ...
	    $_SESSION['biblioStandard'] = array();
            if (!defined('UCS_VERSION')) {
                ...
                $_holiday_date_q = $obj_db->query('SELECT holiday_date FROM holiday WHERE holiday_date IS NOT NULL ORDER BY holiday_date DESC LIMIT 365');
