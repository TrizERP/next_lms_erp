      ...
      while ($row = $loan_due_date_diff->fetch_assoc())
        {
              if ($row['date_diff']<=3)
              {
                  $qry = "Select l.item_code,b.title,l.loan_date,l.due_date from loan AS l LEFT JOIN ".$inte_schema.".tblstudent AS m ON l.member_id=m.enrollment_no LEFT JOIN item AS i ON l.item_code=i.item_code LEFT JOIN biblio AS b ON i.biblio_id=b.biblio_id where m.enrollment_no='".$_SESSION['DUSER_ID']."' AND l.loan_date is not null AND return_date is null AND l.loan_id=".$row['loan_id']."  order by l.due_date desc";                    
