...
require LIB_DIR.'subject_search.inc.php';//added by iresh on 18-3-2011
...
if (isset($_GET['subject_search']) && !empty($_GET['subject_search']))//added by iresh on 17-3-2011 For alphabetical searching
{
    ...
    if (isset($_GET['subject_search'])) {
        $keywords = trim(strip_tags(urldecode($_GET['subject_search'])));
    }
    if ($keywords && !preg_match('@[a-z0-9_.]+=[^=]+\s+@i', $keywords.' ')) {
     $criteria = 'title='.$keywords.' OR author='.$keywords.' OR subject='.$keywords.' OR gmd='.$keywords;
...
if (isset($_GET['subject_search']) && !empty($_GET['subject_search'])) {
    // default vars
    $is_adv = false;
    $keywords = '';
    $criteria = '';
    ...
    // advanced search
    $is_adv = isset($_GET['subject_search']) || isset($_GET['title']) || isset($_GET['author']) || isset($_GET['isbn'])
