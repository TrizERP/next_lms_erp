...
class biblio_list
{
    ...
    protected $searchable_fields = array('title', 'author', 'subject', 'isbn',
		...
		'location', 'itemcode', 'callnumber', 'itemcallnumber', 'notes','standard','subjecttype','material_sub_type','subjectajax','sub','barcode','subtype','searchsimple','letternew','searchkeyword', 'class', 'keywords_tag');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                switch ($_field) {
                    ...
                    case 'gmd' :
                        ...
                        break;
	           case 'mst_sub' :
                        $_subquery = 'SELECT material_sub_id FROM mst_material_sub_type WHERE material_sub_name=\''.$_q.'\'';
                        if ($_b == '-') {
                            $_sql_criteria .= " biblio.material_sub_id NOT IN ($_subquery)";
                        } else {  $_sql_criteria .= " biblio.material_sub_id IN ($_subquery)"; }
                        break;
                   case 'keywords_tag' :
                         if ($_b == '-') {
...
//added start by Parth 3/8/2011 & 4/8/2011
if($_GET['p']=="rescentview")
{
$_sql_str = "select biblio.biblio_id,biblio.title,biblio.image,biblio.gmd_id, biblio.isbn_issn, biblio.labels, b2.count from biblio biblio INNER JOIN biblio_view b2 ON biblio.biblio_id = b2.biblio_id where b2.member_id = $_SESSION[mid] ORDER BY b2.last_update DESC";
...
{
$_sql_str = "select biblio.biblio_id,biblio.title,biblio.image,biblio.gmd_id, biblio.isbn_issn, biblio.labels from biblio biblio INNER JOIN biblio_myself b2 ON biblio.biblio_id = b2.biblio_id where b2.member_id = $_SESSION[mid] ORDER BY b2.last_update DESC";
 $this->resultset = $this->obj_db->query($_sql_str);
        if ($this->obj_db->error) {
            $this->query_error = $this->obj_db->error;
        }
...
{
        // init sql string
      //  $_sql_str = 'SELECT SQL_CALC_FOUND_ROWS biblio.biblio_id,biblio.title,biblio.image,biblio.gmd_id, biblio.isbn_issn, biblio.labels';
	 $_sql_str = 'SELECT SQL_CALC_FOUND_ROWS biblio.biblio_id,biblio.title,biblio.image,biblio.gmd_id, biblio.isbn_issn, biblio.labels,biblio.abstract,biblio.notes,biblio.material_sub_id';//added by iresh on 5-4-2011
        ...
        // promoted flag
       // if ($this->only_promoted) { $_add_sql_str .= ' AND promoted=1 AND gmd_id NOT IN(select gmd_id from mst_gmd where gmd_code="Virtual Resources")'; }
	//if ($this->only_promoted) { $_add_sql_str .= ' AND promoted=1 AND gmd_id NOT IN(select gmd_id from mst_gmd where gmd_code="Virtual Resources")'; }//added by iresh on 5-4-2011
...
//added end by Parth 3/8/2011 & 4/8/2011
    }
...
//added started by Parth 3/8/2011
...
//echo '<td valign=top><div class="subItem authorField"><b>'.__('By').'</b> : '.$_authors.'</div>';
echo '<div class="subItem authorField"><b>'.__('By').'</b> : '.$_authors.'</div>';
//Commented And Added By End Parth 14/7/2011		
}
...
{	
	  ...
	  {
	   echo '<div><strong style="color: red; font-weight: bold;">'.__('No Copy Available For This Book').'</strong></div>';
	  }
...
//if($jj=='set')
//{
//echo '<b>Summary :- </b><br/><iframe name=myFrame id=iframeId src=lib/test3.php?img_set='.$_biblio_d['isbn_issn'].' width=300px frameBorder=0 ></iframe><br/>';
//echo '<b>Summary :- </b><a href="" onclick=javascript:window.open("lib/test3.php?img_set='.$_biblio_d['isbn_issn'].'","Window1","menubar=no,width=600,height=50,toolbar=no,scrollbars=yes,location=no,left=200px");>Summary</a><br/>';
//}
}
}
if(empty($_biblio_d['count']) && $_GET['p']!="myeself")
...
{
echo '<iframe src=lib/like.php?subid='.$_biblio_d['biblio_id'].'&memberid='.$_SESSION['mid'].' width=200px height=100px frameBorder=0></iframe>';
}
}
