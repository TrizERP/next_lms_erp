...
if($_REQUEST['modfunc']=='create')
{
	...
	if(Prompt_Calender('Create a new calendar','',$message))
	{
		...
		DBQuery("INSERT INTO ATTENDANCE_CALENDARS (SYEAR,SCHOOL_ID,TITLE,DEFAULT_CALENDAR) values('".UserSyear()."','".UserSchool()."','".$_REQUEST['title']."','".$_REQUEST['default']."')");
...
{
	$has_assigned_RET=DBGet(DBQuery("SELECT COUNT(*) AS TOTAL_ASSIGNED FROM STUDENT_ENROLLMENT WHERE CALENDAR_ID='$_REQUEST[calendar_id]'"));
