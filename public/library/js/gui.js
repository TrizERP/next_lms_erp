...
var openHTMLpop = function (strURL, intWidth, intHeight, strPopTitle)
{
    ...
    //alert('Hi');
    var str = strURL.split("/", 6);//spliting xl file for iframe not display

    // retrieve required dimensions
    ftop = 0;
    jQuery('.notAJAX').each(function () {
        strhref = jQuery(this).attr('href');
        if (strhref.indexOf(strURL) >= 0) {
            ftop = jQuery(this).offset().top;
        }
    });
    ...
    //blocker.setStyle({top: 0, left: 0, width: '100%', height: screen.height+'px', position: 'absolute', backgroundColor: '#000', opacity: 0.3});
    blocker.setStyle({top: 0, left: 0, width: '100%', height: screen.height + 'px', opacity: 1});
    if (str == ",schoollib,admin,modules,bibliography,catalog_import")
    {
        document.getElementById("htmlPopFrame").style.display = "none";
        document.getElementById("htmlPopTitle").style.display = "none";
        document.getElementById("htmlPop").style.display = "none";
    ...
    htmlPop.setStyle({position: 'absolute', top: yPos + 'px', left: xleft + 'px', margin: 'auto', width: intWidth + 'px', opacity: 1});
    //htmlPop.setStyle({position: 'absolute', top: yPos+'px', left: 200+'px', margin: 'auto', width: intWidth+'px', opacity: 1});
