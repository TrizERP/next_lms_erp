...
//                $reset_button = '<input type="reset" class="button" value="Reset" onclick="formReset(){document.getElementByID(mainForm).Reset();}" />';
//            }
...
//                 $_reset_button = '<input type="button" value="'.$_reset_value.'" class="button" onclick="confSubmit(\'deleteForm\', \'Are you sure to delete '.addslashes($this->record_title).'?\nOnce Deleted it cant be restored again\')" style="color: red; font-weight: bold;" />';
            }
            // back button
            if ($this->back_button) {
        ...
        // check if form tag is included
	if ($this->with_form_tag) {
            $_buttons = '<table cellspacing="0"  cellpadding="3" style="width: 100%; background-color: #fff;">'
                .'<tr><td>'.$_edit_link.' <input type="submit" '.$this->submit_button_attr.' /> '.$_back_button.' '.$_delete_button.'</td><td align="right"></td>'
                .'</tr></table>'."\n";
        }
	if ($this->form_name == 'main')// added by iresh 0n 2-5-2011
	 {
		
            $_buttons = '<table cellspacing="0" cellpadding="3" style="">'
                .'<tr><td>'.$_edit_link.' <input type="hidden" '.$this->submit_button_attr.' /> '.$_back_button.' '.$_delete_button.'</td><td align="right"></td>'
                .'</tr></table>'."\n";
        }
	...
	// hidden form for deleting records
           /* $_buffer .= '<form action="'.preg_replace('/\?.+/i', '', $this->form_action).'" id="deleteForm" target="submitExec" method="post" style="display: inline;">'
            ...
            $_buffer .= '<form action="'.preg_replace('/\?.+/i', '', $this->form_action).'" id="deleteForm" target="submitExec" method="post" style="display: inline;">'
                .'<input type="hidden" name="itemID" value="'.$this->record_id.'" /><input type="hidden" name="itemAction" value="true" />';
            // disabling form
	...
	if ($this->form_name == 'main')// added by iresh 0n 2-5-2011
	 {
          ...
          /*  $_buttons = '<table cellspacing="0" cellpadding="3" style="">'
                .'<tr><td><input type="hidden" '.$this->submit_button_attr.' /> '.$_back_button.' '.$_delete_button.'</td><td align="right">'.$_edit_link.'</td>'
                .'</tr></table>'."\n";*/
        }
        // get the table result
        $_buffer .= $_buttons;
