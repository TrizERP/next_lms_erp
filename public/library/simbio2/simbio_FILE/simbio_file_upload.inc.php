...
class simbio_file_upload extends simbio
{
    ...
    private $allowable_ext = array('.jpg', '.jpeg', '.gif', '.png', '.html', '.htm', '.pdf', '.doc', '.txt','.xls');
    private $max_size = 1024; // in bytes
    private $upload_dir = './';
    public $new_filename;
    ...
    public function doUpload($file_input_name, $str_new_filename = '')
    {
        ...
        // get file extension
 //mfc
   //utility::jsAlert($file_input_name); 
        ...
        if (empty($str_new_filename)) 
        {
            ...
            $str_new_filename_name=explode('.',$_FILES[$file_input_name]['name']);
           
            $this->new_filename = $str_new_filename_name[0]."_".date('y-m-d H:i:s').$file_ext;
...
*/
        // check for file size
                //alert::jsalert('fd');
		$_size_kb = (1024000000)/1024;
                
        //$_size_kb = ((integer)$this->max_size)/1024;
	//$_size_kb = ((integer)$this->max_size);
        //if ($_FILES[$file_input_name]['size'] > $this->max_size) 
               
	if ($_FILES[$file_input_name]['size'] > 1024000000) 
	{
            $this->error = 'Filesize is excedded maximum uploaded file size';
            return FILESIZE_EXCED;
        }
      // utility::jsalert($this->upload_dir.'/'.$this->new_filename);die;
         // uploading file
//if (@move_uploaded_file($_FILES[$file_input_name]['tmp_name'], REPO_BASE_DIR.DIRECTORY_SEPARATOR.$this->new_filename)) { 
//     utility::jsalert($this->upload_dir.$this->new_filename);die;
       
        if (@move_uploaded_file($_FILES[$file_input_name]['tmp_name'],$this->upload_dir.$this->new_filename)) //$this->upload_dir.'/'.$this->new_filename)) 
        { 
        ...
        else 
        {
            ...
            if ($upload_error) {
                ...
                //utility::jsAlert($error_msg); 
            }
            $this->error = 'Upload failed. Upload directory is not writable or not exists. '.$error_msg;
            return UPLOAD_FAILED;
        }
    }
 public function doUpload_mrc($file_input_name, $str_new_filename = '')
    {
        // get file extension
 //mfc
        $file_ext = substr($_FILES[$file_input_name]['name'], strrpos($_FILES[$file_input_name]['name'], '.'));
        ...
        // check for file size
        $_size_kb = (10240000)/1024;
	//$_size_kb = ((integer)$this->max_size);
        if ($_FILES[$file_input_name]['size'] > 10240000) {
            $this->error = 'Filesize is excedded maximum uploaded file size';
            return FILESIZE_EXCED;
        }
...
public function doUpload_csv($file_input_name, $str_new_filename = '')
    {
        // get file extension
 //mfc
        $file_ext = substr($_FILES[$file_input_name]['name'], strrpos($_FILES[$file_input_name]['name'], '.'));
