...
$stat_data.= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_title_gmd height=539px width=800px frameborder=0></iframe>';

$collection_stat[__('Total Titles By Material Type')] = $stat_data;

// total Material Sub Type 
$stat_query = $dbs->query('SELECT material_sub_name, COUNT(biblio_id) AS total_titles
    FROM `biblio` AS b
...
$stat_data = '<div class="chartLink"><a class="notAJAX" href="#" onclick="openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_title_material_sub_type\', 700, 470, \''.__('Total Titles By Material Sub Type').'\')">'.__('Show in chart/plot').'</a></div>';
while ($data = $stat_query->fetch_row()) {
    $stat_data .= '<strong>'.$data[0].'</strong> : '.$data[1];
    $stat_data .= ', ';
}
$stat_data .= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_title_material_sub_type height=539px width=800px frameborder=0></iframe>';

$collection_stat[__('Total Titles By Material Sub Type')] = $stat_data;
...
$stat_data .= '<br/><iframe src='.MODULES_WEB_ROOT_DIR.'reporting/charts_report.php?chart=total_title_colltype height=539px width=800px frameborder=0></iframe>';
...
/*echo 'SELECT b.title,l.item_code,COUNT(l.loan_id) AS total_loans FROM `loan` AS l
    LEFT JOIN item AS i ON l.item_code=i.item_code
    LEFT JOIN biblio AS b ON i.biblio_id=b.biblio_id
    GROUP BY l.item_code ORDER BY COUNT(l.loan_id) DESC LIMIT 10';*/
...
$set_title = str_replace(' ','%20',$set_title);
$stat_data .= '</ul>';
$stat_data .= '<iframe name=new src='.SENAYAN_WEB_ROOT_DIR.'admin/modules/reporting/Highcharts/examples/chart_test.php?set_title='.$set_title.'&set_value='.$set_value.' height=539px width=800px frameborder=0></iframe>';
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'reporting/index.php class="headerText2">Collection Statistic</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
//include(SENAYAN_BASE_DIR.'admin/modules/reporting/Highcharts/examples/chart_test.php');
