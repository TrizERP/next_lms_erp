 ...
 $extend_link = '<input type=checkbox name=loanID[] value='.$loan_list_data['loan_id'].'>';
    ...
    if (isset($_GET['reserveAlert']) AND !empty($_GET['reserveAlert'])) {
	  ...
	  foreach($ritem as $id=>$r)
	  {
		if($r!='')
		{
	           $reserve_q = $dbs->query('SELECT r.member_id, m.member_name,r.item_code
			    FROM reserve AS r
			...
			{
				$reserve_msg = 'Item '.$reserve_d['item_code'].' is being reserved by member '.$reserve_d['member_name'].''; //mfc
...
echo '<div class="printPageInfo">Loan Report<a class="printReport" onclick="window.print()" href="#">['.__('Print Current Page').']</a>&nbsp;<a style="color:#990000;" target="_blank" href="export_loan_data_excel.php?memberid='.$memberID.'" title="Click To View File"><strong>[Export To Excel]</strong></a></div>';
echo ''."\n";
