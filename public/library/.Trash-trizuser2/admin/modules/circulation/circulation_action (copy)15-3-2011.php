...
/*$holiday_dayname='';
$holiday_date='';
$circulation->holiday_dayname =$holiday_dayname;
$circulation->holiday_date = $holiday_date;*/
...
/*
		...
		$bib3=$bib2[0];
		$memberID = $_SESSION['memberID'];	
		echo $temp_item="SELECT count(item_code) AS item_code FROM temp where member_id='$memberID' ";
		...
		echo $temp_val=substr($temp_val,0,-1);

		if($temp_val>=1)
		{
			echo $loan_item="select item_code from loan where member_id='$memberID' AND is_return=0";
			...
			echo "----------------";
			echo $loan_i=substr($loan_i,0,-1);
			echo "----------------";



		/*	echo $count_loan_item="select count(item_code) AS item_code from loan where member_id='$memberID' AND is_return=0";
			$count_loan_item=$dbs->query($count_loan_item);
			$count_loan_i='';
			while($row=$count_loan_item->fetch_assoc())
			{
				$count_loan_i.=$row['item_code'];
			}
			echo "----------------";
			echo $count_loan_i=substr($loan_i,0,-1);
			echo "----------------";
*/
...
/*
			...
			echo $temp_item="select item_code from temp where member_id='$memberID'";
			...
			while($temp_i1=$temp_item->fetch_assoc())
			{
				...
				echo  $temp_i=substr($temp_i,0,-1);
			echo $session_bib="SELECT biblio_id FROM item WHERE item_code IN($temp_i) ";
			...
			echo $count_loan="select count(loan_id) AS loan_id from loan";
			...
			{       echo "hello";
				...
				{       echo "hello00000";
					echo $loan_i.'fff';
					echo $session_bib2="SELECT biblio_id FROM item WHERE item_code IN($loan_i) ";
					...
					if(in_array($bib3,$explode1))
					{
					    ...
					    echo '<script type="text/javascript">';
					    echo 'alert(\''.__('You Cannot Issue Same Copy Of Bookss').'\');';
	...
	/*			if(in_array($bib3,$explode))
				{
					    echo '<script type="text/javascript">';
					    echo 'alert(\''.__('You Cannot Issue Same Copy Of Book').'\');';
			...
			$data['loan_date']=$dbs->escape_string($_loan_date);
		  		
		       $insert = $sql_op->insert('temp', $data);
...
/*		$temp_it="select item_code from temp where member_id='$memberID'";
		...
		$confirm="select confirm_date from temp_request where item_code IN ( $temp) AND member_id='$memberID' AND status='Confirm' ";
		...
		//echo $pp."gggggggggggg";

		$diff = abs(strtotime($pp) - strtotime($current_date));


		//$diff = abs(strtotime($current_date) - strtotime($confirm_date_t_request));
		...
		//{
			//echo "hello";
		        $yesterday='';
			$today = $current_date;
			$yesterday = date('Y-m-d', strtotime($today . " - $days day"));
			//echo $yesterday;
...
//exit;
    ...
    if ($add == LOAN_LIMIT_REACHED) {
        ...
        echo '</body>';
        echo '</html>';
      //commenty by iresh on 13/1/2011  exit();
...
<input type="submit" value="'.__('Cancel').'")" /><input type="hidden" name="cancel" value="cancel" />
</form>';
        echo '</td>';
        echo '</tr>'."\n";
        echo '<tr>'."\n";
