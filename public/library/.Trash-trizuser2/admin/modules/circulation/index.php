...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'circulation/index.php?action=start class="headerText2">Book Circulation</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
<?php
	...
	$select=$dbs->query("select issue,checkin,reserve from member where member_id='".$_SESSION['receipt_record']['memberID']."'");
...
//utility::jsAlert("select issue,checkin,reserve from member where member_id='".$_SESSION['receipt_record']['memberID']."'");
		//print_r($_SESSION['receipt_record']['loan']);
	      //  print_r($_SESSION['receipt_record']['return']);
  	        $count_issue=count($_SESSION['receipt_record']['loan']);
		$count_return= count($_SESSION['receipt_record']['return']);	
                //utility::jsAlert($count_issue);	
		if($issue=='1')
		{
			//utility::jsAlert($count_issue);	
			if(($count_issue) > '0')
