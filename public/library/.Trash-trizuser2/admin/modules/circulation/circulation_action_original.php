...
$memberID = $_SESSION['memberID'];
		...
		$memberID = $_SESSION['memberID'];	
		$temp_item="SELECT count(item_code) AS item_code FROM temp where member_id='$memberID' ";
		...
		{       
			...
			$temp_item="select item_code from temp where member_id='$memberID'";
			...
			{
				    echo '<script type="text/javascript">'; 
                                    //added and commented start by Parth 10/8/2011 
				    //echo 'alert(\''.__('You Cannot Issue Same Copy Of Book').'\');';
				   echo 'alert(\''.__('Item Code not Exist').'\');';
                                     //added and commented End by Parth 10/8/2011     
		                    echo '</script>';
				    die();
			}
    ...
    if ($add == LOAN_LIMIT_REACHED) {
        ...
        echo '</html>';
exit();
      //commenty by iresh on 13/1/2011  
...
if (isset($_POST['memberID']) OR isset($_SESSION['memberID'])) {
    ...
    if (!$member->valid()) {
        ...
        //added start by Parth 10/8/2011
        utility::jsAlert('Invalid MemberID Please Check Again!!!'); 
        //added ended by Parth 10/8/2011     
    } else {
