...
//commented and added by Parth 8/7/2011
        //$data['label_name'] = 'label-'.strtolower(str_ireplace(array(' ', 'label-', '_'), array('-', '', '-'), $dbs->escape_string($labelName)));
$data['label_name'] = strtolower(str_ireplace(array(' ', 'label-', '_'), array('-', '', '-'), $dbs->escape_string($labelName)));
//Ended Addition By Parth 8/7/2011
	...
	$checkflag = 0;
	$rec_f = $dbs->query('SELECT label_name from mst_label where label_id ='.$itemID);
		while($row = $rec_f->fetch_assoc())
		{
			$setlabel = $row['label_name'];
			//utility::jsAlert(__('You can not Delete Topic '.$setlabel));
		}
	$rec_p = $dbs->query('SELECT labels from biblio');
		while($rowlabel = $rec_p->fetch_assoc())
		{
			$getlabel = $rowlabel['labels'];
			if(strpos($getlabel,$setlabel))
				{				
			//utility::jsAlert(__('You can not Delete Topic '.$setlabel));
				$checkflag=1;
				}
	...
	if($checkflag==1)
	{
		$rec_name = $dbs->query('SELECT label_name from mst_label where label_id ='.$itemID);
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'master_file/label.php class="headerText2">Label</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
   ...
   /*added by iresh on 25-1-2011*/ $form->addTextField('text', 'labelDesc', __('Label Description'), $rec_d['label_desc'], 'style="width: 140px;" maxlength="50" onkeyup="return checkspecialcharacterdynamic(this.name);"');
    ...
    if ($can_read AND $can_write) {
/*added and commented by Parth 8/7/2011 */    
