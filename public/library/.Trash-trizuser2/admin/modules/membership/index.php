...
/*if(!empty($_SESSION['membermail']))
{
require_once SENAYAN_BASE_DIR.'admin/modules/membership/member_notification_mail.php';
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
 ...
 $_SESSION['membermail']= trim($dbs->escape_string(strip_tags($_POST['memberEmail'])));
    ...
    if (empty($memberID) OR empty($memberName)) {
        ...
        utility::jsAlert(__('Please Enter Password!'));
        exit();
    }	
//commented and added by Parth 30/7/2011
	/*else if (($mpasswd1 AND $mpasswd2) AND ($mpasswd1 != $mpasswd2)) {
        ...
        utility::jsAlert(__('Please Enter Confirmation Password!'));
        exit();
    }else if (($mpasswd1 != $mpasswd2)) {
        utility::jsAlert(__('Password confirmation does not match. See if your Caps Lock key is on!'));
        exit(); 
	}else {
	...
	if (!empty($_FILES['image']) AND $_FILES['image']['size']) {
		...
		//send mail of updation by Parth 23/7/2011
		//require_once SENAYAN_BASE_DIR.'admin/modules/membership/member_notification_mail.php';
                // upload status alert
...
<div class="menuBoxInner memberIcon">
   ...
   <!--comment by iresh on 25-1-2011 <input type="text" name="keywords" size="30" /><?php if (isset($_GET['expire'])) { echo '<input type="hidden" name="expire" value="true" />'; } ?>-->
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
   ...
   if (!($can_read AND $can_write)) {
    ...
    // member institution
    $form->addTextField('text', 'instName', __('Institution'), $rec_d['inst_name'], 'style="width: 100%;" onchange="return checkspecialcharacterdynamic(this.name);"');
    // member type
...
$str_input = simbio_form_element::selectlist('memberTypeID', $mtype_options, $rec_d['member_type_id']);
$form->addAnything(__('Membership Type').'*', $str_input);
    //$str_input .= simbio_form_element::checkbox('materialsubid', $mst_material_sub_type_options, '', 'style="width: 20%;"');
 //$str_input .= '<div id="select" class="alterCell2" colspan="3"></div>';
...
$form->addAnything(__('Member messaging preferences '), $str_input);
   ...
   // $form->addSelectList('memberTypeID', __('Membership Type').'*', $mtype_options, $rec_d['member_type_id']);
    ...
    // member postal
    $form->addTextField('text', 'memberPostal', __('Postal Code'), $rec_d['postal_code'], 'style="width: 60%;" onchange="return numericcheck(this.name);"');
    // member phone
    $form->addTextField('text', 'memberPhone', __('Phone Number'), $rec_d['member_phone'], 'style="width: 60%;" onchange="return numericcheck(this.name);"');
    // member fax
    $form->addTextField('text', 'memberFax', __('Fax Number'), $rec_d['member_fax'], 'style="width: 60%;" onchange="return numericcheck(this.name);"');
    // member pin
 $form->addTextField('text', 'memberPIN', __('Personal ID Number'), $rec_d['pin'], 'style="width: 100%;" onchange="return numericcheck(this.name);"');
    // member notes
    ...
    // member email
    $form->addTextField('text', 'memberEmail', __('E-mail'), $rec_d['member_email'], 'style="width: 40%;" onchange="return emailcheck(this.name);"');
    // member password
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('m.member_id',
            ...
            'DATE_FORMAT(m.last_update,"%d-%m-%Y") AS \''.__('Last Updated').'\'');
    } else {
