        ...
        while (!feof($file)) {
            ...
            if ($row_count < $record_offset) {
                ...
                if ($field) {
                    ...
                    $items = trim($field[17]);
		    $specific_detail= $field[18]?'\''.$field[18].'\'':'NULL';
                    $copies = trim($field[19]);
		    $price = trim($field[20]);
		    $arr_label=array();
          	    $arr_label[] = array('label-new', $field[21]?$field[21]:'NULL');
                    	  
		    $url = $arr_label?serialize($arr_label):'NULL';
	            $url = '\''.$url.'\'';
                  
                    $supplier_id = utility::getID($dbs, 'mst_supplier', 'supplier_id', 'supplier_name', $field[22], $supplier_id_cache);
		    $subtitle= preg_replace('@^\\\s*'.$field_enc.'@i', '', $field[23]);
                    $subtitle = '\''.$subtitle.'\'';

                    $volumeno = $field[24]?'\''.$field[24].'\'':'NULL';
                    $serialno = $field[25]?'\''.$field[25].'\'':'NULL';
 
                    $keywords= preg_replace('@^\\\s*'.$field_enc.'@i', '', $field[26]);
                    $keywords = '\''.$keywords.'\'';  

                    $bookreview= preg_replace('@^\\\s*'.$field_enc.'@i', '', $field[27]);
                    $bookreview = '\''.$bookreview.'\'';       

                    $standards = trim($field[28]);   

                   $frequency_id = utility::getID($dbs, 'mst_frequency', 'frequency_id', 'frequency', $field[29], $frequency_id_cache);      
                    // sql insert string
                   $sql_str = "INSERT IGNORE INTO biblio (title, gmd_id, edition,spec_detail_info,
                        isbn_issn, publisher_id, publish_year,
                        collation, series_title, call_number,
                        language_id, publish_place_id, classification,
                        notes, image, file_att, input_date, last_update,labels,sub_title,vol_no,serial_no,tags,review,frequency_id)
                            VALUES ($title, $gmd_id, $edition,$specific_detail,
                            $isbn_issn, $publisher_id, $publish_year,
                            $collation, $series_title, $call_number,
                            $language_id, $publish_place_id, $classification,
                            $notes, $image, $file_att, $curr_datetime, $curr_datetime,$url,$subtitle,$volumeno,$serialno,$keywords,$bookreview,$frequency_id)";
			...
			if (!empty($standards)) {
                            ...
                            foreach ($standards as $standard) {
                                ...
                                $standard_id = utility::getID($dbs, 'mst_standard', 'standard_id', 'standard_name', $standard, $standard_id_cache);
                                $biblio_standard_sql .= " ($biblio_id, $standard_id, 2),";
                            }
                            // remove last comma
                            $biblio_standard_sql = substr_replace($biblio_standard_sql, '', -1);
                            // execute query
                            $dbs->query($biblio_standard_sql);
                             //echo $dbs->error;
                        }
                        // items
                        if (!empty($items)) {
