  ...
  if($form->edit_mode = false)
{
	 if(empty($copies))
  	...
  	$data['item_code'] = $dbs->escape_string($itemCode);
       ...
       for($i=0;$i<$copies;$i++)//loop added by iresh on 26-1-2011 to add multiple copies of book by simply inserting number of copies
       {		
           //$insert = $sql_op->insert('item', $data);
         //  $data['item_code']++;
		$data['item_code']=str_pad($data['item_code'],13,"0",STR_PAD_LEFT);
		$insert = $sql_op->insert('item', $data);	    
		$data['item_code']++;
       }
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/item.php class="headerText2">Item List</a>';
echo $bradecum;
        ?>	
	</td>
</tr>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
   ...
   $form->addTextField('text', 'copies', __('Copies'), $rec_d['copies'], 'style="width: 40%;" onchange="return numericcheck(this.name);"');
    ...
    // collection type
        ...
        $coll_type_q = $dbs->query("SELECT coll_type_id, coll_type_name FROM mst_coll_type order by coll_type_id");
