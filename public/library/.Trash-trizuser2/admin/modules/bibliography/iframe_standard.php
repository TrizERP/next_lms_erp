...
{
    var confirmBox = confirm('Are you sure to remove selected standard?' + "\n" + 'Once deleted, it can\'t be restored!');
    if (confirmBox) {
...
if (isset($_GET['removesess'])) {
    $idx = $_GET['removesess'];
    unset($_SESSION['biblioStandard'][$idx]);
    echo '<script type="text/javascript">';
    echo 'alert(\'Standard removed!\');';
    echo 'location.href = \'iframe_standard.php\';';
    echo '</script>';
}
...
if (isset($_POST['remove'])) {
    ...
    $sql_op->delete('biblio_standard', 'standard_id='.$id.' AND biblio_id='.$bid);
    echo '<script type="text/javascript">';
    echo 'alert(\'Standard succesfully removed!\');';
    echo 'location.href = \'iframe_standard.php?biblioID='.$bid.'\';';
    echo '</script>';
}
...
if ($biblioID) {
    ...
    $biblio_topic_q = $dbs->query("SELECT bt.*,t.standard_id, t.standard_name AS standard_name FROM biblio_standard AS bt
        LEFT JOIN mst_standard AS t ON bt.standard_id=t.standard_id
        WHERE bt.biblio_id=$biblioID");
    ...
    while ($biblio_topic_d = $biblio_topic_q->fetch_assoc()) {
        ...
        // remove link
        $remove_link = '<a href="#" onclick="confirmProcess('.$biblioID.', '.$biblio_topic_d['standard_id'].')"
            style="color: #FF0000; text-decoration: underline;">Delete</a>';
        $topic = $biblio_topic_d['standard_name'];

        $table->appendTableRow(array($remove_link, $topic));
    ...
    if ($_SESSION['biblioStandard']) {
        ...
        foreach ($_SESSION['biblioStandard'] as $biblio_session) {
            // remove link
            $remove_link = '<a href="iframe_standard.php?removesess='.$biblio_session[0].'"
                style="color: #000000; text-decoration: underline;">Remove</a>';

            if ($biblio_session) {
                $topic_q = $dbs->query("SELECT standard_name FROM mst_standard WHERE standard_id=".$biblio_session[0]);
