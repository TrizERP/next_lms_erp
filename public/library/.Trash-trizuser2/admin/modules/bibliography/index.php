...
{
utility::jsAlert(__('Invalid Extension'));
}
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
	...
	utility::jsAlert(__('Material Resource Type can not be empty'));
        exit();
	}if($_POST['materialresourceid']=="14")//added by parth 10/8/2011
	{
     	   if($_POST['frequencyID']==0)
       	 {
		utility::jsAlert(__('Frequency cant be empty'));
       	 exit();
       	 }
	}if($_POST['gmdID']==0)
	{
...
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/index.php?action=detail&physical=physical class="headerText2">Physical Resources</a>';
}
else if(isset($_REQUEST['action1']))
{
    
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/index.php class="headerText2">Physical Resource List</a>';
}
else 
{
    
$bradecum .= '<a href='.MODULES_WEB_ROOT_DIR.'bibliography/index.php class="headerText2">Library Resource List</a>';
}
...
<li><a href="<?php echo  MODULES_WEB_ROOT_DIR; ?>bibliography/index.php?action=detail&physical=physical" class="headerText2"><?php echo __('Physical Resources'); ?></a> </li><li>  
<a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/virtual_resources.php?action=detail" class="headerText2"><?php echo __('Virtual Resources'); ?></a></li>
</ul></td></tr></table>
...
if(isset($_REQUEST['action']))
{
echo strtoupper(__('Physical Resources')); ?> - <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/index.php?action=detail" class="headerText3">
<?php echo __('Add New Physical Resource'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/index.php?action1=detail" class="headerText3"><?php echo __('Physical Resource List'); ?></a>
<?php
}
...
//commented added by Parth 20/7/2011
...
//commented ended by Parth 20/7/2011
//added started by Parth 2/9/2011
...
//added ended by Parth 2/9/2011
    ...
    // biblio item add
    ///*commnent by iresh and Parth on 7-2-2011 and 2/9/2011
...
//commnted ended by Parth 3/9/2011
    ...
    function showTitleAuthors($obj_db, $array_data)
    {
        ...
        if(isset($_REQUEST['action1']))
	{
        $query = 'SELECT b.title, b.publish_year, b.publisher_id, a.author_name, opac_hide, promoted FROM biblio AS b
            LEFT JOIN biblio_author AS ba ON b.biblio_id=ba.biblio_id
            LEFT JOIN mst_author AS a ON ba.author_id=a.author_id
            WHERE b.biblio_id='.$array_data[0]; 
        $_sql_biblio_q = $query;
    ...
    if ($can_read AND $can_write) {
       /* $datagrid->setSQLColumn('biblio.biblio_id', 'biblio.biblio_id AS bid',
            ...
            'DATE_FORMAT(biblio.last_update,"%d-%m-%Y %H:%i:%s") AS \''.__('Last Update').'\'','biblio.image AS \''.__('').'\'');*/
        $datagrid->setSQLColumn('biblio.biblio_id', 'biblio.biblio_id AS bid',
            'biblio.title AS \''.__('Title').'\'',
            'biblio.image AS \''.__('').'\'');
       ...
       /* $datagrid->setSQLColumn('biblio.biblio_id AS bid', 'biblio.title AS \''.__('Title').'\'',
            'biblio.isbn_issn AS \''.__('ISBN/ISSN').'\'',
            'IF(COUNT(item.item_id)>0, COUNT(item.item_id), \'<strong style="color: #FF0000;">'.__('None').'</strong>\') AS \''.__('Copies').'\'',
            'biblio.last_update AS \''.__('Last Update').'\'','biblio.image AS \''.__('IMAGE').'\'');*/ 
          $datagrid->setSQLColumn('biblio.biblio_id AS bid', 'biblio.title AS \''.__('Title').'\'',
          'biblio.image AS \''.__('').'\''); 
        // modify column value
    ...
    if(isset($_REQUEST['action1']))
		{
                $table_spec = 'biblio LEFT JOIN item ON biblio.biblio_id=item.biblio_id WHERE biblio.material_resource_id IN (Select material_resource_id from mst_material_resource_type WHERE material_resource_name = "Physical Library")';
		}
    else
		{
