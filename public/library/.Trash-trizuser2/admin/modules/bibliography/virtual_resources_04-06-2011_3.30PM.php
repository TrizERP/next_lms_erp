...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
 ...
 // biblio gmd
  ...
  $gmd_q = $dbs->query('SELECT gmd_id, gmd_name FROM mst_gmd where gmd_code!="5"');
	...
	// $gmd_options=explode(",",$gmd_options);
         //print_r($gmd_options);
	//foreach($gmd_options as $id
 //$ajax_exp = "ajaxFillSelect('".SENAYAN_WEB_ROOT_DIR."admin/AJAX_lookup_handler.php', 'mst_gmd', 'gmd_id:publisher_name', 'publisherID', $('publ_search_str').getValue())";
	
//$str_input = simbio_form_element::selectList('gmdID', $gmd_options, $rec_d['gmd_id'], 'onblur="ajaxCheckID(\''.SENAYAN_WEB_ROOT_DIR.'admin/new.php\', \'mst_gmd\', \'gmd_id\', \'msgBox\', \'gmdID\')" style="width: 40%;"');

/*$str_input = simbio_form_element::selectList('gmdID', $gmd_options, $rec_d['gmd_id'], 'onchange="ddd(\''.SENAYAN_WEB_ROOT_DIR.'admin/new.php\', \'mst_gmd\', \'gmd_id\', \'msgBox\', \'gmdID\')" style="width: 40%;"');
  ...
  /*  // biblio labels
    ...
    if ($form->edit_mode) {
        echo '<div class="infoBox" style="overflow: auto;">'
            .'<div style="float: left; width: 80%;">'.__('You are going to edit biblio datasssssss').' : <b>'.$rec_d['title'].'</b>  <br />'.__('Last Updated').$rec_d['last_update'].'</div>'; //mfc
            if ($rec_d['image']) {
    ...
    $datagrid->setSQLcriteria('WHERE g.gmd_code="4" ');
    // is there any search
