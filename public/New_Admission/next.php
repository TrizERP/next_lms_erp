...
if (isset($_REQUEST['btn_submit'])) 
{
    ...
    if ($standard != '') 
    {
     ...
     $query = "INSERT INTO `new_admission_inquiry_registration`
            (`admission_std`,`syear`,`date_of_birth`,`age`, `mobile`, `otp`, `token`, `created_on`,`father_adhar`,`admission_for_child_twins`,`child_name`,`father_name`,`mail`,`address`,`mother_adhar`,`student_quota`,`sibling_details`,`sub_institute_id`) 
        VALUES('" . $standard . "','".$syear."', '" . $dob . "', '" . $age . "', " . $mobile . ", '" . $otp . "', '" . $token . "',Now(),'" . $father_adhar . "','".$admission_for_child_twins."','".$child_name."','".$father_name."','".$mail."','".$address."','".$mother_adhar."','".$student_quota."','".$sibling_details."','".$sub_institute_id."')";
    ...
    $sms = "https://49.50.67.32/smsapi/httpapi.jsp?username=MMISVR&password=abc@123&from=MMISVR&coding=0&to=$_SESSION[mobile]&text=$final_token";
    $ch = curl_init();
