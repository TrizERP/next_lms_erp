...
//	die();
	$subArr = mysqli_query($cn, "SELECT * FROM sub_std_map WHERE sub_institute_id = '".$sub_institute_id."' AND standard_id = '".$standard."' order by sort_order");
...
if ($_POST['action'] == 'getChapter') {
	...
	AND standard_id = '".$standard."' AND subject_id = '".$subject."' order by sort_order");
...
if ($_POST['action'] == 'getTopic') {
    ...
    AND chapter_id = '".$chapter."' order by topic_sort_order";
