    ...
    if (in_array($ext, $allowed)) {
        if (is_uploaded_file($_FILES['filename']['tmp_name'])) {
            ...
            foreach ($full_array as $key => $subvalue) {
                //START for insert sub_institute_id in table
                $dataArr[$key]['sub_institute_id'] = trim($_SESSION['SUB_INSTITUTE_ID']);
                //END for insert sub_institute_id in table
        ...
        // $fieldQuery = '';  //Comment for insert sub_institute_id in table
        ...
        //START for insert sub_institute_id in table
        $fieldQuery = 'sub_institute_id,'; 
        $valueFields1[1]['field'] = 'sub_institute_id';
        //END for insert sub_institute_id in table

        $vf = 2; //Change $vf = 1 to 2 for insert sub_institute_id in table
        ...
        foreach($dataArr as $key => $value){
            ...
            foreach($valueFields1 as $kvf => $valueFields){
                if($valueFields['field'] == "SYEAR" || $valueFields['field'] == "syear"){
                ...
                else if($valueFields['field'] == "admission_date" || $valueFields['field'] == "ADMISSION_DATE" || $valueFields['field'] == "dob" || $valueFields['field'] == "DOB" || $valueFields['field'] == "start_date" || $valueFields['field'] == "followup_date" || $valueFields['field'] == "date_of_birth" || $valueFields['field'] == "birthdate" || $valueFields['field'] == "date"){
                    ...
                    if(in_array($valueFields['field'], $relationFields)){
                        ...
                        $checkSubInstitute = mysqli_query($cn, "SELECT DISTINCT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME IN ('sub_institute_id','SUB_INSTITUTE_ID') AND TABLE_SCHEMA='triz_erp_2' AND TABLE_NAME = '".$relationTable[$valueFields['field']]['TABLE_NAME']."'");
        ...
        $valueQuery = rtrim($valueQuery,',');
        //echo $insertQuery."<br/>".$fieldQuery."<br/>".$valueQuery;
        //die;
