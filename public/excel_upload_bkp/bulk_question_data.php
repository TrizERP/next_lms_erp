...
$topic_id = isset($_REQUEST['topic']) ? $_REQUEST['topic'] : null;
...
if (isset($_REQUEST['submit'])) {
        ...
        foreach ($dataArr as $key => $value) {
            ...
            if ($value['Question'] != "") {
                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    if(!empty($questionLevelArr)){
                        $question_level_id = mysqli_fetch_assoc($questionLevelArr);
                        $question_level_id = $question_level_id['id'];
                    }
                    //END Get Question Level ID

                    $questionSkill = "SELECT * FROM lms_mapping_type WHERE name = '" . mysqli_real_escape_string($cn, $value['Competancy_Skill']) . "'
                    AND parent_id = 73548";
                    $questionSkillArr = mysqli_query($cn, $questionSkill) or die(mysqli_error($cn));
                    if(!empty($questionSkillArr)){
                    $question_skill_id = mysqli_fetch_assoc($questionSkillArr);
                    $question_skill_id = $question_skill_id['id'];
                    }
                    ...
                    $insertQuesQuery = "INSERT INTO lms_question_master(question_type_id,grade_id,standard_id,subject_id,chapter_id,topic_id,question_title,description,points,multiple_answer,sub_institute_id,status,created_by,created_on,concept,subconcept,answer,hint_text)
                    VALUES('" . $question_type_id . "','" . $_REQUEST['grade'] . "','" . $_REQUEST['standard'] . "',
                    '" . $_REQUEST['subject'] . "','" . $_REQUEST['chapter'] . "','" . $topic_id . "','" . mysqli_real_escape_string($cn, $value['Question']) . "','" . mysqli_real_escape_string($cn, $value['Description']) . "','" . $value['Points'] . "','0','" . $_REQUEST['sub_institute_id'] . "','1','" . $_REQUEST['user_id'] . "',now(),'" . mysqli_real_escape_string($cn, $value['Concept']) . "','" . mysqli_real_escape_string($cn, $value['SubConcept']) . "','" . mysqli_real_escape_string($cn, $value['Answer']) . "','" . mysqli_real_escape_string($cn, $value['Hint']) . "')";
                    ...
                    //INSERT LMS Mapping Compentacy Skill
                    if(!empty($questionSkillArr)){
                    $insertQueryCT = "INSERT INTO lms_question_mapping(questionmaster_id,mapping_type_id,mapping_value_id)
                    VALUES('" . $question_id . "','73548','" . $question_skill_id . "')";
                    mysqli_query($cn, $insertQueryCT) or die(mysqli_error($cn));
                    }
                    ...
                    for ($i = 1; $i <= 4; $i++) {
                        ...
                        if ($value[$option] != "") {
                            ...
                            $insertQuery = "INSERT INTO answer_master(question_id,answer,correct_answer,sub_institute_id,created_by,created_on)
    ...
    <br><br><span style='color:red'>Already Exists -  " . $records_not_uploaded . "
