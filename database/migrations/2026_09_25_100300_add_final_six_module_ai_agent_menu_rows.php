 ...
 * does not exist. This is the fourth and final instalment of the fix first made for Fees by
 * 2026_09_18_130000.
 *
 ...
 * these, and if the Fees row has no grants this migration writes none.
 *
 * FOUR LINKS ARE SPELLED FOR KEYS THAT PREDATE THIS WORK, AND TWO CARRY A HYPHEN
 *
 * The right is always asked for as `agents.<ai_modules key>` — which is what
 * `rbacModuleKey(module)` builds in the frontend registry — so the link has to spell the
 * key exactly. `ai_agents.document-templates` and `ai_agents.migration-modules` are
 * hyphenated because those `ai_modules` keys are, and `ai_agents.migration-modules` is the
 * UTILITY module's row: that key has claimed `/Utility/**` since the workspace was seeded.
 * Writing either with an underscore would create a row nothing ever looks up.
 *
 * NOTHING BELONGING TO AN EARLIER MODULE IS MODIFIED. The Fees row is read and never
 * written; the seventeen rows added before these are not touched at all.
 *
 * IDEMPOTENT, keyed on `link` and on (menu_id, profile_id, sub_institute_id).
 *
 * Run it on its own — `php artisan migrate` must never be run bare on this project:
 *
 *   php artisan migrate --path=database/migrations/2026_09_25_100300_add_final_six_module_ai_agent_menu_rows.php
 */
return new class extends Migration
{
     ...
     *
     * The links spell the `ai_modules` key, not the menu slug. Utility is therefore
     * `migration-modules`, and two of the six carry a hyphen. Sort orders continue from 21,
     * which `ai_agents.transportation` holds.
     */
    private const CHILDREN = [
        'ai_agents.inventory' => ['Inventory', 'mdi mdi-package-variant-closed', 22],
        'ai_agents.front_desk' => ['Front Desk', 'mdi mdi-desk', 23],
        'ai_agents.task_management' => ['Task Management', 'mdi mdi-clipboard-check-outline', 24],
        'ai_agents.complaint' => ['Complaint', 'mdi mdi-alert-circle-outline', 25],
        'ai_agents.migration-modules' => ['Utility', 'mdi mdi-database-cog-outline', 26],
        'ai_agents.document-templates' => ['Document Templates', 'mdi mdi-file-document-multiple-outline', 27],
    ];
    ...
    public function down(): void
    {
        ...
        // Only the six rows this migration introduced. `ai_agents` and the seventeen
