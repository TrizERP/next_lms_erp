...
{
    private function tenantWithInventory(): ?array
    {
        try {
            $row = DB::table('item_scan_details as isd')
                ->join('academic_year as ay', function ($j) {
                    $j->on('ay.sub_institute_id', '=', 'isd.sub_institute_id')
                      ->on('ay.syear', '=', 'isd.syear');
                })
                ->select('isd.sub_institute_id', 'isd.syear', DB::raw('COUNT(*) as c'))
                ->whereNotNull('isd.sub_institute_id')
                ->where('isd.sub_institute_id', '>', 0)
                ->groupBy('isd.sub_institute_id', 'isd.syear')
                ->orderByDesc('c')
                ->first();
        } catch (\Throwable $e) {
            return null;
    ...
    {
        $uri = '/api/brain/' . $tenant . '/inventory/intelligence' . ($syear ? '?syear=' . $syear : '');
        ...
        return json_decode((new BrainInventoryIntelligenceController())->index($request)->getContent(), true);
    }

    public function test_payload_carries_canonical_contract_keys(): void
    {
        $info = $this->tenantWithInventory();
        if ($info === null) {
            $this->markTestSkipped('No inventory scan data with matching academic year in this database.');
        }
    ...
    public function test_findings_have_traceable_evidence(): void
    {
        $info = $this->tenantWithInventory();
        if ($info === null) {
            $this->markTestSkipped('No inventory scan data.');
        }
