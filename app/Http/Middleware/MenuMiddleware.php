...
class MenuMiddleware {
	...
	public function handle(Request $request, Closure $next)
    {
		...
		if ($user_profile_name == 'student' || $user_profile_name == 'Student' || $user_profile_name == 'STUDENT') {
			...
			INNER JOIN tblmenumaster m ON (i.menu_id = m.id OR g.menu_id = m.id) AND FIND_IN_SET(" . $sub_institute_id . ", m.sub_institute_id) WHERE u.sub_institute_id IN ('" . $sub_institute_id . "') AND u.id = '" . $user_id . "'";
		}
		else{
			...
			if($sub_institute_id == 0 && $is_admin == 1)
			{
	            ...
	            LEFT JOIN tblindividual_rights i ON u.id = i.user_id AND u.sub_institute_id = i.sub_institute_id
	            LEFT JOIN tblgroupwise_rights g ON u.user_profile_id = g.profile_id AND u.sub_institute_id = g.sub_institute_id
	            INNER JOIN tblmenumaster m ON (i.menu_id = m.id OR g.menu_id = m.id) AND FIND_IN_SET(" .$client_id. ", m.client_id) WHERE u.sub_institute_id IN ('" . $sub_institute_id . "') AND u.id = '" . $user_id . "'";
			}
			else
			{
				$extra_user_id = '';
				...
				{
					$new_sub_institute_id = session()->has('new_sub_institute_id');
					if(isset($new_sub_institute_id) && $new_sub_institute_id == 1)
					{
						$extra_user_id = '';
					}
				}
				...
				{
					$extra_user_id = " AND u.id = '" . $user_id . "' ";
				}
				$rightsQuery = "SELECT GROUP_CONCAT(distinct m.id) AS MID
			            FROM tbluser u
			            LEFT JOIN tblindividual_rights i ON u.id = i.user_id AND u.sub_institute_id = i.sub_institute_id
			            LEFT JOIN tblgroupwise_rights g ON u.user_profile_id = g.profile_id AND u.sub_institute_id = g.sub_institute_id
			            INNER JOIN tblmenumaster m ON (i.menu_id = m.id OR g.menu_id = m.id) AND FIND_IN_SET(" . $sub_institute_id . ", m.sub_institute_id) WHERE u.sub_institute_id IN ('" . $sub_institute_id . "') $extra_user_id ";
		...
		if (count($checkMenu) > 0)
		{
			if ($checkMenu[0]['menu_type'] == 'MASTER')
			{
				...
				if ($type != "API")
				{
					if($sub_institute_id == 0 && $is_admin == 1)
					{
						$data = tblmenumasterModel::where(['parent_menu_id' => "0", 'level' => "1"])->whereRaw("find_in_set('$client_id',client_id) and status = 1 and id in (" . $rightsMenusIds . ") and menu_type IS NULL")->orderBy('sort_order')->get()->toArray();

						$subMenuData = tblmenumasterModel::where('parent_menu_id', '!=', 0)->whereRaw("find_in_set('$client_id',client_id) AND level = 2 and id in (" . $rightsMenusIds . ") and status = 1 and menu_type IS NULL")->orderBy('sort_order')->get()->toArray();
						...
						$subChildMenuData = tblmenumasterModel::where('parent_menu_id', '!=', 0)->whereRaw("find_in_set('$client_id',client_id) AND level = 3 and id in (" . $rightsMenusIds . ") and status = 1 and menu_type IS NULL")->orderBy('sort_order')->get()->toArray();
						$i = 0;
					...
					else
					{
						...
						$subChildMenuData = tblmenumasterModel::where('parent_menu_id', '!=', 0)->whereRaw("find_in_set('$sub_institute_id',sub_institute_id) AND level = 3 and id in (" . $rightsMenusIds . ") and status = 1 and menu_type IS NULL")->orderBy('sort_order')->get()->toArray();
						$i = 0;
						foreach ($subChildMenuData as $key => $value) {
							$finalSubChildMenu[$value['parent_menu_id']][$i] = $subChildMenuData[$key];
							$i++;
