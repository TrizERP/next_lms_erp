...
class map_student_controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return \App\Helpers\is_mobile($type, "transportation/map_student/show", $data, "view");
    }
    ...
    public function create(Request $request)
    {
        ...
        if (isset($_REQUEST['grade']) && isset($_REQUEST['standard']) && isset($_REQUEST['division'])) {
            ...
            $student_data = \App\Helpers\SearchStudent($_REQUEST['grade'], $_REQUEST['standard']);
        } else if (isset($_REQUEST['grade'])) {
            $student_data = \App\Helpers\SearchStudent($_REQUEST['grade']);
        } 

        //START set default shift_from and shift_to
        $sql = "SELECT *,s.id as shift_id FROM academic_section a 
        INNER JOIN transport_school_shift s ON s.sub_institute_id = a.sub_institute_id
        WHERE a.sub_institute_id = '".session()->get('sub_institute_id')."' AND a.id = '".$_REQUEST['grade']."' AND shift_title like concat('%',shift,'%')";
        ...
        foreach ($student_data as $id => $arr) {
            ...
            } else 
            {
                ...
                $responce_arr['stu_data'][$id]['ddFromBus'] = $bus;
                $responce_arr['stu_data'][$id]['ddToBus'] = $bus;
                $responce_arr['stu_data'][$id]['ddFrom'] = array();
                $responce_arr['stu_data'][$id]['ddTo'] = array();
                $responce_arr['stu_data'][$id]['ddShift'] = $this->ddShift();
            }           
        }

        return \App\Helpers\is_mobile($type, "transportation/map_student/add", $responce_arr, "view");
    }
    ...
    {       
        ...
        $type = $request->input('type');
        return \App\Helpers\is_mobile($type, "map_student.index", $res, "redirect");
    }
    ...
    public function ajaxChackRemainCapacity(Request $request) {
        if ( $request->ajax() ) {
            ...
            $totalCapacity = isset($getTotalCapacity->sitting_capacity) ? $getTotalCapacity->sitting_capacity : '';
            $remainCapacity = $totalCapacity - $totalReserveCapacity;

            return $res = ['status' => 200, 'total_capacity' => $totalCapacity, 'total_remain_capacity' => $remainCapacity];
        }
    }
}
