...
class send_birthday_notification_controller extends Controller
{
    ...
    public function send_birthday_notification() {
        ...
        $students = DB::table('tblstudent')
            ->select('tblstudent.id', 'tblstudent.first_name', 'tblstudent.middle_name', 'tblstudent.last_name', 'tblstudent.sub_institute_id', 'tblstudent.dob', 'school_setup.SchoolName', 'school_setup.Logo', 'gcm_users.gcm_regid')
...
//die();
        ...
        $users = DB::table('tbluser')
            ->select('tbluser.id', 'tbluser.first_name', 'tbluser.middle_name', 'tbluser.last_name', 'tbluser.sub_institute_id', 'tbluser.birthdate', 'school_setup.SchoolName', 'school_setup.Logo', 'gcm_users.gcm_regid')
        ...
        if ( !empty($students) ) {
            foreach ($students as $student) {
                $description = "Happy Birthday " . strtoupper($student->first_name) ." ". strtoupper($student->middle_name) ." ". strtoupper($student->last_name);
                ...
                //$schoolLogo = ( $student->Logo ) ? $_SERVER['APP_URL'].'/admin_dep/images/'.$student->Logo : '';
                $schoolLogo = $_SERVER['APP_URL'].'/admin_dep/images/birthday.jpg';

                if (isset($student) && isset($description)) 
                {
                    $type = 'Happy Birthday';
                    $message = array(
                        'body' => $description, 
        ...
        if (!empty($users) ) {
            foreach ($users as $user) {
                ...
                //$schoolLogo = ( $user->Logo ) ? $_SERVER['APP_URL'].'/admin_dep/images/'.$user->Logo : '';
                $schoolLogo = $_SERVER['APP_URL'].'/admin_dep/images/birthday.jpg';
