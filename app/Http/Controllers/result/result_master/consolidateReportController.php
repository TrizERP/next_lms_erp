...
class consolidateReportController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // $getTerm = DB::table('academic_year')->where(['sub_institute_id' => $sub_institute_id, 'syear' => $syear])->orderBy('sort_order')->get()->pluck('title', 'term_id');

        // $getExamMasters = DB::table('result_create_exam as rce')->join('result_exam_master as rem', 'rem.id', '=', 'rce.exam_id')->where(['rce.sub_institute_id' => $sub_institute_id, 'rce.syear' => $syear, 'rce.standard_id' => $standard])->where('rce.report_card_status','Y')
        // ->orderByRaw('rem.SortOrder')
        // ->pluck('rem.ExamTitle', 'rem.Id');
        
        // return $getExamMasters;

        // $getSubjectList = DB::table("sub_std_map as ssm")->join('subject as sub', 'ssm.subject_id', '=', 'sub.id')
        ...
        //     ->where(['ssm.sub_institute_id' => $sub_institute_id, 'ssm.standard_id' => $standard, 'allow_grades' => "Yes"])->orderBy('ssm.sort_order')->get()->pluck('subject_name', 'subject_id');
        ...
        // Build examMasterWise as [ExamTitle][display_name][title] => exams
        foreach ($examData as $exam) {
            if (!isset($examMasterWise[$exam->ExamTitle][$exam->display_name][$exam->title])) {
                $examMasterWise[$exam->ExamTitle][$exam->display_name][$exam->title] = $exam;
                $createExamCount++;
            }
        }
        ...
        foreach ($examMasterWise as $examTitle => $subjects) {
            foreach ($subjects as $subjectName => $titles) {
                foreach ($titles as $title => $exam) {
                    ...
                    // Assign marks to student structure
                    foreach ($studentData as $student) {
                        $studentMarks[$student['student_id']][$examTitle][$subjectName][$title] = [
                            'exam_details' => $exam,
