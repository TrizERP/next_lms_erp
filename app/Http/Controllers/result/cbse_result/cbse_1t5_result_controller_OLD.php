...
// exit;
    ...
    public function getAllExam($standard_id, $term_id)
    {
        ...
        $str = 'SELECT e.title,em.ExamTitle,IF((e.con_point IS NULL) OR (e.con_point = ""),e.points,e.con_point) AS points ,em.Id,e.id as examId
                ...
                WHERE ' . $term . ' e.sub_institute_id = ' . session()->get('sub_institute_id') . '
        ...
        $responce[$id + 1]['mark'] = ($total_mark - 10);

        // echo "<pre>";
        // print_r(array_column($responce, 'mark','exam_id'));
        // exit;
                ...
                foreach ($all_exam as $exam_id => $exam_detail) {
                    ...
                    if (count($all_exam) - 1 != $exam_id) {
                        ...
                        if (isset($marks_arr[$arr_student['student_id']][$subject][$exam_detail['title']])) {
                            ...
                            foreach ($title as $title_data) {
                                ...
                                if (isset($is_absent) && $is_absent == "N.A.") {
                                    ...
                                    if ($con_point != NULL && $con_point != $total_mark) {
                                        ...
                                        // $marks = ($weightage * $gotMarks) / $points;
                                    }
                                    if ($title_data == "Periodic Test") {
                                        // continue;

                                        array_push($marksArr, $mark);
                                    } else {
                                        // break;
                                        $tot_obt_marks = $mark;
                                    }
                            ...
                            $total_gain_mark = $total_gain_mark + $tot_obt_marks;
                        }
                        $obtained_marks = $tot_marks = 0;
                        if (!empty($marksArr)) {
                        ...
                        if ($abFlag == 1) {
                            ...
                            // $total_gain_mark = $total_gain_mark;
                        }
