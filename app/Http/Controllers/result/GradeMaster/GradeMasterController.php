...
class GradeMasterController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return \App\Helpers\is_mobile($type, "result/GradeMaster/show_grade_type", $school_data, "view");
    }

    public function getData() {
        $sub_institute_id = session()->get('sub_institute_id');
        $syear = session()->get('syear');
        $data['grade'] = GradeMaster::
                        where(['sub_institute_id' => $sub_institute_id])
                        ->orderBy('sort_order')->get();
        foreach ($data['grade'] as $id => $arr) {
            $data['grade_data'][$arr['id']] = GradeMasterData::
                            where([
    ...
    public function store(Request $request) {
        ...
        $type = $request->input('type');
        return \App\Helpers\is_mobile($type, "grade_master.index", $res, "redirect");
    }

    public function AddAllData(Request $request, $id) {
        $type = $request->input('type');
        $data['grade_id'] = $id;
        return \App\Helpers\is_mobile($type, "result/GradeMaster/add_grade_data", $data, "view");
    }
