...
class ExamMasterController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return \App\Helpers\is_mobile($type, "result/ExamMaster/show_exam", $school_data, "view");
    }
    ...
    public function create(Request $request) {
        ...
        $maxCode = ExamMaster::
                ...
                ->max('Code');
        $maxCode = $maxCode + 1;
        $maxSortOrder = ExamMaster::
                where(['SubInstituteId' => $sub_institute_id])
                ->max('SortOrder');
        $maxSortOrder = $maxSortOrder + 1;

        $ddvalue = ExamTypeMater::
                where(['SubInstituteId' => $sub_institute_id])->get()->toArray();


        $dataStore['Code'] = $maxCode;
        $dataStore['SortOrder'] = $maxSortOrder;
        $dataStore['ddValue'] = $ddvalue;
        return \App\Helpers\is_mobile($type, 'result/ExamMaster/add_exam', $dataStore, "view");
    }
    ...
    public function store(Request $request) {
        ...
        $type = $request->input('type');
        return \App\Helpers\is_mobile($type, "exam_master.index", $res, "redirect");
    }

    public function getData() {
        $sub_institute_id = session()->get('sub_institute_id');
         $exam = ExamMaster::select('result_exam_master.*',  DB::raw('COUNT(result_create_exam.id) AS total_count,result_exam_type_master.ExamType'))
    ...
    public function edit(Request $request, $id) {
        ...
        return \App\Helpers\is_mobile($type, "result/ExamMaster/add_exam", $data, "view");
    }

    public function update(Request $request, $id) {

//        \App\Helpers\ValidateInsertData('exam_master', 'update');
