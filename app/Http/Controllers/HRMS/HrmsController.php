...
class HrmsController extends Controller
{
    ...
    public function compressImageTo100KB($file, $quality = 85)
    {
        $extension = strtolower($file->getClientOriginalExtension());
        $sourcePath = $file->getPathname();

        // Always convert to JPEG to simplify compression
        ...
        while (filesize($tempPath) > 102400 && $quality > 10) {
            $quality -= 5;
            // Recreate the image resource from original source
            if ($extension === 'png') {
                $image = imagecreatefrompng($sourcePath);
            } else {
                $image = imagecreatefromjpeg($sourcePath);
    ...
    public function hrmsInTimeStore(Request $request)
    {
        ...
        if ($request->hasFile('photo_in')) {
            ...
            $compressedFile = $this->compressImageTo100KB($file);
            $fileSize = $compressedFile->getSize();;
            /*
...
*/
            ...
            Storage::disk('digitalocean')->putFileAs('public/hrms/attendance_photo/', $compressedFile, $photoInFilename, 'public');
...
*/
            ...
            Storage::disk('digitalocean')->putFileAs('public/hrms/attendance_photo/', $compressedFile, $photoOutFilename, 'public');
