...
class outwardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $outward = DB::table('outward')
            ...
            ->select('outward.*', 'place_master.title as place_id','physical_file_location.title as file_name','physical_file_location.file_location as file_location_id')
        ...
        return \App\Helpers\is_mobile($type, "inward_outward/show_outward", $outward_data, "view");
    ...
    public function create(Request $request) {
        ...
        $get_outward_no = DB::select("SELECT (IFNULL(MAX(CAST(outward_number AS INT)),0) + 1) AS outward_no
                                    FROM outward
    ...
    {        
        ...
        $type = $request->input('type');
        return \App\Helpers\is_mobile($type, "add_outward.index", $message, "redirect");
    }
