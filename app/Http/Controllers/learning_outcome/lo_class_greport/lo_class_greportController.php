...
class lo_class_greportController extends Controller
{
        ...
        if (isset($request['id'])) {
            ...
            foreach ($sub_dd as $keys => $subject) {
                ...
                $result = DB::table("learning_outcome_indicator as LO")
                    ...
                    ->selectRaw('LO.SUBJECT,LO.INDICATOR,LQ.QUESTION_TITLE,sum(LQ.QUESTION_OUT_OF) tot,sum(LOM.MARKS) mar,(sum(LOM.MARKS)*100/sum(LQ.QUESTION_OUT_OF)) per')
            ...
            return is_mobile($type, 'learning_outcome/lo_class_greport/FinalShow', $school_data, 'view');
        } else {
            $school_data['data'] = $this->get_all_dd();
            $type = $request->input('type');

            return is_mobile($type, 'learning_outcome/lo_class_greport/show', $school_data, 'view');
        }
    }
    ...
    public function create(Request $request)
    {
        ...
        foreach ($sub_dd as $keys => $subject) {
            ...
            $result = DB::table("learning_outcome_indicator as LO")
                ...
                ->selectRaw('LO.SUBJECT,LO.INDICATOR,LQ.QUESTION_TITLE,sum(LQ.QUESTION_OUT_OF) tot,sum(LOM.MARKS) mar,(sum(LOM.MARKS)*100/sum
                (LQ.QUESTION_OUT_OF)) per')
                ->where("LQ.EXAM_TYPE", "=", $exam_type)
                ->where("LQ.medium", "=", $medium)
                ->where("LQ.standard", "=", $std)
                ->where("LQ.subject", "=", $subject)
    ...
    public function store(Request $request)
    {
        ...
        return is_mobile($type, 'lo_class_greport.index', $res, 'redirect');
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return is_mobile($type, "learning_outcome/lo_class_greport/edit", $data, "view");
    }
