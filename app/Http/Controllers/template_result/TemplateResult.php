...
//        $data['data'] = $this->getData();
        ...
        return \App\Helpers\is_mobile($type, "template_result/search", $data, "view");
    }
    ...
    public function show_result(Request $request) {
        ...
        $templateResult_html = templateMasterModel::where('sub_institute_id','=',$sub_institute_id)->where('module_name','=','Result')->first();
        if(isset($templateResult_html)){
        	$templateResult_html_content = $templateResult_html['html_content'];
        }else{
        	$templateResult_html = templateMasterModel::where('sub_institute_id','=','0')->where('module_name','=','Result')->first();
        	$templateResult_html_content = $templateResult_html['html_content'];
        }
        ...
        $getClasstech = $this->getClassTeacher($sub_institute_id,$syear,$_REQUEST['grade'], $_REQUEST['standard'], $_REQUEST['division']);

        $teacher_remark = $this->getTeacherRemark();

        $graphResult = $this->getGraphResult($all_subject_mark);
        // echo "<pre>";
        // print_r($all_subject_mark);
...
//        exit;
        ...
        // print_r($data['data']);
        // die;
        // $templateResult_html_content = $templateResult_html_content_;
        $gc = 1;
foreach ($data['data'] as $stuent_id => $all_data) { 
	...
	$templateResult_html_content = str_replace(htmlspecialchars("{{ADMISSION_NO}}"), $all_data['admission_no'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{STUDENT-ROLL_NO}}"), $all_data['roll_no'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{STUDENT_GR_NO}}"), $all_data['gr_no'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{STUDENT_NAME}}"), $all_data['name'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{STUDENT_IMG}}"), '/storage/student/'.$all_data['image'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{GUARDIAN_NAME}}"), $all_data['father_name'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{MOTHERS_NAME}}"), $all_data['mother_name'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{HOUSE}}"), $all_data['house'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{HEIGHT}}"), $all_data['height'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{WEIGHT}}"), $all_data['weight'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{RESIDENTIAL_ADDRESS}}"), $all_data['residential_address'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{ACADEMIC-YEAR}}"), $all_data['year'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{CONTACT_NO}}"), $all_data['contact_no'], $templateResult_html_content);
    ...
    $templateResult_html_content = str_replace(htmlspecialchars("{{TOTAL_ATTENDANCE_STUDENT}}"), stristr($all_data['att'],"/",true), $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{TOTAL_WORKING_DAYS}}"), str_replace("/","",stristr($all_data['att'],"/")), $templateResult_html_content);
    ...
    $templateResult_html_content = str_replace(htmlspecialchars("{{DIRECTOR_SIGN}}"), '<img height="50px" width="100px" src="/storage/result/director_sign/'.$footer_data['director_signatiure'].'" />', $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{SCHOOL_TEACHER_SIGN}}"), '<img height="50px" width="100px" src="/storage/result/teacher_sign/'.$footer_data['teacher_sign'].'" />', $templateResult_html_content);
    ...
    $templateResult_html_content = str_replace(htmlspecialchars("{{CLASS_TEACHER-REMARKS_SECTION}}"), "Class Teacher's remarks: ".$all_data['teacher_remark'], $templateResult_html_content);
    ...
    $templateResult_html_content = str_replace(htmlspecialchars("{{LEFT_LOGO}}"),'<img height="80px" width="80px" src="/storage/result/left_logo/'.$header_data['left_logo'].'" />' , $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{RIGHT_LOGO}}"), '<img height="80px" width="80px" src="/storage/result/right_logo/'.$header_data['right_logo'].'" />', $templateResult_html_content);

    $templateResult_html_content = str_replace(htmlspecialchars("{{CLASS_TEACHER}}"), $all_data['class_teacher'], $templateResult_html_content);
    
    $templateResult_html_content = str_replace(htmlspecialchars("{{SCHOOL-EMAIL}}"), $school_data['Email'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{SCHOOL_PHONE}}"), $school_data['Mobile'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{SCHOOL-AFILATION}}"), str_replace("Affiliation No:","",$header_data['line3']), $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{STD_SHORT_NAME}}"), $school_data['ShortCode'], $templateResult_html_content);
    $templateResult_html_content = str_replace(htmlspecialchars("{{REPORT-CARD_TERM}}"), 'Term I', $templateResult_html_content);

      $templateResult_html_content = str_replace(htmlspecialchars("{{CO-SCHOLASTIC SECTION}}"), View::make('template_result/co_scholastic_areas',compact('all_data')), $templateResult_html_content);
        ...
        $templateResult_html_content = str_replace(htmlspecialchars("{{GRAPH_RESULT}}"), View::make('template_result/result_graph',compact('gc','garphdat')), $templateResult_html_content);
        ...
        return \App\Helpers\is_mobile($type, "template_result/cbse_result", $data_, "view");
    }
    ...
    {
        $str_ = "SELECT * from school_setup WHERE  SchoolName = '".$school_name."' LIMIT 1";
        $result = DB::select(DB::raw($str_));
        $result = json_decode(json_encode($result),true);
...
//        exit;
    ...
    public function getGraphResult($data){
    	...
    	foreach ($data as $key1 => $value) {
    		...
    		foreach ($value as $key => $value) {
    		 	if(array_key_exists($key, $storeData)){
    		 		if($storeData[$key]['higt'] < $value['TOTAL_GAIN']){
    		 			$storeData[$key]['higt'] = $value['TOTAL_GAIN'];
    		 			$storeData[$key]['avge'] += $value['TOTAL_GAIN'];
    		 			$storeData[$key]['final_avge'] = $storeData[$key]['avge']/$numSub;
    		 		}else{
    		 			$storeData[$key]['avge'] = $storeData[$key]['avge'] + $value['TOTAL_GAIN'];
    		 			$storeData[$key]['final_avge'] = $storeData[$key]['avge']/$numSub;
    		 		}
    		 	}else{    		 		
    		 		$storeData[$key]['higt'] = $value['TOTAL_GAIN'];
    	...
    	foreach ($data as $key_ => $value) {
    		foreach ($value as $key2 => $value) {
    			if(array_key_exists($key2, $storeData)){
					$storeData[$key2]['marks'] = $value['TOTAL_GAIN'];  
    			}
    		}
    		$finalData[$key_] = $storeData;
    	}
    ...
    public function getClassTeacher($id_si,$syear,$grade_id,$standard_id,$division_id){
        ...
        ->where(['ct.sub_institute_id'=>$sub_institute_id,'ct.syear'=>$syear,'ct.grade_id'=>$grade_id,'ct.standard_id'=>$standard_id,'ct.division_id'=>$division_id])
        ->get();  
         $rep = [];
...
//        exit;
    ...
    public function getAllMark($all_exam, $all_subject, $all_student) 
    {
       ...
       // exit;
        ...
        $str = 'SELECT ex.id,rm.student_id,s.subject_id,s.display_name,s.elective_subject,SUM(ex.points) total_points,ex.con_point,SUM(rm.points) points,exm.Id exam_id
        ...
        foreach ($all_student as $students => $arr_student) 
        {
            foreach ($all_subject as $subject_id => $subject) 
            {
                ...
                {                    
                    ...
                    {
                        $responce_arr[$arr_student['student_id']][$subject]['TOTAL_GAIN'] = number_format($total_gain_mark,0);
        ...
        if (count($ret_mark_grade) > 0) {
            ...
            if ($type == "GRADE") {
                $sql_data = "select comark.student_id,comark.co_scholastic_id, cop.title parent_title,co.title child_title,cograde.title obtain_grade
                                ...
                                left join result_co_scholastic_grades cograde on cograde.id = comark.grade
