...
class feesReceiptBookMasterController extends Controller
{
    public function index(Request $request)
    {
        ...
        ->where(['fees_receipt_book_master.sub_institute_id' => $sub_institute_id,'fees_receipt_book_master.syear' => $syear])
    ...
    public function create(Request $request)
    {
        ...
        $receiptId = feesReceiptBookMasterModel::selectRaw("MAX(CAST(receipt_id AS UNSIGNED))+1 AS newcode")->where(['sub_institute_id'=>$sub_institute_id,'syear'=>$syear])->get()->toArray();
    ...
    public function standardList(Request $request)
    {
        $syear = $request->session()->get('syear');
        $sub_institute_id = $request->session()->get('sub_institute_id');
        $standardList = standardModel::where(['sub_institute_id' => $sub_institute_id])->pluck('grade_id', 'id')->toArray();
    ...
    public function gradeList(Request $request)
    {
        $syear = $request->session()->get('syear');
        $sub_institute_id = $request->session()->get('sub_institute_id');
        $gradeList = academic_sectionModel::where(['sub_institute_id' => $sub_institute_id])->pluck('title','id')->toArray();
...
//        $feeHeadList = fees_title::where(['syear' => $syear,'sub_institute_id' => $sub_institute_id,'other_fee_id' => '0'])->get()->toArray();
        $feeHeadList = fees_title::where(['syear' => $syear,'sub_institute_id' => $sub_institute_id])->get()->toArray();
    ...
    public function store(Request $request)
    {
        ...
        if($submit == 'Update')
        {
            $chkExistHeadSql = "SELECT * FROM fees_receipt_book_master WHERE fees_head_id IN (" . implode(",", $fees_head_id) . ") AND grade_id IN ('" . implode("','", $grade_ids) . "') 
            AND standard_id IN ('" . implode("','", $standard_ids) . "') AND receipt_id != '".$receipt_id."' AND sub_institute_id = '".$sub_institute_id."' AND syear = '".$syear."' ";
        }
        else
        {
            $chkExistHeadSql = "SELECT * FROM fees_receipt_book_master WHERE fees_head_id IN (" . implode(",", $fees_head_id) . ") AND grade_id IN ('" . implode("','", $grade_ids) . "') 
            AND standard_id IN ('" . implode("','", $standard_ids) . "') AND sub_institute_id = '".$sub_institute_id."' AND syear = '".$syear."' ";
        }    
        $chkExistHead = DB::select($chkExistHeadSql);    
        
        if(count($chkExistHead) == 0)
        {
            if($submit == 'Update')
            {
                feesReceiptBookMasterModel::where(["receipt_id" => $receipt_id,'sub_institute_id' => $sub_institute_id,'syear' => $syear])->delete();
            }
            ...
            // echo ('<pre>');print_r($bank_logo_name);exit;
            if($file_name != '')
            {
    ...
    public function destroy(Request $request,$id)
    {
        ...
        $syear = $request->session()->get('syear');
        $sub_institute_id = $request->session()->get('sub_institute_id');
        feesReceiptBookMasterModel::where(["receipt_id" => $id,'sub_institute_id' => $sub_institute_id,'syear' => $syear])->delete();
        $res['status_code'] = "1";
        $res['message'] = "Fees Receipt Book Added successfully";
        return is_mobile($type, "fees_receipt_book_master.index", $res);
    }
