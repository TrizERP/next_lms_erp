...
class otherNewfeesReportController extends Controller
{
    ...
    {    
        ...
        $other_feesData = DB::select("SELECT CONCAT_WS(' ',s.first_name,s.middle_name,s.last_name) AS student_name,s.enrollment_no,s.mobile,c.student_id,
        st.name as standard_name,d.name as division_name,h.display_name as fees_head,
        ...
        INNER JOIN tblstudent_enrollment se ON se.student_id = s.id AND se.syear = c.syear AND se.end_date is null
        INNER JOIN standard st on st.id = se.standard_id
        INNER JOIN division d on se.section_id = d.id
        WHERE c.sub_institute_id = '".$sub_institute_id."' AND c.syear = '".$syear."' AND c.is_deleted='N' ".$extraSearch."
        ORDER BY c.deduction_date
        ");
