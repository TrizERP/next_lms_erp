...
class feesModificationController extends Controller
{
      /**
...
// echo "<pre>";print_r($extraSearchArrayRawfp);exit;
    ...
    public function store(Request $request){
        ...
        $checkedFees = $request->checkedFees;
        $payment_modeArr = $request->payment_mode;
        $bank_nameArr = $request->bank_name;
        $bank_branchArr = $request->bank_branch;
        $cheque_noArr = $request->cheque_no;
        $cheque_dateArr = $request->cheque_date;

        $i=0;
        foreach ($checkedFees as $fees_id => $student_id) {

            $payment_mode = isset($payment_modeArr[$fees_id]) ? $payment_modeArr[$fees_id] : '';
            $bank_name = isset($bank_nameArr[$fees_id]) ? $bank_nameArr[$fees_id] : '';
            $bank_branch = isset($bank_branchArr[$fees_id]) ? $bank_branchArr[$fees_id] : '';
            $cheque_no = isset($cheque_noArr[$fees_id]) ? $cheque_noArr[$fees_id] : '';
            $cheque_date = isset($cheque_dateArr[$fees_id]) ? $cheque_dateArr[$fees_id] : '';
        ...
        return is_mobile($type, "fees_modification.index", $res);
    }
}
