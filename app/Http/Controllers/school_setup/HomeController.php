...
class gapi
{
    ...
    public function __construct($client_email, $key_file, $delegate_email = null)
    {
        ...
        if (version_compare(PHP_VERSION, '5.3.0') < 0) {
            throw new Exception('GAPI: PHP version '.PHP_VERSION.' is below minimum required 8.2.0');
        }
    ...
    protected function processFilter($filter)
    {
        ...
        $filter = str_replace(array(',', ';'), array('\,', '\;'),
            $filter); //Escape Google Analytics reserved characters
        $filter = preg_replace('/(&&\s*|\|\|\s*|^)([a-z0-9]+)(\s*'.$valid_operators.')/i', '$1ga:$2$3',
            $filter); //Prefix ga: to metrics and dimensions
        $filter = preg_replace('/[\'\"]/i', '', $filter); //Clear invalid quote characters
        $filter = preg_replace(array('/\s*&&\s*/', '/\s*\|\|\s*/', '/\s*'.$valid_operators.'\s*/'),
            array(';', ',', '$1'), $filter); //Clean up operators
...
class gapiRequest
{
    ...
    private function fopenRequest($get_variables = null, $post_variables = null, $headers = null)
    {
        ...
        return array(
            'body' => $response !== false ? $response : 'Request failed, consider using php5-curl module for more information.',
...
class HomeController extends Controller
{
    public function getAnalyticsSummary(Request $request)
    {
        ...
        define('ga_profile_id', '206728908');

        // require(app_path() . '\Http\Controllers\school_setup\ga.php');

        $ga = new gapi("triz-725@teizapp.iam.gserviceaccount.com", "teizapp-83364785632e.p12");

        $token = $ga->getToken();
        // $token = "asd";

        $vars['ids'] = "ga:206728908";
        $vars['metrics'] = "rt:activeUsers";
        $vars['dimensions'] = "rt:medium";
        ...
        if (isset($d['totalsForAllResults']['rt:activeUsers'])) {
            return $d['totalsForAllResults']['rt:activeUsers'];
        } else {
            return 1;
        }
