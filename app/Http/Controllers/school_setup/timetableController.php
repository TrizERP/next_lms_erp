...
class timetableController extends Controller
{
    ...
    public function AcademicwiseStandard(Request $request){        
        ...
        $sub_institute_id = $request->session()->get("sub_institute_id");                        
        $stdData = standardModel::where(['sub_institute_id' => $sub_institute_id,'grade_id' => $academic_id])
        ->get()->toArray();
        return $stdData;    
    }
    ...
    {     
        ...
        $type = $request->input('type');        
        $res = $this->getTimetable_data($request,$academic_section_id,$standard_id,$division_id,$sub_institute_id,$syear);
        return is_mobile($type,'school_setup/show_timetable',$res,"view");
    }
    ...
    {    
        ...
        {            
            ...
            {                
                ...
                foreach($week_data_arr as $week_day => $subject_id)
                {
                    ...
                    {
                        $check_exist_data = DB::SELECT("SELECT count(*) as total_data 
                                                        FROM timetable
                                                        WHERE sub_institute_id = '".$sub_institute_id."' 
                                                        AND syear = '".$syear."' 
                                                        AND academic_section_id = '".$hid_academic_section_id."'
                                                        AND standard_id = '".$hid_standard_id."' 
                                                        AND division_id = '".$hid_division_id."' 
                        ...
                        if(is_array($pval[$week_day]))
                        {
                            ...
                            {    
                                ...
                                {       
                                    $check_exist_data_batch = DB::SELECT("SELECT count(*) as total_data 
                                                        FROM timetable
                                                        WHERE sub_institute_id = '".$sub_institute_id."' 
                                                        AND syear = '".$syear."' 
                                                        AND academic_section_id = '".$hid_academic_section_id."'
                                    ...
                                    if($check_exist_data_batch[0]->total_data != 0)
                                    {
                                        ...
                                        timetableModel::where(["sub_institute_id" => $sub_institute_id,"syear" => $syear,"academic_section_id" => $hid_academic_section_id,"standard_id" => $hid_standard_id,"division_id" => $hid_division_id,"period_id" => $period_id,"week_day" => $week_day,"batch_id" => $batch_id])->update($finalArray);
                                    }
                                    else
                                    {
                        ...
                        else
                        {
                            ...
                            if($check_exist_data[0]->total_data != 0)
                            {
                                ...
                                timetableModel::where(["sub_institute_id" => $sub_institute_id,"syear" => $syear,"academic_section_id" => $hid_academic_section_id,"standard_id" => $hid_standard_id,"division_id" => $hid_division_id,"period_id" => $period_id,"week_day" => $week_day])->update($finalArray);
                            }
                            else
                            {
                                $finalArray = array(
                            ...
                            {
                                $get_academic_section_id = standardModel::where(['sub_institute_id' => $sub_institute_id,'id' => $standard_arr[$period_id][$week_day]])->get()->toArray();
                                ...
                                if($check_exist_data[0]->total_data != 0)
                                {
                                    ...
                                    timetableModel::where(["sub_institute_id" => $sub_institute_id,"syear" => $syear,"academic_section_id" => $new_academic_section_id,"standard_id" => $standard_arr[$period_id][$week_day],"division_id" => $division_arr[$period_id][$week_day],"period_id" => $period_id,"week_day" => $week_day])->update($finalArray);
                                }
                                else
                                {
                                    $finalArray = array(
        ...
        $type = $request->input('type');
        $res = $this->getTimetable_data($request,$hid_academic_section_id,$hid_standard_id,$hid_division_id,$sub_institute_id,$syear);
        $res['message'] = 'Timetable Added Successfully';               
        return is_mobile($type, "timetable.index", $res, "redirect");        
    }

    public function getTimetable_data(Request $request,$academic_section_id,$standard_id,$division_id,$sub_institute_id,$syear )
    {                        
        $html = "";        
        ...
        $subject_data =  sub_std_mapModel::where(['sub_institute_id'=>$sub_institute_id,"standard_id"=>$standard_id])
        ...
        foreach($week_data as $wkey => $wval)       
        {
            ...
            foreach($period_data as $pkey => $pval)                                                        
            {
                ...
                $assigned_teacher_data = timetableModel::select('timetable.*')                                
                                ->where(['sub_institute_id'=>$sub_institute_id,'syear' => $syear,'period_id' => $pval['id'],'week_day' => $wval])        
                                ->get()->toArray();
                ...
                if(isset($old_timetable_data[$wval][$pval['id']]['SUBJECT_ID']))
                {
                   ...
                   // $sub_count = $total_batches;               
                    ...
                    while($sub_count > 0)
                    {
                        ...
                        $old_assigned_teacher_id_array = array();
                        ...
                        $old_teacher_data = DB::select('SELECT `tbluser`.*, CONCAT_WS(" ",tbluser.first_name,tbluser.middle_name,tbluser.last_name) AS teacher_name,
                            (CASE WHEN total_lecture IS NULL THEN "Unlimited" ELSE tbluser.total_lecture - COUNT(t.id) END) AS remaining_lecture
                            FROM `tbluser`
                            INNER JOIN `tbluserprofilemaster` ON `tbluserprofilemaster`.`id` = `tbluser`.`user_profile_id`
                            LEFT JOIN `timetable` AS `t` ON `t`.`teacher_id` = `tbluser`.`id` AND `t`.`sub_institute_id` = `tbluser`.`sub_institute_id`
                            WHERE (`tbluser`.`sub_institute_id` = "'.$sub_institute_id.'" AND `tbluserprofilemaster`.`name` = "Teacher") 
                            '.$extra_where.'    
                            GROUP BY `tbluser`.`id`
                            ORDER BY `tbluser`.`first_name` ASC');
                ...
                {   //IF NOT BATCHES FOUND
                    ...
                    $new_teacher_data = DB::select('SELECT `tbluser`.*, CONCAT_WS(" ",tbluser.first_name,tbluser.middle_name,tbluser.last_name) AS teacher_name,
                            (CASE WHEN total_lecture IS NULL THEN "Unlimited" ELSE tbluser.total_lecture - COUNT(t.id) END) AS remaining_lecture
                            FROM `tbluser`
                            INNER JOIN `tbluserprofilemaster` ON `tbluserprofilemaster`.`id` = `tbluser`.`user_profile_id`
                            LEFT JOIN `timetable` AS `t` ON `t`.`teacher_id` = `tbluser`.`id` AND `t`.`sub_institute_id` = `tbluser`.`sub_institute_id`
    ...
    public function getNewStandardDiv(Request $request)
    {
        ...
        $standard_data =  standardModel::where(['sub_institute_id'=>$sub_institute_id])->get(["id","name"])->toArray();
        $division_data =  divisionModel::where(['sub_institute_id'=>$sub_institute_id])->get(["id","name"])->toArray();                    
    ...
    public function deleteTimetable(Request $request)
    {
        ...
        $check_timetable_data = timetableModel::where(['sub_institute_id' => $sub_institute_id,
                                                'syear' => $syear,
                                                'standard_id' => $standard_id,
                                                'division_id' => $division_id,
                                                'week_day' => $week_day,
    ...
    public function getMappingTeachers(Request $request)
    {
        ...
        $teacher_data = DB::SELECT("SELECT *
                                    FROM tbluser
                                    WHERE sub_institute_id = '".$sub_institute_id."' 
                                    AND FIND_IN_SET('".$subject_id."',subject_ids)");               
