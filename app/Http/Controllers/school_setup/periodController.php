...
class periodController extends Controller
{
    ...
    public function getData($request){
        ...
        $data = periodModel::select('period.*')
        ->where(['period.sub_institute_id'=>$sub_institute_id])		
        ->get();
        //->leftjoin('academic_section', 'academic_section.id', '=', 'period.academic_section_id')
        //->join('academic_year', 'academic_year.id', '=', 'period.academic_year_id')
        //,'academic_section.title as academic_section_name','academic_year.title as academic_year_name'
        return $data;
    }
	...
	{           
		...
		$data = DB::select("SELECT count(*) as tot FROM period WHERE sub_institute_id = '".$sub_institute_id."' AND UPPER(title) = '".$title."'");//AND academic_section_id = '".$academic_section_id."' 
    ...
    public function update(Request $request,$id){        
        ...
        $data = DB::select("SELECT count(*) as tot 
        					FROM period 
        					WHERE sub_institute_id = '".$sub_institute_id."' AND id='".$id."' ");//AND academic_section_id = '".$request->get('academic_section_id')."'
		$total_count = $data[0]->tot;
		// dd($data);
