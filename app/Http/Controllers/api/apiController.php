    ...
    public function check_otp(Request $request, JwtToken $jwt)
    {
        ...
        //START - For Hills School - Display only current year student display
			$exists = DB::table('tblstudent')
                ...
                ->get()
                ->pluck('sub_institute_id','id')
                ->toArray();

            // comment on 08/03/24 ISSUE GENERATED BY THIS
                // $is_exist = false;
                // if (count($exists) > 0) {
                //     $is_exist = true;
                // }
        ...
        if ($validator->fails()) {
            ...
            $studentData = [];            
            $exists_student_ids        = array_values(array_unique(array_keys($exists)));
            $exists_sub_institute_ids  = array_values(array_unique(array_values($exists)));
            if(!empty($exists)){
                $studentData[] = $this->studentData(1,$_REQUEST,$exists_student_ids, $exists_sub_institute_ids);
            }

            $otherExists_student_ids        = array_values(array_unique(array_keys($otherExists)));
            $otherExists_sub_institute_ids  = array_values(array_unique(array_values($otherExists)));
            if($otherExists){
            ...
            foreach ($studentData as $key => $value) {
                foreach ($value as $key1 => $value1) {
                    ...
                    if(in_array($value1->sub_institute_id, [201,202,203,204,254,324,326,327])){
                    	$first_name = strtoupper($value1->first_name);//$value1->id; //commented on 20-01-2026
...
// added by saroj uma 08/03/024
public function studentData($is_exist,$request,$student_id, $sub_institute_id){
    // echo "<pre>";print_r($student_id);print_r($sub_institute_id);exit;
    $_REQUEST = $request;
    $select = [
        "tblstudent.first_name",
        "tblstudent.id",
    ...
    $query = DB::table("tblstudent")
        ->join('tblstudent_enrollment', 'tblstudent_enrollment.student_id', '=', 'tblstudent.id')
        ->join('school_setup', function ($join) use ($sub_institute_id) {
            $join->on('school_setup.id', '=', 'tblstudent.sub_institute_id');
            if (empty(array_intersect($sub_institute_id, [201,202,203,204]))) {
                $join->on('school_setup.syear', '=', 'tblstudent_enrollment.syear');
            }
        })
        ->join('academic_section', 'tblstudent_enrollment.grade_id', '=', 'academic_section.id')
        ->join('standard', 'standard.id', '=', 'tblstudent_enrollment.standard_id')
...
// end by saroj uma 08/03/024
    ...
    public function homescreen(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $hasRenderColumns = Schema::hasColumn("mobile_homescreen", "render_type");
