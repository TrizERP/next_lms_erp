...
{    
    ...
    public function getData($request){ 
        ...
        {            
            $answer_arr = answermasterModel::where(["question_id"=>$val['id'],"sub_institute_id"=>$sub_institute_id])->get()->toArray(); 
    ...
    public function store(Request $request){
        ...
        return redirect()->route('online_exam.show', ['questionpaper_id'=>$request->get('questionpaper_id'),'online_exam_id' => $online_exam_id]);
    }
	...
	public function get_calculate_marks(Request $request){
        ...
        if(isset($answer_multiple))
        {
            ...
            {      
                $original_ans = DB::select("SELECT GROUP_CONCAT(id) AS right_answers
                    FROM answer_master
                    WHERE question_id = '".$key."' AND correct_answer = 1 ");
    ...
    {       
        ...
        if($arr == ""){
            ...
            $obtain_marks = DB::select("SELECT SUM(points) AS obtain_marks
    ...
    {        
        ...
        $data['online_exam_data'] = lmsOnlineExamModel::where(['id'=>$online_exam_id,'student_id'=>$user_id])->get()->toArray();
    ...
    {        
        ...
        $data['attempted_data'] = lmsOnlineExamModel::where(['student_id'=>$student_id,'question_paper_id'=>$questionpaper_id])->orderby('start_time')->get()->toArray();
    ...
    public function ajax_getQuestionList(Request $request)
    {
        ...
        $qData = lmsQuestionMasterModel::select('lms_question_master.id','question_title','a.ans_status')
