...
// ================= CHECK FIRST QUIZ =================
$latestExam = DB::table('lms_online_exam')
    ->where('student_id', $student_id)
    ->first();

if (!$latestExam) {
    // ✅ FIRST QUIZ → ALWAYS EASY
    $levelToUse = 'easy';
} else {
    // ✅ AFTER FIRST QUIZ → USE CURRENT LEVEL
    ...
    if ($currentLevel == 'easy') {
        ...
        $levelToUse = $currentLevel;
    }
}

// ✅ GET LEVEL ID
$selectedLevelId = $this->getLevelId($levelToUse, $sub_institute_id);
// dd($levelToUse, $selectedLevelId);
    // ================= PYTHON CALL =================
...
//   DB::enableQueryLog();
        $randomQuestions = DB::table('lms_question_master as lqm')
            ...
            ->join('lms_mapping_type as lmt', 'lmt.id', '=', 'lm.mapping_value_id')
            ->join('answer_master as am', 'lqm.id', '=', 'am.question_id')
            ->select('lqm.*', 'lm.mapping_value_id', 'lmt.name as mapping_val')
            ...
            // ✅ APPLY LEVEL FILTER ONLY IF EXISTS
            ->when($selectedLevelId, function ($query) use ($selectedLevelId) {
                $query->where('lm.mapping_value_id', $selectedLevelId);
            })
            ->groupBy('lqm.id', 'lqm.question_title')
            ->orderByRaw('RAND()')
            ->limit(10)
            ->get()
            ->toArray();
            
//   dd(DB::getQueryLog($randomQuestions));exit;
    ...
    if (!empty($questionList)) {
        ...
        if (empty($questionList)) {
            ...
            return is_mobile($type, 'pal.index', $res, "redirect");
        }
...
public function store(Request $request)
{
    ...
    $question_ids_array = $request->question_ids;
    ...
    // ================= SQL INSERT =================
    $questionPaperId = DB::table('question_paper')->insertGetId([
        'grade_id' => $grade_id,
        'standard_id' => $standard_id,
        'subject_id' => $subject_id,
        'paper_name' => $paper_name,
        'paper_desc' => $chapter_id,
        'timelimit_enable' => 1,
    ...
    try {
        ...
        foreach ($chapterWise as $chId => $data) {
            ...
            neo4jCreateRelationship(
                ...
                'Chapter',
                ['chId' => (int)$chId],
                ['proficiency_score' => (int)$score] // ✅ property
            );
        }
    ...
    // ================= ANSWERS =================
    $answer_single = $request->get('answer_multiple');
    // return $answer_single;
    $rightInterest = [];

    if (is_array($answer_single)) {
        foreach ($answer_single as $question_id => $answer_ids) {
            // $answer_ids is an array like ["483088##0","483092##0","483096##0","483100##0","483104##1"]
            foreach ($answer_ids as $answer_id_raw) {
                $arr = explode("##", $answer_id_raw);
                ...
                $ans_status = ($arr[1] == 1) ? "right" : "wrong";

                if ($ans_status == "right") {
                    $rightInterest[$interest]++;
                }

                lmsOnlineExamAnswerModel::insert([
                    'question_paper_id' => $questionPaperId,
...
$total_right = $data['online_exam_data']['total_right'] ?? 0;
$total_wrong = $data['online_exam_data']['total_wrong'] ?? 0;
...
if ($total > 0) {
    $percentage = ($total_right / $total) * 100;
...
// ✅ PASS TO BLADE
$data['online_exam_data']['student_level'] = $level;
        ...
        //dd($online_answer_data);
       // call neo4j API with assId = $online_exam_id and sub_institute_id = $sub_institute_id get data in $res['neo4j_data']
        $type = $request->input('type');
        $data['status_code'] = 1;
