...
{
    public $searchArr = array("<p>","</p>","&nbsp;","\n", "\r","'","<",'"');
    public $replaceArr = array("","","","","","","",'');
    ...
    public function getData($request){
        ...
        // DB::enableQueryLog();
        $data['basic_ids'] = sub_std_mapModel::select('standard.grade_id','sub_std_map.subject_id','sub_std_map.standard_id',
        ...
        // echo "<pre>"; print_r($content_data_arr); exit;

        // dd(DB::getQueryLog());
        // $content_data_array = [];
        // foreach ( $content_data as $content ) {
        //     $content_data_array[$content['chapter_id']][] = $content;
        // }

        // dd($content_data_array);
    ...
    public function store(Request $request){     
        ...
        foreach($chapter_name as $key => $val)
        {
            ...
            $sort_order_val = isset($sort_order[$key]) ? $sort_order[$key] : '';
        ...
        $type = $request->input('type');
        return redirect()->route('chapter_master.index', ['standard_id' => $request->get('standard'),'subject_id' => $request->get('subject')]);//->with(['data' => $res]);        
    ...
    public function update(Request $request,$id){
        ...
        $type = $request->input('type');
        //return is_mobile($type, "chapter_master.index", $res, "view");
        return redirect()->route('chapter_master.index', ['subject_id' => $request->get('subject'),'standard_id' => $request->get('standard')]);//->with(['data' => $res]);
    }
    ...
    public function show(Request $request,$id)
    {
        ...
        if($action == "subjectwise")//show subjectwise graph
        {
            ...
            $subject = DB::select("SELECT display_name FROM sub_std_map WHERE subject_id = '".$subject_id."' AND standard_id = '".$standard_id."'");
    ...
    public function get_chapter_data(Request $request,$subject_id,$standard_id,$subject_name)
    {
        ...
        $chapters = DB::select("SELECT * FROM chapter_master c WHERE 
                                c.sub_institute_id = '".$sub_institute_id."' AND c.standard_id = '".$standard_id."' AND c.subject_id = '".$subject_id."'
                                AND c.syear = '".$syear."' ");
    ...
    public function get_topic_data(Request $request,$chapter_name,$chapter_id)
    {
        ...
        $topics = DB::select("SELECT * FROM topic_master t WHERE t.sub_institute_id = '".$sub_institute_id."' AND t.chapter_id = '".$chapter_id."'
                            AND t.syear = '".$syear."' ");
    ...
    public function get_content_data(Request $request,$topic_name,$topic_id)
    {
        ...
        $contents = DB::select("SELECT * FROM content_master c WHERE 
                                c.sub_institute_id = '".$sub_institute_id."' AND c.topic_id = '".$topic_id."'
                                AND c.syear = '".$syear."' ");
    ...
    public function get_content_mapping_data(Request $request,$content_title,$content_id)
    {
        ...
        $contentMappings = DB::select("SELECT c.*,CONCAT_WS(' ',t1.name,' => ',t2.name) mapping_value 
                            FROM content_mapping_type c
                            INNER JOIN lms_mapping_type t1 ON t1.id = c.mapping_type_id
                            INNER JOIN lms_mapping_type t2 ON t2.id = c.mapping_value_id
    ...
    public function get_question_data(Request $request,$topic_name,$topic_id)
    {
        ...
        $questions = DB::select("SELECT *,SUBSTRING(q.question_title,1,20) as ques_title 
                                FROM lms_question_master q 
                                WHERE q.sub_institute_id = '".$sub_institute_id."' AND q.topic_id = '".$topic_id."' ");
    ...
    public function get_question_mapping_data(Request $request,$ques_title,$question_id)
    {
        ...
        $questionMappings = DB::select("SELECT c.*,SUBSTRING(CONCAT_WS(' ',t1.name,' => ',t2.name),1,35) as mapping_value
                            FROM lms_question_mapping c
                            INNER JOIN lms_mapping_type t1 ON t1.id = c.mapping_type_id
                            INNER JOIN lms_mapping_type t2 ON t2.id = c.mapping_value_id
                            WHERE c.questionmaster_id = '".$question_id."'");
