...
{    
    ...
    public function getData($request){ 
        ...
        $data['question_arr'] = counsellingQuestionModel::select('counselling_question_master.*','c.title as course_title')
    ...
    public function store(Request $request){        
        ...
        return redirect()->route('lmsCounsellingExam.show', ['course_id'=>$request->get('course_id'),'online_exam_id' => $online_exam_id]);
    }
        ...
        if(isset($answer_multiple))
        {
            ...
            {      
                $original_ans = DB::select("SELECT GROUP_CONCAT(id) AS right_answers
                    FROM counselling_answer_master
    ...
    {   
        ...
        $data['online_exam_data'] = counsellingOnlineExamModel::where(['id'=>$online_exam_id,'user_id'=>$user_id])->get()->toArray();
    ...
    {        
        ...
        $data['attempted_data'] = lmsOnlineExamModel::where('student_id',$student_id)->orderby('start_time')->get()->toArray();
