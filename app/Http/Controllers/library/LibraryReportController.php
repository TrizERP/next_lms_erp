...
class LibraryReportController extends Controller
{
    ...
    public function PrintBarcode(Request $request)
    {
        ...
        $data = DB::table('tblstudent as s')
                ...
                ->latest('s.id')->limit(500)
                ->get()->toArray();

        $res['details'] = $data;
        $res['print_type'] = "member";
    ...
    public function PrintBarcodeCreate(Request $request)
    {
        ...
        if ($print_type == "member") {
            $data = DB::table('tblstudent as s')
                ...
                ->where('se.syear', $syear)
                ->whereNull('se.end_date');
                // ->when(!$request->has('grade') && !$request->has('standard') && !$request->has('division') && !$request->has('from_item_code') && !$request->has('to_item_code'), function ($q) {
                //     $q->latest('s.id')->limit(500);
                // });
        } else {
            $data = DB::table('library_items as li')
                ...
                ->whereNull('li.deleted_at')
                ->where('li.sub_institute_id', $sub_institute_id)
                ->when($request->has('from_item_code') && $request->has('to_item_code'), function ($q) use ($request, $sub_institute_id) {
                    $q->where(function ($query) use ($request, $sub_institute_id) {
                        if ($sub_institute_id == 254) {
                            $query->whereBetween('li.item_code', [$request->from_item_code, $request->to_item_code]);
                        } else {
                            $prefix = substr($request->from_item_code, 0, 1);
                            $start_num = intval(substr($request->from_item_code, 1));
                            $end_num = intval(substr($request->to_item_code, 1));

                            for ($i = $start_num; $i <= $end_num; $i++) {
                                $code = $prefix . str_pad($i, 6, '0', STR_PAD_LEFT);
                                $query->orWhere('li.item_code', $code);
                            }
                        }
                    });
                });
                // , function($q){
                //     $q->latest('li.item_code')->limit(500);
                // });
        }
        ...
        $res['print_type'] = $request->print_type;
        $res['search_by'] = $request->search_by;
        $res['from_item_code'] = $request->from_item_code;
        $res['to_item_code'] = $request->to_item_code;
        return is_mobile($type, "library/reports/print_barcode", $res, "view");
    }
