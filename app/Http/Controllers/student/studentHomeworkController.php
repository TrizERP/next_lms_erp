    ...
    public function store(Request $request)
    {
        ...
        if (empty($student_details)) {
            ...
            foreach ($student_details as $id => $arr) {
                // get student level from last enrty or PAL
                $student_level = 'Easy';
                $getStudentLastEntry = studentHomeworkModel::where(['student_id' => $arr['id'], 'standard_id' => $standard_id, 'subject_id' => $subject_id, 'syear' => $syear, 'sub_institute_id' => $sub_institute_id])->orderBy('id', 'desc')->first();
                if ($getStudentLastEntry && $getStudentLastEntry->student_level !== null && $getStudentLastEntry->student_level != '') {
                    ...
                    $getPAL = DB::table('result_personalize_marks')->where(['student_id' => $arr['id'], 'standard_id' => $standard_id, 'subject_id' => $subject_id, 'syear' => $syear, 'sub_institute_id' => $sub_institute_id])->orderBy('id', 'desc')->first();
                ...
                $addhomeworkArray['student_level'] = $student_level;
