...
class studentCertificateController extends Controller
{
    ...
    public function showStudentCertificate(Request $request)
    {
        ...
        $tData = DB::select('SELECT * FROM template_master WHERE module_name ="'.$template.'" AND
        ...
        receipt_line_4,receipt_prefix,receipt_logo,last_receipt_number order by sort_order limit 1";
        $sql = preg_replace('/\n+/', '', $sql);
        ...
        foreach ($data as $key => $value) 
        {
            ...
            $certificate_no_sql = "SELECT (IFNULL(MAX(cast(c.certificate_number AS UNSIGNED)),0) + 1) AS certificate_no
                                FROM certificate_history c
                                WHERE c.sub_institute_id = '".$sub_institute_id."' AND certificate_type = '".$template."' AND syear = '".$syear."' ";
            $certificate_no_result = DB::select($certificate_no_sql);    
            $certificate_no =  $certificate_no_result[0]->certificate_no;
            $certificate_no1 = $certificate_no + $i;
            $i++;
            $html_content = $tData[0]['html_content'];
            $new_html_content = $this->create_html_content($syear,$sub_institute_id,$html_content,$value,$receipt_book_arr,$template,$certificate_no1);
    ...
    public function studentCertificateAPI(Request $request) {
       try {
        ...
        {               
            $data = DB::select("SELECT * FROM certificate_history 
    ...
    public function create_html_content($syear,$sub_institute_id,$html_content,$value,$receipt_book_arr,$template,$certificate_no)
    {
        ...
        $html_content = str_replace(htmlspecialchars("<<affiliation_no_value>>"), strtoupper($value['affiliation_no']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<school_code_value>>"), strtoupper($value['school_code']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<nationality_value>>"), strtoupper($value['nationality']), $html_content);
        ...
        $html_content = str_replace(htmlspecialchars("<<candidate_belongs_to_value>>"), strtoupper($value['candidate_belongs_to']), $html_content);
        ...
        $html_content = str_replace(htmlspecialchars("<<class_in_which_pupil_last_studied_value>>"), strtoupper($value['class_in_which_pupil_last_studied']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<last_school_board_value>>"), strtoupper($value['last_school_board']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<whether_failed_value>>"), strtoupper($value['whether_failed']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<subjects_studied_value>>"), strtoupper($value['subjects_studied']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<whether_qualified_value>>"), strtoupper($value['whether_qualified']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<if_to_which_class_value>>"), strtoupper($value['if_to_which_class']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<month_up_paid_school_dues_value>>"), strtoupper($value['month_up_paid_school_dues']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<admission_under_value>>"), strtoupper($value['admission_under']), $html_content);
        ...
        $html_content = str_replace(htmlspecialchars("<<games_played_value>>"), strtoupper($value['games_played']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<general_conduct_value>>"), strtoupper($value['general_conduct']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<date_of_application_for_certificate_value>>"), date('d-m-Y',strtotime($value['date_of_application_for_certificate'])), $html_content);        
        $html_content = str_replace(htmlspecialchars("<<date_of_issue_of_certificate_value>>"), date('d-m-Y',strtotime($value['date_of_issue_of_certificate'])), $html_content);
        $html_content = str_replace(htmlspecialchars("<<reason_leaving_school_value>>"), strtoupper($value['reason_leaving_school']), $html_content);
        
        $html_content = str_replace(htmlspecialchars("<<proof_for_dob_value>>"), strtoupper($value['proof_for_dob']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<whether_school_is_under_goverment_value>>"), strtoupper($value['whether_school_is_under_goverment']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<date_on_which_pupil_name_was_struck_value>>"), date('d-m-Y',strtotime($value['date_on_which_pupil_name_was_struck'])), $html_content);
        $html_content = str_replace(htmlspecialchars("<<any_fees_concession_value>>"), strtoupper($value['any_fees_concession']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<whether_ncc_cadet_value>>"), strtoupper($value['whether_ncc_cadet']), $html_content);
        $html_content = str_replace(htmlspecialchars("<<any_other_remarks_value>>"), strtoupper($value['any_other_remarks']), $html_content);
    ...
    public function ajax_saveData(Request $request)
    {
        ...
        FROM fees_receipt_book_master
        WHERE syear = '".$syear."'
        AND sub_institute_id = '".$sub_institute_id."' AND status=1
        GROUP BY receipt_line_1,receipt_line_2,receipt_line_3,
        receipt_line_4,receipt_prefix,receipt_logo,last_receipt_number  order by sort_order limit 1 ";
        ...
        foreach ($data as $key => $value) 
        {
            ...
            $html_content = $tData[0]['html_content'];
            $new_html_content = $this->create_html_content($syear,$sub_institute_id,$html_content,$value,$receipt_book_arr,$template,$certificate_no);
        ...
        $last_insert_ids = rtrim($last_insert_ids,',');
