...
class ConversationalAIV2 extends Component
{
    /**
     * Main API method to get student details
     */
    public function genkitDetailsAPI($enrollment_no, $action_type)
    {
        // Get session data
        ...
        // Find student IDs
        $student_id = DB::table('tblstudent as s')
            ->join('tblstudent_enrollment as ts', 'ts.student_id', '=', 's.id')
            ->where(['s.sub_institute_id' => $sub, 'ts.syear' => $year])
        ...
        foreach ($students as $key => $student) {
            ...
            if ($action_type === 'remain_fees' || $action_type === 'paid_fees') {
                // Call the fees controller for details
                try {
                    ...
                    if (isset($apiResponse['status']) && $apiResponse['status'] == 1 && isset($apiResponse['data'])) {
                        ...
                        $html = '<div class="text-muted p-2">No records found for enrollment number: ' . htmlspecialchars($enrollment_no) . '</div>';
                ...
                $url = "https://kgenkit.vercel.app/api/genkit-k12?enrollment_no={$enrollment_no}&action={$action_type}&syear={$year}&sub_institute_id={$sub}&student_id={$student['id']}";
                ...
                try {
                    ...
                    if (empty($data) || !is_array($data)) {
                        ...
                        $html = $this->formatDetailsHTML($action_type, $data);
                    }
                    $allData[] = $data;
                } catch (\Exception $e) {
                    $html = '<div class="text-muted p-2">Error fetching data: ' . htmlspecialchars($e->getMessage()) . '</div>';
        ...
        return ['action_type' => $action_type, 'raw_data' => $allData, 'html' => $allHtml];
    }
    
    /**
     * Common HTML formatting function for all detail types
     */
    private function formatDetailsHTML($actionType, $data)
    {
        switch ($actionType) {
            case 'fees_details':
    ...
    /**
     * Mock Queries API method
     */
    private function MockQueriesAPI($sentence)
    {
        $type = request()->type;
    ...
    /**
     * Get field map for different tables
     */
    ...
    /**
     * Generate HTML for fees details
     */
    ...
    /**
     * Generate HTML for remaining/unpaid fees
     */
    ...
    {
        return view('livewire.conversational-a-i-v2', ['intents' => $this->getIntentsList()['intents']]);
    }
}