...
class EsoLearningContentResolver
{
    ...
    /**
     * The first-look teach step: the content model, then a real video.
     *
     * The Learn stage is `['teach', 'reteach']`, and this is its front half.
     * A concept's first explanation is exactly where a good video earns the
     * most — the student has no prior model to fall back on — so teach takes
     * the best-matching video available.
     */
    public function forTeach(ConceptNode $node, LearnerNodeState $state, ?int $subInstituteId = null): ?array
    {
        return $this->withVideo($node, $state, $subInstituteId, 0);
    }

    /**
     * The reteach composition, one rung further down the same list.
     *
     * The offset is what keeps "let's try that a different way" honest. Teach
     * has already shown rank 0, so a first failed check moves to rank 1, a
     * second to rank 2, and so on — the student gets a genuinely different
     * explanation rather than the video they just failed to learn from, and it
     * needs no new learner state because the ordering is deterministic.
     */
    public function forReteach(ConceptNode $node, LearnerNodeState $state, ?int $subInstituteId = null): ?array
    {
        // cfu_attempts is 1 on the first reteach, and teach consumed rank 0.
        return $this->withVideo($node, $state, $subInstituteId, max(1, (int) $state->cfu_attempts));
    }

    /**
     * Compose the content model with an approved video.
     *
     * The order below is the whole policy:
     *
     *  R1  An authored override that carries media wins. A human deliberately
     *      attached that asset to this node; a discovered video does not
     *      outrank a decision someone actually made.
     *  R2  Otherwise an approved video for the concept, if one exists. This
     *      runs whether or not the content model returned anything, which is
     *      what makes the feature reach the ~90% of concepts whose chapter has
     *      never been through the extractor.
     *  R3  Otherwise exactly the pre-existing behaviour, including null.
     *
     * This is a separate entry point rather than a change to forNode() on
     * purpose: forNode() still serves the D5 retrieval refresher, which is a
     * memory jog before a check rather than a teaching screen, and should not
     * silently acquire a video because the Learn stage did.
     */
    protected function withVideo(ConceptNode $node, LearnerNodeState $state, ?int $subInstituteId, int $rankOffset): ?array
    {
        $base = $this->forNode($node, $state, $subInstituteId);

        if ($base !== null && ($base['media_url'] ?? null) !== null) {
            return $base;
        }

        $video = $this->videos->forConcept((int) $node->concept_id, $subInstituteId, $rankOffset);

        // Past the end of the list. Rather than drop to text — which on this
        // estate usually means nothing at all — fall back to the best video we
        // have. A student re-watching the one good explanation is better served
        // than one handed a blank screen.
        if ($video === null && $rankOffset > 0) {
            $video = $this->videos->forConcept((int) $node->concept_id, $subInstituteId, 0);
        }
        ...
        // The derived variant's extracted text, when there is any, becomes the
        // supporting copy under the player rather than being thrown away.
        $body = trim((string) ($base['body'] ?? ''));
        if ($body === '') {
            $body = trim((string) ($video['body'] ?? ''));
        }

        $video['body'] = $body !== '' ? $body : null;
        $video['title'] = $video['title'] ?? ($base['title'] ?? null);
