...
class tblstudentModel extends Model
{
    ...
    public function issuedBookItem(){
        ...
        return $this->hasMany(LibraryBookCirculation::class, 'student_id', 'id')
                    ->join('library_items',function($join) use ($syear){ 
                        $join->on('library_book_circulations.book_id', '=', 'library_items.book_id')
                        ->on('library_items.id','=','library_book_circulations.item_code');
                    })
