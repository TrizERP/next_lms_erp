...
final class FeesSignalRules
{
    ...
    public function applicable(): array
    {
        ...
        return [
            ...
            'fee_payment_failures' => fn () => $this->paymentFailures(),
            'fee_gateway_reconciliation_gap' => fn () => $this->gatewayReconciliationGap(),
            'fee_nach_mandate_coverage' => fn () => $this->nachMandateCoverage(),
            'fee_configured_late_backlog' => fn () => $this->configuredLateBacklog(),
            'fee_cancellation_reasons' => fn () => $this->cancellationReasonsPattern(),
            'fee_revision_impact' => fn () => $this->feeRevisionImpact(),
        ];
    }
    ...
    private function paymentFailures(): array
    {
        $failures = $this->fees->paymentFailures();
        if (! $failures['available']) {
            return $this->skip('failures_not_recorded');
        }
        if ($failures['failureCount'] < 1) {
            return $this->skip('no_failures');
        }

        $evidence = [$this->writer->recordEvidence([
            'source' => 'vivek_erp.tblstudent_fees_failure',
            'failures' => $failures['failureCount'],
            'failedAmount' => $failures['failedAmount'],
            'affectedAccounts' => $failures['affectedAccounts'],
            'repeatFailureAccounts' => $failures['repeatFailureAccounts'],
            'primaryReason' => $failures['reasons'][0]['reason'] ?? 'Unspecified',
            'syear' => $this->syear,
            'currency' => 'INR',
        ])];
        ...
        return $this->writer->raise([
            'source' => 'vivek_erp.tblstudent_fees_failure',
            'classification' => 'collection_risk',
            'severity' => $failures['failedAmount'] > 100000 || $failures['repeatFailureAccounts'] >= 5 ? 'critical' : 'high',
            'priority' => 'high',
            'confidence' => 0.95,
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_payment_failures',
                'title' => sprintf(
                    '%d payment %s failed totaling %s across %d %s',
                    $failures['failureCount'],
                    $failures['failureCount'] === 1 ? 'attempt' : 'attempts',
                    Narrative::money($failures['failedAmount']),
                    $failures['affectedAccounts'],
                    $failures['affectedAccounts'] === 1 ? 'account' : 'accounts'
                ),
                'syear' => $this->syear,
                'affectedCount' => $failures['affectedAccounts'],
                'impactAmount' => $failures['failedAmount'],
                'unit' => 'accounts',
            ],
        ], $evidence);
    }

    /* ================================================== gateway reconciliation gap */

    private function gatewayReconciliationGap(): array
    {
        $recon = $this->fees->reconciliation();
        if (! $recon['available']) {
            return $this->skip('reconciliation_not_recorded');
        }
        if ($recon['reconciliationGapAmount'] < 1.0 && $recon['unmatchedCount'] < 1) {
            return $this->skip('no_gateway_gap');
        }

        $evidence = [$this->writer->recordEvidence([
            'source' => 'vivek_erp.fees_reconciliation',
            'gatewayTransactions' => $recon['gatewayTransactions'],
            'gatewayAmount' => $recon['gatewayTotalAmount'],
            'erpAmount' => $recon['erpRecordedAmount'],
            'gapAmount' => $recon['reconciliationGapAmount'],
            'unmatchedCount' => $recon['unmatchedCount'],
            'currency' => 'INR',
        ])];
        ...
        return $this->writer->raise([
            ...
            'severity' => $recon['reconciliationGapAmount'] > 50000 ? 'critical' : 'high',
            'priority' => 'high',
            'confidence' => 1.0,
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_gateway_reconciliation_gap',
                'title' => sprintf(
                    '%s gateway settlement gap detected across %d online %s',
                    Narrative::money($recon['reconciliationGapAmount']),
                    $recon['gatewayTransactions'],
                    $recon['gatewayTransactions'] === 1 ? 'transaction' : 'transactions'
                ),
                'syear' => $this->syear,
                'affectedCount' => $recon['unmatchedCount'],
                'impactAmount' => $recon['reconciliationGapAmount'],
                'unit' => 'transactions',
            ],
        ], $evidence);
    }

    /* ================================================== NACH mandate coverage */

    private function nachMandateCoverage(): array
    {
        $mandates = $this->fees->bankMandates();
        if (! $mandates['available'] || $mandates['totalEligible'] < 10) {
            return $this->skip('mandates_not_recorded_or_low_population');
        }

        $cov = $mandates['coveragePercent'];
        if ($cov === null || $cov >= 75.0) {
            return $this->skip('adequate_mandate_coverage');
        }

        $evidence = [$this->writer->recordEvidence([
            'source' => 'vivek_erp.tblstudent_bank_detail',
            'registered' => $mandates['registeredMandates'],
            'eligible' => $mandates['totalEligible'],
            'coveragePercent' => $cov,
            'rejected' => $mandates['rejectedMandates'],
            'pending' => $mandates['pendingMandates'],
        ])];

        $uncovered = max(0, $mandates['totalEligible'] - $mandates['registeredMandates']);

        return $this->writer->raise([
            'source' => 'vivek_erp.tblstudent_bank_detail',
            'classification' => 'collection_readiness',
            'severity' => $cov < 40.0 ? 'high' : 'medium',
            'priority' => 'medium',
            'confidence' => 0.9,
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_nach_mandate_coverage',
                'title' => sprintf(
                    'Bank mandate coverage is %s%% — %d eligible %s lack active auto-debit registration',
                    number_format($cov, 1),
                    $uncovered,
                    $uncovered === 1 ? 'account' : 'accounts'
                ),
                'syear' => $this->syear,
                'affectedCount' => $uncovered,
                'impactAmount' => null,
                'unit' => 'accounts',
            ],
        ], $evidence);
    ...
    /* ============================================== configured late date backlog */

    private function configuredLateBacklog(): array
    {
        $late = $this->fees->lateRules();
        if (! $late['available']) {
            return $this->skip('no_late_rules');
        }
        if ($late['overdueAccountsPastConfiguredDate'] < 1) {
            return $this->skip('no_overdue_past_late_date');
        }

        $evidence = [$this->writer->recordEvidence([
            'source' => 'vivek_erp.fees_late_master',
            'overdueAccounts' => $late['overdueAccountsPastConfiguredDate'],
            'overdueAmount' => $late['overdueAmountPastConfiguredDate'],
            'rulesCount' => $late['rulesCount'],
            'currency' => 'INR',
        ])];
        ...
        return $this->writer->raise([
            ...
            'severity' => $late['overdueAmountPastConfiguredDate'] > 200000 ? 'critical' : 'high',
            'priority' => 'high',
            'confidence' => 0.95,
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_configured_late_backlog',
                'title' => sprintf(
                    '%d %s overdue past configured institutional cutoff (%s in arrears)',
                    $late['overdueAccountsPastConfiguredDate'],
                    $late['overdueAccountsPastConfiguredDate'] === 1 ? 'account is' : 'accounts are',
                    Narrative::money($late['overdueAmountPastConfiguredDate'])
                ),
                'syear' => $this->syear,
                'affectedCount' => $late['overdueAccountsPastConfiguredDate'],
                'impactAmount' => $late['overdueAmountPastConfiguredDate'],
                'unit' => 'accounts',
            ],
        ], $evidence);
    }

    /* ============================================== cancellation reasons concentration */

    private function cancellationReasonsPattern(): array
    {
        $adj = $this->fees->adjustments();
        if (! $adj['available'] || empty($adj['cancellationReasons'])) {
            return $this->skip('no_cancellation_reasons');
        }

        $top = $adj['cancellationReasons'][0];
        if ($top['count'] < 3) {
            return $this->skip('too_few_cancellations');
        }
        ...
        $evidence = [$this->writer->recordEvidence([
            ...
            'count' => $top['count'],
            'amount' => $top['amount'],
            'totalCancelled' => $adj['cancelledAmount'],
            'currency' => 'INR',
        ])];
        ...
        return $this->writer->raise([
            ...
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_cancellation_reasons',
                'title' => sprintf(
                    'Cancellation concentration: %d receipts voided due to "%s" (%s)',
                    $top['count'],
                    $top['reason'],
                    Narrative::money($top['amount'])
                ),
                'syear' => $this->syear,
                'affectedCount' => $top['count'],
    ...
    private function feeRevisionImpact(): array
    {
        $rev = $this->fees->feeRevisions();
        if (! $rev['available'] || $rev['revisionCount'] < 1) {
            return $this->skip('no_fee_revisions');
        }

        $evidence = [$this->writer->recordEvidence([
            'source' => 'vivek_erp.fees_breackoff_logs',
            'revisions' => $rev['revisionCount'],
            'standardsAffected' => $rev['affectedStandardsCount'],
            'syear' => $this->syear,
        ])];
        ...
        return $this->writer->raise([
            ...
            'relatedEntityType' => 'Fees',
            'metadata' => [
                'rule' => 'fee_revision_impact',
                'title' => sprintf(
                    '%d mid-session fee structure %s recorded across %d %s',
                    $rev['revisionCount'],
                    $rev['revisionCount'] === 1 ? 'modification' : 'modifications',
                    $rev['affectedStandardsCount'],
                    $rev['affectedStandardsCount'] === 1 ? 'class' : 'classes'
                ),
                'syear' => $this->syear,
                'affectedCount' => $rev['affectedStandardsCount'],
                'impactAmount' => null,
                'unit' => 'classes',
            ],
        ], $evidence);
